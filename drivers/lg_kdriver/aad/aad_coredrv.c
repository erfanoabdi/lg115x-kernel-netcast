...
LX_AAD_A2_MONO_ST_STATE_T		stCurMonoStState = LX_AAD_A2_DEFAULT0_STATE;
...
int AAD_SIF_SetBandSetup ( LX_AAD_SIF_SOUNDSYSTEM_T eSoundSystem )
{
	...
	if ( g_eAadInitType == LX_AAD_INIT_ATSC_A2_SELECT )
	{
		...
		if(g_i_eSystem == eSoundSystem)
		{
			...
			stCurMonoStState = LX_AAD_A2_DEFAULT0_STATE;
...
int AAD_SIF_SetUserAnalogMode ( LX_ATV_AUDIO_MODE_SET_T eAudioModeSet )
{
	...
	UINT8 *debugStrInit[9]  = {"TYPE_NONE",   "ATSC_A2", 		"ATSC_BTSC",   	"DVB",		"DVB_ID",	\
							   "DVB_IN(DIA)", "DVB_CN", 		"BTSC_US",		"TYPE_MAX" };
...
int AAD_SIF_SetThresholdLevel ( LX_AAD_SIF_THRESHOLD_T level )
{
	...
	if(level.mode & LX_AAD_THRES_ID_MAGNITUDE)
	{
		...
		{
			//level.stereoIDMagHigh = (11 + 4) + 17;	//0x20(32) : UI 11 + 4 + 17 = 32
			//level.stereoIDMagLow  = level.stereoIDMagHigh - 6;	//0x1A(26)
			g_stThresholdLevel.stereoIDMagHigh = level.stereoIDMagHigh + 17;
			g_stThresholdLevel.stereoIDMagLow  = g_stThresholdLevel.stereoIDMagHigh - 6;
		}
		else
		{
			//level.stereoIDMagHigh = 11 + 4;		//0x0F(15) : UI 11 + 4 = 15
			//level.stereoIDMagLow  = level.stereoIDMagHigh - 3;	//0x0B(11)
...
int AAD_SIF_GetThresholdLevel ( LX_AAD_SIF_THRESHOLD_T	*pLevel )
{
	...
	{
		//Get a BTSC and A2 Threshold level
		pLevel->stereoIDMagHigh = g_stThresholdLevel.stereoIDMagHigh;
		pLevel->stereoIDMagLow  = g_stThresholdLevel.stereoIDMagLow;
	...
	switch ( ui8SysSel )
	{
		...
		case 0x8://FM-Stereo (A2, M/N)
		{
			...
			if(ui8PrmCarDet == 1)
			{
				...
				else if( (ui16Carrier2Quality <= CAR2_QUAL_THRES_HI_ILSAN_MAX)	\
					   &&(ui16Carrier2Quality >= CAR2_QUAL_THRES_HI_ILSAN_MIN) )	//0x400 ~ 0x5FF, Ulsan Ilsan-Dong field stream issue
				{
					//Org. threshold high is 0x64 and low is 0x14.
					AAD_RdFL(id_mag_thd_hi);
...
int AAD_SetManualVolumeForHDEVMode ( LX_ATV_AUDIO_MODE_SET_T eAudioModeSet )
{
	...
	//Restore a AAD I2S Gain Control for High Deviation Off Mode.
	//AAD_SIF_SetHighDeviationMode function.
...
int AAD_Monitor_Task( void *i_pvParam )
{
	...
	do
	{
		...
		{
			//Enable AAOS(Automatic Audio Output Select) if audio system is not PAL D/K system.
			if(g_i_eSystem == LX_AAD_SIF_SYSTEM_DK && g_suiAadWaitQForMonitor == LX_AAD_TASK_START)
			{
				OS_LockMutex(&g_AAD_Sema);
...
static int AAD_NTSC_MN_A2_Monitor_Task( void )
{
	...
	switch (stCurMonoStState)
	{
		case LX_AAD_A2_DEFAULT0_STATE:
			stCurMonoStState = LX_AAD_A2_WAIT1_STATE;
			break;

		case LX_AAD_A2_WAIT1_STATE:
			#ifdef AAD_ATSC_A2_TASK_DEBUG
			AAD_DEBUG( "AAD1-1> Cur = 0x%X, Prev = 0x%X, CMo = %d, CSt = %d, St = %d\n",	\
			ui32CurStatus, ui8PrevStatus, ui32ContinuousMonoCount, ui32ContinuousStereoCount, stCurMonoStState );
			#endif

			if(ui32CurStatus == 0x13 && ui8PrevStatus == 0x13)
			{
				stCurMonoStState = LX_AAD_A2_SEQ3_ST_STATE;
			}
			else if(ui32CurStatus == 0x3 && ui8PrevStatus == 0x3)
			{
				stCurMonoStState = LX_AAD_A2_SEQ2_MONO_STATE;
			}
		...
		case LX_AAD_A2_SEQ3_ST_STATE:
			...
			else if(ui32CurStatus == 0x3)
			{
				...
				{
					stCurMonoStState = LX_AAD_A2_SEQ4_MONO_STATE;
				}
			}
	...
	AAD_DEBUG( "AAD> g_MonitorAudioMode = 0x%02X, Cur = 0x%02X, Prev = 0x%02X, CntMo = %02d, CntSt = %02d, State = %02d\n",	\
				g_MonitorAudioMode, ui32CurStatus, ui8PrevStatus, ui32ContinuousMonoCount, ui32ContinuousStereoCount, stCurMonoStState );
#endif
