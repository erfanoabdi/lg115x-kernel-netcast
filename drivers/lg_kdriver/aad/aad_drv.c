...
UINT8 *ui8AadPrintType[6]  = { "AAD_PRINT", "AAD_TRACE", "AAD_ISR_DEBUG",	\
							   "AAD_ERROR", "AAD_DEBUG", "AAD_DEBUG_TMP" };

/* AAD Debug Print Color */
UINT8 *ui8AadPrintColor[8] = {
			"COLOR_BLACK",
			"COLOR_RED",
			"COLOR_GREEN",
			"COLOR_YELLOW",
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,36)) || !defined(SUPPORT_AAD_UNLOCKED_IOCTL)
...
#if 0
	//Memorize the I/O register values
	memcpy ((void*) &g_stAadBootReg, (void*) g_pstAADReg, sizeof (AAD_REG_T));

	//Temp test code
	memset ( (void*) g_pstAADReg, 0x0, sizeof (AAD_REG_T) );
#endif
...
static int AAD_resume(struct platform_device *pdev)
{
	...
	//memcpy ((void*)g_pstAADReg, (void*)&g_stAadBootReg, sizeof (AAD_REG_T));
...
#if	1	//temp
	OS_DEBUG_DisableModuleByIndex(g_aad_debug_fd, 0);					//AAD_PRINT
	OS_DEBUG_DisableModuleByIndex(g_aad_debug_fd, 1);					//AAD_TRACE
	OS_DEBUG_DisableModuleByIndex(g_aad_debug_fd, 2);					//AAD_ISR_DEBUG
	OS_DEBUG_EnableModuleByIndex (g_aad_debug_fd, 3, DBG_COLOR_NONE);	//AAD_ERROR
	OS_DEBUG_DisableModuleByIndex(g_aad_debug_fd, 4);					//AAD_DEBUG
//	OS_DEBUG_EnableModuleByIndex (g_aad_debug_fd, 4, DBG_COLOR_NONE);	//AAD_DEBUG
	OS_DEBUG_DisableModuleByIndex(g_aad_debug_fd, 5);					//AAD_DEBUG_TMP
#else	//all enable for debug
	OS_DEBUG_EnableModuleByIndex(g_aad_debug_fd, 0, DBG_COLOR_GREEN );	//AAD_PRINT
	OS_DEBUG_EnableModuleByIndex(g_aad_debug_fd, 1, DBG_COLOR_GREEN );	//AAD_TRACE
	OS_DEBUG_EnableModuleByIndex(g_aad_debug_fd, 2, DBG_COLOR_GREEN );	//AAD_ISR_DEBUG
	OS_DEBUG_EnableModuleByIndex(g_aad_debug_fd, 3, DBG_COLOR_RED );	//AAD_ERROR
	OS_DEBUG_EnableModuleByIndex(g_aad_debug_fd, 4, DBG_COLOR_GREEN);	//AAD_DEBUG
	OS_DEBUG_EnableModuleByIndex(g_aad_debug_fd, 5, DBG_COLOR_GREEN);	//AAD_DEBUG_TMP
#endif	//#if	1
	...
	else
	{
		if(platform_device_register(&aad_device))
		{
			...
			AAD_PRINT("[%s] platform device register failed\n",AAD_MODULE);
		}
		else
		{
	...
	{
		AAD_DEBUG_TMP ("LX_CHIP_REV(L9, XX)(0x%X)\n", lx_chip_rev());
...
AAD_Open(struct inode *inode, struct file *filp)
{
	...
	if ( my_dev->dev_open_count == 0 )
	{
		...
		{
			iErrReturn = request_irq(pstAadRegCfg->irq_num, (irq_handler_t)aad_interrupt, 0, "AAD", NULL);
...
AAD_Close(struct inode *inode, struct file *file)
{
	...
	if ( my_dev->dev_open_count <= 0 )
	{
		if (lx_chip_rev() >= LX_CHIP_REV(L9, A0))
		{
			free_irq(pstAadRegCfg->irq_num, NULL);
...
#endif
{
	...
	switch(cmd)
	{
 		...
 		case AAD_IOW_DEBUG_PRINT_CTRL:
		{
			...
			if( (i_stDebugCtrl.printType >= 6) || (i_stDebugCtrl.printColor >= 8) )
			{
