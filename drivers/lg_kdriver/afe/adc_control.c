...
int ADC_Periodic_Task(void)
{
	...
	*/
	...
	if(1)
	{
		if( ( ( lx_chip_rev( ) < LX_CHIP_REV( H13, B0 )) && ( gADC_type == LX_ADC_INPUT_SOURCE_RGB_SCART) && (CVD_Get_VLock_Flag() > 0) && (CVD_Get_HLock_Flag() > 0) ) \
			|| (( lx_chip_rev( ) >= LX_CHIP_REV( H13, B0 )) &&  ( gADC_type == LX_ADC_INPUT_SOURCE_RGB_SCART) && ( ADC_Check_Sync_Exist() == 0) ) \
		...
		if( ( (gADC_type == LX_ADC_INPUT_SOURCE_RGB_PC) && (abs(previous_size_inform.ext_vsize - adc_size.ext_vsize) > 5)) \
				||  ( (gADC_type != LX_ADC_INPUT_SOURCE_RGB_PC) && (abs(previous_size_inform.ext_vsize - adc_size.ext_vsize) > 20 /*30*/)) \
				|| (abs(previous_size_inform.vd_vsize - adc_size.vd_vsize) > 3000) || (abs(previous_size_inform.vd_hsize - adc_size.vd_hsize) > 5) \
				/* || (prev_size_transition_flag> 0) */ )
		{
...
int ADC_Get_Component_Pseudo_Pulse(LX_AFE_ADC_COMP_PSEUDO_PULSE_T *pComp_PSP_t)
{
	...
	if ( lx_chip_rev( ) >= LX_CHIP_REV( H13, A0 ) )
		ADC_H13Ax_Get_Line_Count(&LineCount);
	else 
		LineCount = 0;
...
int ADC_Get_ACE_FB_Status(LX_AFE_SCART_MODE_T *pSCMode)
{
	...
	if ( lx_chip_rev( ) >= LX_CHIP_REV( H13, B0 ) )
		ADC_H13Ax_Read_ACE_GPIO(&gpio_status);
