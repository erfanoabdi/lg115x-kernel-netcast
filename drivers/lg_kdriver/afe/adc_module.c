...
#define L9_SYNC_LEVEL_100	0xC0
#define L9_SYNC_LEVEL_108	0xBC
...
static int ADC_Thread(void *data)
{
	while(1)
	{
		...
		{
			CVD_DEBUG("force sleep ADC Thread\n");
...
int ADC_Initialize(void)
{
	...
	if (lx_chip_rev() >= LX_CHIP_REV(H13, A0)) 
	{
		...
		ADC_PowerOn = 			ADC_H13Ax_PowerOn;
		ADC_PowerOff =                 ADC_H13Ax_PowerOff;
		ADC_InitDaughterBoard=         ADC_H13Ax_InitDaughterBoard;
...
#ifdef	KDRV_CONFIG_PM
		ADC_RunSuspend=                ADC_H13Ax_RunSuspend;
		ADC_RunResume=                 ADC_H13Ax_RunResume;
#endif

	}
	else if (lx_chip_rev() >= LX_CHIP_REV(L9, B0)) // added by won.hur(won.hur@lge.com) @2011.08.17
	{
#ifdef INCLUDE_L9_CHIP_KDRV
		g_comp_sync_level_108 = L9_SYNC_LEVEL_108;
		g_comp_sync_level_100 = L9_SYNC_LEVEL_100;
		...
		ADC_Mute_Control	=	ADC_L9Bx_Mute_Control;
		ADC_LVDS_Channel_Enable_Control	=	ADC_L9Bx_LVDS_Channel_Enable_Control;
		ADC_Post_Gain_Control	=	ADC_L9Bx_Post_Gain_Control;
		ADC_Set_Comp_Params	=	ADC_L9Bx_Set_Comp_Params;
		ADC_Read_Sync_Low_Level	=	ADC_L9Bx_Read_Sync_Low_Level;
		ADC_Enable_Sync_Low_Level_Read	=	ADC_L9Bx_Enable_Sync_Low_Level_Read;
		ADC_Check_LLPLL_status	=	ADC_L9Bx_Check_LLPLL_status;
		ADC_LVDS_Src_Control	=	ADC_L9Bx_LVDS_Src_Control;
		ADC_Read_ACE_Reg	=	ADC_L9Bx_Read_ACE_Reg;
		ADC_Write_ACE_Reg	=	ADC_L9Bx_Write_ACE_Reg;
#ifdef	KDRV_CONFIG_PM
