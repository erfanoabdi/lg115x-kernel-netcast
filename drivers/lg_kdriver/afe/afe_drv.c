...
//	OS_DEBUG_DisableModule ( g_afe_debug_fd );
	...
	OS_DEBUG_EnableModuleByIndex ( g_afe_debug_fd, 0, DBG_COLOR_NONE );	//AFE_PRINT
	OS_DEBUG_DisableModuleByIndex ( g_afe_debug_fd, 1 );				//CVD_DEBUG
	OS_DEBUG_DisableModuleByIndex ( g_afe_debug_fd, 2 );				//ADC_DEBUG
	OS_DEBUG_DisableModuleByIndex ( g_afe_debug_fd, 3 );				//AFE_TRACE
	OS_DEBUG_EnableModuleByIndex (g_afe_debug_fd, 4, DBG_COLOR_NONE );	//AFE_ERROR
	OS_DEBUG_DisableModuleByIndex(g_afe_debug_fd, 5);					//CVD_CSD_DEBUG
	OS_DEBUG_DisableModuleByIndex(g_afe_debug_fd, 6);					//CVD_WA_DEBUG
	OS_DEBUG_DisableModuleByIndex(g_afe_debug_fd, 7);					//ADC_CALIB_DEBUG
	OS_DEBUG_DisableModuleByIndex(g_afe_debug_fd, 8);					//ADC_APA_DEBUG
	OS_DEBUG_DisableModuleByIndex(g_afe_debug_fd, 9);					//ADC_IFM_DEBUG
	OS_DEBUG_DisableModuleByIndex(g_afe_debug_fd, 10);					//CVD_THREAD_DEBUG
	OS_DEBUG_DisableModuleByIndex(g_afe_debug_fd, 11);					//ADC_THREAD_DEBUG
...
//			AFE_PRINT("CVD GET STATE\n");
		...
		//	AFE_PRINT("CVD Set Scart Overlay\n");
			ret = CVD_Set_Scart_Overlay((BOOLEAN)arg);
			break;

		case AFE_IOW_CVD_SET_SOURCE_TYPE:
			ADC_DEBUG("CVD Set Source Type\n");
			ret = CVD_Set_Source_Type_Control(arg);
			break;
		...
		//	AFE_PRINT("CVD Get FB Status\n");
			CVD_Get_FB_Status(arg);
#if 0
			unused = (void *)kmalloc(sizeof(4),GFP_KERNEL);
			mode = (LX_AFE_SCART_MODE_T *)kmalloc(sizeof(LX_AFE_SCART_MODE_T) , GFP_KERNEL);

			ret = copy_from_user((void *)mode, (void *)arg, sizeof(LX_AFE_SCART_MODE_T));
			switch(*mode)
			{
				case LX_SCART_MODE_RGB:
					ADC_IBD(unused);
				break;

				case LX_SCART_MODE_CVBS:
					ADC_AAD(unused);
				break;

				default:
					AFE_PRINT("%s %d wrong param\n", __func__ , __LINE__);
				break;
		...
		break;
		 ...
		 case AFE_IOW_VPORT_OUTPUT:
			...
			ret = CVD_Vport_Output(arg);
		 break;

		case AFE_IOR_VPORT_REG_READ:
			//AFE_PRINT("AFE REG READ\n");
			ret = CVD_Vport_Reg_Read(arg);
			break;

		case AFE_IOW_VPORT_REG_WRITE:
			ADC_DEBUG("AFE REG WRITE\n");
			CVD_Vport_Reg_Write(arg);
			ret=0;
			break;
		...
		case AFE_IOW_CVD_PROGRAM_COLOR_SYSTEM:
			...
			ret = CVD_Program_Color_System(arg);
			break;
 		...
 		case AFE_IOW_DEBUG_PRINT_CTRL:
			{
				...
				if( (i_stDebugCtrl.printType >= NUM_OF_AFE_PRINT) || (i_stDebugCtrl.printColor >= NUM_OF_AFE_PRINT_COLOR) )
				{
				...
				if ( i_stDebugCtrl.printEnable == 0)
				{
					...
					AFE_PRINT("AFE_PRINT_TYPE[%s] is disabled.\n", ui8AfePrintType[i_stDebugCtrl.printType]);
				}
				else
				{
					OS_DEBUG_EnableModuleByIndex(g_afe_debug_fd, i_stDebugCtrl.printType, i_stDebugCtrl.printColor);
					AFE_PRINT("AFE_PRINT_TYPE[%s] is enabled Color is [%s].\n", ui8AfePrintType[i_stDebugCtrl.printType], ui8AfePrintColor[i_stDebugCtrl.printColor]);
				}
		...
		case AFE_IOW_CVD_CHANNEL_CHANGE:
			{
				...
				if ( lx_chip_rev( ) >= LX_CHIP_REV( H13, B0 ) )
					ret =	CVD_H13Bx_Channel_Change_Control(bATV_Channel_Change);
				else
					ret = 0;
		...
		case AFE_IOW_CVD_VDAC_MUTE_CONTROL:
			{
				...
				if ( lx_chip_rev( ) >= LX_CHIP_REV( H13, B0 ) )
					ret =	CVD_H13Bx_VDAC_mute_control(mute_enable);
				else
					ret = 0;
			}
			break;
