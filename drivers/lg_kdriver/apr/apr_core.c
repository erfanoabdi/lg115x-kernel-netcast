...
#define APR_ERROR	printk
#define APR_PRINT		printk
...
#define APR_VSYNC_Register DE_VSYNC_Register
...
#ifdef APR_USE_NEW_ALGO
//static int APR_DetectSourceChange(void);
#endif
...
int APR_Core_Init(LX_APR_INIT_T *pstParams)
{
	...
	do {
		...
		APR_VSYNC_Register(APR_DetectHandler);
#else
		APR_THREAD_Register(APR_DetectHandler);
		APR_VSYNC_Register(APR_VsyncHandler);
#endif
		APR_HAL_InitInterrupt(TRUE);
...
static int APR_SetThreshold(APR_THR_T *pstParams)
{
	...
	do {
		...
		{
			APR_PRINT("isDefault [%d]\n", pstParams->isDefault);
			APR_PRINT("tpd_thr [%d]\n", pThr->tpd_thr);
			APR_PRINT("ptn_thr [%d]\n", pThr->ptn_thr);
			APR_PRINT("mtn_thr_2d [%d]\n", pThr->mtn_thr_2d);
			APR_PRINT("mtn_thr_3d [%d]\n", pThr->mtn_thr_3d);
			APR_PRINT("fmt_th1 [%d]\n", pThr->fmt_th1);
			APR_PRINT("fmt_th2 [%d]\n", pThr->fmt_th2);
			APR_PRINT("fmt_th3 [%d]\n", pThr->fmt_th3);
			APR_PRINT("fmt_th4 [%d]\n", pThr->fmt_th4);
...
static int APR_CheckConfig(void)
{
	...
	do {
		...
		if((cur_scan_info != pCfg->fd3.scan) || (cur_color_sample != pCfg->fd3.color) || (cur_input_height != pCfg->in_win_y))
		{
			...
			if(_g_apr_trace >= LX_APR_CMD_LOG)
				APR_PRINT("%s : scan_info  %d , color_sample  %d , in_win_y  %d\n", __F__, pCfg->fd3.scan, pCfg->fd3.color, pCfg->in_win_y);
			if(!ret) ret = RET_TRY_AGAIN;
		}
		...
		if(pCfg->mute_status == 1)
		{
			...
			if(_g_apr_trace >= LX_APR_CMD_LOG)
				APR_PRINT("mute on -  initialize voting buffer to all 2D\n");
		}
	} while(0);
									...
									UINT32 lr_crop)
{
	...
	do {
		...
		{
			if(pCfg->cap.enb >= LX_APR_CAP_LOG) APR_PRINT("%s : pFrameBase = 0x%08x\n", __F__, (UINT32)ov_info->pFrameBase);
			break;
		}
		if(ov_info->src_valid == FALSE)
		{
			if(pCfg->cap.enb >= LX_APR_CAP_LOG) APR_PRINT("%s : src_valid [ %d ]\n", __F__, ov_info->src_valid);
			ret = RET_ERROR;
			break;
		}
		if((h_line_num > APR_MAX_LINE) || (h_line_width > APR_MAX_STRIP_WIDTH) || (ov_info->v < (top_offset<<1)))
		{
			if(pCfg->cap.enb >= LX_APR_CAP_LOG) APR_PRINT("h_size[%d]/v_size[%d]/top_offset[%d]/bot_offset[%d]/lr_crop[%d]/h_line_num[%d]/h_line_width[%d]\n", \
		...
		{
			if(pCfg->cap.enb >= LX_APR_CAP_FMT) APR_PRINT("sampling_size  %d\n", sampling_size);
			ret = RET_ERROR;
			break;
		}

		if(pCfg->cap.enb == LX_APR_CAP_LOG) APR_PRINT("pFrameBase : 0x%08X, h/v/s[%d/%d/%d]  h_line/h_width/v_line/v_width [%d/%d/%d/%d] top/bot/lr_crop [%d/%d/%d] -> sampling_size[%d]\n",\
...
#define BOTTOM_WIN_X_OFFSET (0)
	...
	do {
		...
		{
			APR_PRINT("In Out info : in_offset h/v[ %d / %d ] in_size h/v[ %d / %d ]  out_size[ %d / %d ]\n",\
					ov_info->input.h_offset, ov_info->input.v_offset, ov_info->input.h_size, ov_info->input.v_size,\
					ov_info->h, ov_info->v);
			APR_PRINT("pFrameBase : 0x%08X, line_num[%d] x_pos[%d] y_pos[%d] width[%d] v_sampling_size[%d]\n",\
	...
	do {
		switch(pos)
		{
			...
			case APR_GET_END :
				...
				if(pCfg->cap.enb == LX_APR_CAP_TIME) APR_PRINT("%s :	%d usec  elapsed\n", fn_str, time_diff);
				break;
		}
	} while(0);
...
static int APR_VotingFormat(APR_METHOD_T method_idx, LX_APR_FORMAT fmt, BOOLEAN vote_enable)
{
	...
	do {
		...
		if(pCfg->enable == LX_APR_CMD_LOG) {
			APR_PRINT("Voting -  method [ %d ] \n", method_idx + 1);
			for(i=0;i<vld_thr;i++) {
				APR_PRINT("%d  ", pCfg->voting_fmt[method_idx][i]);
			}
			if(valid_flag) APR_PRINT("   final => %d\n", cur_fmt);
			else APR_PRINT("\n");
		}
...
static int APR_NegotiateFormat(BOOLEAN nego_enble)
{
	...
	do {
		...
		if((pCfg->out_type == LX_PE_OUT_2D) || (pCfg->out_type == LX_PE_OUT_2D_3D) || (pCfg->out_type == LX_PE_OUT_PIP))
		{
			...
			if(0)//pData->tpd < 30)
			{
				if(pCfg->cap.enb == LX_APR_CAP_LOG){printk("case 0 -  tpd[%d]  no nego\n", pData->tpd);}
				pCfg->apr_fmt = pCfg->fmt_after_vote[APR_METHOD_3];
				break;
			}
			...
			if((pCfg->fmt_after_vote[APR_METHOD_3] == LX_APR_FMT_3D_SS) || (pCfg->fmt_after_vote[APR_METHOD_3] == LX_APR_FMT_3D_TB))
			{
				...
				if(pCfg->cap.enb == LX_APR_CAP_LOG){printk("ss[%d] tb[%d] unk[%d] tpd[%d]\n", ss_count, tb_count, unk_count, pData->tpd);}
						...
						((pCfg->fmt_after_vote[APR_METHOD_3] == LX_APR_FMT_3D_TB) && (tb_count <= 2) && (ss_count <= tb_count))  )
				{
					...
					{
						pCfg->apr_fmt = LX_APR_FMT_2D_2D;
						if(pCfg->cap.enb == LX_APR_CAP_LOG){printk("case 1 \n");}
						break;
					}
				}
				...
				{
					pCfg->apr_fmt = LX_APR_FMT_2D_2D;
					if(pCfg->cap.enb == LX_APR_CAP_LOG){printk("case 2 -  tpd[%d]\n", pData->tpd);}
					break;
				}
				if(0)//unk_count == pThr->vld_thr)
				{
					pCfg->apr_fmt = LX_APR_FMT_UNKNOWN;
					if(pCfg->cap.enb == LX_APR_CAP_LOG){printk("case 3 \n");}
					break;
				}
			}
			pCfg->apr_fmt = pCfg->fmt_after_vote[APR_METHOD_3];
	...
	do {
		switch(pCfg->out_type)
		{
			...
			case LX_PE_OUT_PIP :
				...
				#else
				APR_MSG("APR : === WIN 0  [G] LR HISTOGRAM\n     ");
				for(itr=0;itr<rb_bin_apr;itr++) APR_MSG("[%d]", pData->hist_l_g[itr]);APR_PRINT("\n");
				APR_MSG("APR : === WIN 0  [B] LR HISTOGRAM\n     ");
				for(itr=0;itr<rb_bin_apr;itr++) APR_MSG("[%d]", pData->hist_l_b[itr]);APR_PRINT("\n");
				APR_MSG("APR : === WIN 0  [R] LR HISTOGRAM\n     ");
				for(itr=0;itr<rb_bin_apr;itr++) APR_MSG("[%d]", pData->hist_l_r[itr]);APR_PRINT("\n");
				APR_MSG("APR : === WIN 1  [G] LR HISTOGRAM\n     ");
				for(itr=0;itr<rb_bin_apr;itr++) APR_MSG("[%d]", pData->hist_r_g[itr]);APR_PRINT("\n");
				APR_MSG("APR : === WIN 1  [B] LR HISTOGRAM\n     ");
				for(itr=0;itr<rb_bin_apr;itr++) APR_MSG("[%d]", pData->hist_r_b[itr]);APR_PRINT("\n");
				APR_MSG("APR : === WIN 1  [R] LR HISTOGRAM\n     ");
				for(itr=0;itr<rb_bin_apr;itr++) APR_MSG("[%d]", pData->hist_r_r[itr]);APR_PRINT("\n");
				APR_MSG("APR : === WIN 0  [G] TB HISTOGRAM\n     ");
				for(itr=0;itr<rb_bin_apr;itr++) APR_MSG("[%d]", pData->hist_t_g[itr]);APR_PRINT("\n");
				APR_MSG("APR : === WIN 0  [B] TB HISTOGRAM\n     ");
				for(itr=0;itr<rb_bin_apr;itr++) APR_MSG("[%d]", pData->hist_t_b[itr]);APR_PRINT("\n");
				APR_MSG("APR : === WIN 0  [R] TB HISTOGRAM\n     ");
				for(itr=0;itr<rb_bin_apr;itr++) APR_MSG("[%d]", pData->hist_t_r[itr]);APR_PRINT("\n");
				APR_MSG("APR : === WIN 1  [G] TB HISTOGRAM\n     ");
				for(itr=0;itr<rb_bin_apr;itr++) APR_MSG("[%d]", pData->hist_b_g[itr]);APR_PRINT("\n");
				APR_MSG("APR : === WIN 1  [B] TB HISTOGRAM\n     ");
				for(itr=0;itr<rb_bin_apr;itr++) APR_MSG("[%d]", pData->hist_b_b[itr]);APR_PRINT("\n");
				APR_MSG("APR : === WIN 1  [R] TB HISTOGRAM\n     ");
				for(itr=0;itr<rb_bin_apr;itr++) APR_MSG("[%d]", pData->hist_b_r[itr]);APR_PRINT("\n");
				#endif
				break;
			case LX_PE_OUT_3D_2D :
			case LX_PE_OUT_TB :
			...
			case LX_PE_OUT_DUAL_FULL :
				...
				#else
				APR_MSG("APR : === WIN 0   DCE HISTOGRAM\n     ");
				for(itr=0;itr<rb_bin_apr;itr++) APR_MSG("[%d]", pData->dce_l[itr]);
				for(itr=0;itr<rb_bin_apr;itr++) sum += pData->dce_l[itr];
				APR_MSG("\nAPR : Total : %d  pixels\n", sum);
				sum = 0;
				APR_MSG("APR : === WIN 1   DCE HISTOGRAM\n     ");
				for(itr=0;itr<rb_bin_apr;itr++) APR_MSG("[%d]", pData->dce_r[itr]);
				for(itr=0;itr<rb_bin_apr;itr++) sum += pData->dce_r[itr];
				APR_MSG("\nAPR : Total : %d  pixels\n", sum);
				#endif
				break;
			default :
...
static int APR_DumpLrcHistData(void)
{
	...
	do {
		...
		APR_MSG("\nLRC : == value	/  threshold  ====	[outtype %d  %dx%d] \n", pCfg->out_type, pCfg->out_win_x, pCfg->out_win_y);
		APR_MSG("tpd[%d] / %d	 ,	 motion_l[%d] / 2d-%d 3d-%d , motion_r[%d]\n", pData->tpd, pThr->tpd_thr, pData->motion_l, pThr->mtn_thr_2d, pThr->mtn_thr_3d, pData->motion_r);
		APR_MSG("cb[%d] cc[%d] ll[%d] -> max_pd[%d] / %d\n", pData->cb, pData->cc, pData->ll, max_pd, pThr->ptn_thr);
		APR_MSG("ssr[%d] tbr[%d] / 2D->3D[%d] , 3D->2D[%d]\n", pData->ssr, pData->tbr, pThr->fmt_th2, pThr->fmt_th1);
		APR_MSG("ssh[%d] ssv[%d] tbh[%d] tbv[%d] \n", pData->ssh, pData->ssv, pData->tbh, pData->tbv);
		APR_MSG("LRCR win_pos[%d] (0:LRTB_FULL,1:LRTB_HAF1,2:LRTB_HAF2) \n", pStm->win_pos);

#ifdef APR_USE_NEW_ALGO
		APR_MSG("thr_adj[%d] lr_diff[%d] tb_diff[%d] / fmt_th3[%d] ,  lr-tb[%d] / fmt_th4[%d] , / fmt_th6[%d] fmt_th7[%d] fmt_th8[%d] fmt_th9[%d] fmt_th10[%d]\n", \
		thr_adj, pData->hist_lrc.lr_diff, pData->hist_lrc.tb_diff, tmp_thr3, (int)abs(pData->hist_lrc.lr_diff - pData->hist_lrc.tb_diff), \
		tmp_thr4, tmp_thr6, tmp_thr7, pThr->fmt_th8, pThr->fmt_th9, pThr->fmt_th10);
#else
		APR_MSG("lr_diff[%d] tb_diff[%d] / %d ,  lr-tb[%d] / %d , / th6[%d] th7[%d] th8[%d] th9[%d] th10[%d]\n", \
		pData->hist_lrc.lr_diff, pData->hist_lrc.tb_diff, pThr->fmt_th3/win_half_size, (int)abs(pData->hist_lrc.lr_diff - pData->hist_lrc.tb_diff), \
		pThr->fmt_th4/win_half_size, pThr->fmt_th6/win_half_size, pThr->fmt_th7/win_half_size, pThr->fmt_th8, pThr->fmt_th9, pThr->fmt_th10);
#endif
		APR_MSG("vld_thr[%d] \n", pThr->vld_thr);
		APR_MSG("motion[%d/%d], spatial[%d/%d], apl[%d/%d], noise[%d/%d],  dual_screen_fmt[%d](0:2D,8:SS,9:TB)\n", pData->motion_l, pData->motion_r, pData->spatial_l, pData->spatial_r, pData->apl_l, pData->apl_r, pData->nt_l, pData->nt_r, pData->hw_fmt);
#ifdef APR_USE_NEW_ALGO	// Added by won.hur 2011.12.13
		APR_MSG("APR : ==>> Format tmp[%d] final[%d]		(0:2D,1:CB,2:LL,3:CC,4:SS,5:TB,6:UNKNOWN)  Test pattern(%d)\n", pCfg->tmp_fmt, pCfg->apr_fmt, pCfg->tpd_flag);
		APR_MSG("\nAPR_MESSAGE_A= %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d\n",\
		...
		APR_MSG("APR_MESSAGE_B= %d %d %d %d %d %d %d %d %d %d %d\n",\
...
#else
		APR_MSG("APR : ==>> Format tmp[%d] final[%d]		(0:2D,1:CB,2:LL,3:CC,4:SS,5:TB)  Test pattern(%d)\n", pCfg->tmp_fmt, pCfg->apr_fmt, pCfg->tpd_flag);
#endif
		APR_MSG("\n\n");
	} while(0);
	return ret;
...
static int APR_DumpSrcHistData(void)
{
	int ret = RET_OK;
	APR_CFG_T *pCfg = &apr_info.cfg;
	APR_DTA_T *pData = &apr_info.dta;
	...
	do {
		if(pCfg->hst_enb == LX_APR_HST_DUMP) {
			APR_PRINT("\nHistogram - SRC\n");
			APR_PRINT("tpd[%d] ssr[%d] tbr[%d]\n", pData->tpd, pData->ssr, pData->tbr);
			APR_PRINT("    lr_diff		tb_diff\n");
			APR_PRINT("y   %08d		%08d \n", pData->hist_src.lr_diff[0],pData->hist_src.tb_diff[0]);
			APR_PRINT("cb  %08d		%08d \n", pData->hist_src.lr_diff[1],pData->hist_src.tb_diff[1]);
			APR_PRINT("cr  %08d		%08d \n", pData->hist_src.lr_diff[2],pData->hist_src.tb_diff[2]);
			APR_PRINT("g   %08d		%08d \n", pData->hist_src.lr_diff[3],pData->hist_src.tb_diff[3]);
			APR_PRINT("b   %08d		%08d \n", pData->hist_src.lr_diff[4],pData->hist_src.tb_diff[4]);
			APR_PRINT("r   %08d		%08d \n", pData->hist_src.lr_diff[5],pData->hist_src.tb_diff[5]);
			APR_PRINT("h   %08d		%08d \n", pData->hist_src.lr_diff[6],pData->hist_src.tb_diff[6]);
			APR_PRINT("s   %08d		%08d \n", pData->hist_src.lr_diff[7],pData->hist_src.tb_diff[7]);
			APR_PRINT("v   %08d		%08d \n", pData->hist_src.lr_diff[8],pData->hist_src.tb_diff[8]);
			APR_PRINT("Histogram - LRC\n");
			APR_PRINT("gbr %08d		%08d \n", pData->hist_lrc.lr_diff,pData->hist_lrc.tb_diff);
		}
	} while(0);
...
static int APR_DumpData4Simulation(void)
{
	...
	if(pCfg->hidden_cmd == LX_APR_HCMD_DUMP_SCRIPT_DATA) {
		APR_PRINT("MESSAGE_B= %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d",\
		...
		APR_PRINT("\n");
#else
		APR_PRINT(" %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d \n",\
	...
	if(pCfg->hidden_cmd == LX_APR_HCMD_DUMP_EXCEL_DATA) {
		APR_PRINT("MESSAGE_A= %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d\n",\
				...
				pData->fd3.blk_stat.mean[0],pData->fd3.blk_stat.mean[1],pData->fd3.blk_stat.mean[2],pData->fd3.blk_stat.mean[3],\
				pData->fd3.blk_stat.mean[4],pData->fd3.blk_stat.mean[5],pData->fd3.blk_stat.mean[6],pData->fd3.blk_stat.mean[7]);
		APR_PRINT("MESSAGE_B= %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d\n",\
				pThr->fmt_th5,pThr->vld_thr,pData->motion_l,pData->motion_r,pData->spatial_l,pData->spatial_r,pData->apl_l,pData->apl_r,pData->nt_l,\
				pData->nt_r,pData->hw_fmt,\
		...
		APR_PRINT("MESSAGE_C= %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d \n\n",\
...
static int APR_DumpCurrentConfig(void)
{
	...
	do {
		APR_PRINT("\n");
		APR_PRINT("src_type [%d] (0:DTV,1:ATV,2:CVBS,3:COMP,4:PC,5:HDMI)\n", pCfg->src_type);
		...
		APR_PRINT("Image class [%d] (0:TP,1:2D_HARD,2:2D_WEAK_SSm,3:2D_WEAK_TB,4:SS_HARD,5:SS_WEAK,6:TB_HARD,7:TB_WEAK)\n", pCfg->img_class);
	...
	do {
		...
		if((pCfg->out_type == LX_PE_OUT_2D) || (pCfg->out_type == LX_PE_OUT_2D_3D))
		{
			...
			if(pData->tpd > pThr->tpd_thr)
			{
				...
				if(pData->motion_l < pThr->mtn_thr_2d || pData->motion_l == 255)// ¿òÁ÷?Ó?Ì ¾ø´Â Å×½ºÆ® ÆÐÅà ¿µ»ó || PC È­¸é?Î °æ¿ì (TNR off 444 ¿µ»ó¿¡¼­. µ¿¿µ»ó?º Á¦¿Ü)  -> (dqms : 1110-08276, 1110-09207, 1110-09326, 1110-09998)
				{
					pCfg->tpd_flag = TRUE;
#ifdef APR_USE_NEW_ALGO		// Added by won.hur 2011.12.13
					APR_MSG(" MSG from W.H[0-2] : APR is going UNKNOWN status!\n");
					pCfg->tmp_fmt = LX_APR_FMT_UNKNOWN;
#else
					pCfg->tmp_fmt = LX_APR_FMT_2D_2D;
#endif
					goto LABEL_DECIDE_FMT;
				}
				else // ¿òÁ÷?Ó?Ì ?Ö´Â ÅÂ½ºÆ® ÆÐÅÏ ¿µ»ó  (BD 3D Å×½ºÆ® Å¸?ÌÆ² - ±×·¡ÇÈ ¿µ»ó )
				{
					// go to next step
				}
#endif
...
#else
				if(max_pd == pData->cb)
					pCfg->tmp_fmt = LX_APR_FMT_3D_CB;
				else if(max_pd == pData->cc)
					pCfg->tmp_fmt = LX_APR_FMT_3D_CC;
				else if(max_pd == pData->ll)
					pCfg->tmp_fmt = LX_APR_FMT_3D_LL;
#endif
				goto LABEL_DECIDE_FMT;
			}
			else
					...
					pStm->win_pos == APR_WIN_LRTB_MAX)
			{
				lr_dif = pData->hist_lrc.lr_diff;
				tb_dif = pData->hist_lrc.tb_diff;
				thr3   = pThr->fmt_th3 / APR_WIN_RATIO;
				thr4   = pThr->fmt_th4 / APR_WIN_RATIO;
				thr6   = pThr->fmt_th6 / APR_WIN_RATIO;
				thr7   = pThr->fmt_th7 / APR_WIN_RATIO;
				thr_const0 = 2000 / APR_WIN_RATIO;
				thr_const1 = 5000 / APR_WIN_RATIO;
				thr_const2 = 200 / APR_WIN_RATIO;
				thr_const3 = 10;
			}
#endif
...
#ifdef APR_USE_NEW_ALGO 	// Added by won.hur 2011.12.19
...
//		if(pCfg->ldf_fmt == prev_fmt) break;
		...
		for(i=0;i<32;i++)
		{
			...
			strcat(str, &ch);
			if(pData->d2d.state_num[i+1] == 0) break;
			sprintf(&ch, ">");
			strcat(str, &ch);
		}
...
//		if(pCfg->apr_fmt == prev_fmt) break;
		...
		strcat(str, ((pCfg->src_type >= LX_PE_SRC_ATV) && (pCfg->src_type <= LX_PE_SRC_PC))?"ANALOG":"DIGITAL");
		ret = APR_OSD_DrawString(	str, 0, line_idx++);
		memset(str, 0, STRING_MAX_SIZE);
...
static int APR_DisplayResult(void)
{
	...
	if(1)
	{
		...
		if(++count%5== 0 && pCfg->out_type == LX_PE_OUT_2D)
		{
			//APR_DumpHistogram();
			APR_DumpLrcHistData();
			APR_DumpSrcHistData();
