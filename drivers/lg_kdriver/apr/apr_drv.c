...
int APR_Init(void)
{
	...
	OS_DEBUG_EnableModuleByIndex ( g_apr_debug_fd, 0, DBG_COLOR_YELLOW );
	OS_DEBUG_EnableModuleByIndex ( g_apr_debug_fd, 1, DBG_COLOR_GRAY );
	...
	{
		APR_PRINT("[%s] platform driver register failed\n",APR_MODULE);
	...
	else
	{
		if(platform_device_register(&apr_device))
		{
			...
			APR_PRINT("[%s] platform device register failed\n",APR_MODULE);
		}
		else
		{
			APR_PRINT("[%s] platform register done\n", APR_MODULE);
		}
	...
	if ( err < 0 )
	{
		DBG_PRINT_ERROR("can't register APR device\n" );
		return -EIO;
	}
	...
	for ( i=0; i<APR_MAX_DEVICE; i++ )
	{
		...
		if (err)
		{
			DBG_PRINT_ERROR("error (%d) while adding APR device (%d.%d)\n", err, MAJOR(dev), MINOR(dev) );
			return -EIO;
		}
	...
	APR_PRINT("APR device initialized\n");
