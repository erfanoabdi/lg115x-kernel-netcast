...
int APR_FD3_Detect3D(void)
{
	...
	APR_CFG_T *pCfg  = &apr_info.cfg;
	APR_DTA_T *pData = &apr_info.dta;
//	LX_APR_FORMAT m0_fmt = pCfg->cur_fmt;
//	LX_APR_FORMAT m1_fmt = pCfg->tmp_fmt;
	LX_APR_FORMAT m2_fmt = LX_APR_FMT_2D_2D;

	FD3DBG("\nFD3 cfg : sampling[%d](0:all,1:2line) , scan[%d](0:P,1:I) , color[%d](0:420,1:422:2:444)\n",\
	...
	do {
		if ( pData->tpd < 210 )
	   	{
			APR_HAL_SetFd3SubtConfig(pCfg, 3);
		}
		...
		else
		{
			...
			{
				APR_HAL_SetFd3SubtConfig(pCfg, pData->fd3.bMask);
			}
	...
	FD3DBG("FD3 result : %d (0:2D,4:SS,5:TB)\n", m2_fmt);
