...
{
	AUD_KDRV_PRINT("%s.%d: ADEC_MODULE_MAN_ARM\n", __FUNCTION__, __LINE__);
	AUD_KDRV_PRINT("%s.%d: cmd = %d\n", __FUNCTION__, __LINE__, _cmd);

	if (_sender != NULL)
	{
		AUD_KDRV_PRINT("%s.%d: SENDER INFO] module=%d, core=%d, requestResponse=%d, id=%d, paramLen=%d\n", __FUNCTION__, __LINE__,
...
SINT32 AUDIO_IMC_Init(void)
{
	...
	_gIPCList[IPC_WRITE_TO_DSP0].pWriteOffset		= (void *)(&g_pRealRegAud->aud_ipc_a2d_wptr),
	_gIPCList[IPC_WRITE_TO_DSP0].pReadOffset		= (void *)&g_pRealRegAud->aud_ipc_a2d_rptr,
	...
	_gIPCList[IPC_READ_FROM_DSP0].pWriteOffset		= (void *)&g_pRealRegAud->aud_ipc_d2a_wptr,
	_gIPCList[IPC_READ_FROM_DSP0].pReadOffset		= (void *)&g_pRealRegAud->aud_ipc_d2a_rptr,
	...
	_gIPCList[IPC_WRITE_TO_DSP1].pWriteOffset		= (void *)&g_pRealRegAud->aud_ipc_a2p_wptr,
	_gIPCList[IPC_WRITE_TO_DSP1].pReadOffset		= (void *)&g_pRealRegAud->aud_ipc_a2p_rptr,
	...
	_gIPCList[IPC_READ_FROM_DSP1].pWriteOffset		= (void *)&g_pRealRegAud->aud_ipc_p2a_wptr,
	_gIPCList[IPC_READ_FROM_DSP1].pReadOffset		= (void *)&g_pRealRegAud->aud_ipc_p2a_rptr,
	...
	if (result != 0)
	{
		AUD_KDRV_ERROR("%s.%d: IMC_Init() Error!!!\n", __FUNCTION__, __LINE__);
		goto IMC_INIT_ERROR;
	}
	...
	for (ipcIndex = 0; ipcIndex < IPC_LIST_MAX; ipcIndex++)
	{
		...
		if (result != 0)
		{
			AUD_KDRV_ERROR("%s.%d: IPC_Init() Error - index=%u !!!\n", __FUNCTION__, __LINE__, ipcIndex);
			goto IMC_INIT_ERROR;
		}
	...
	if (result != 0)
	{
		AUD_KDRV_ERROR("%s.%d: IMC_SetTargetWriter() Error!!!\n", __FUNCTION__, __LINE__);
		goto IMC_INIT_ERROR;
	}
	result = IMC_SetTargetReader(_gpImc, ADEC_CORE_DSP0, _gIPCList[IPC_READ_FROM_DSP0].pIpc);
	if (result != 0)
	{
		AUD_KDRV_ERROR("%s.%d: IMC_SetTargetReader() Error!!!\n", __FUNCTION__, __LINE__);
		goto IMC_INIT_ERROR;
	}
	...
	if (result != 0)
	{
		AUD_KDRV_ERROR("%s.%d: IMC_RegisterCmdProcessor() Error!!!\n", __FUNCTION__, __LINE__);
		goto IMC_INIT_ERROR;
	}
	...
	if (result != 0)
	{
		AUD_KDRV_ERROR("%s.%d: IPC_SetInterrupt() Error!!!\n", __FUNCTION__, __LINE__);
		goto IMC_INIT_ERROR;
	}
...
SINT32 AUDIO_IMC_SendCmd(int cmd, ADEC_MODULE_ID targetModule)
{
	...
	int responseID;
	LX_AUD_ALLOC_FREE_INFO_T * pMod;

	//Check a ADEC module infomation.
	pMod = AUDIO_GetAllocModInfo(targetModule);
	if(pMod == NULL)
	{
	...
	if( ((targetModule >= ADEC_MODULE_BYPASS_00) && (targetModule <= ADEC_MODULE_BYPASS_11))	\
	  &&(cmd == ADEC_CMD_INIT) )
	{
		AUD_KDRV_IPC_DEBUG("%s.%d : ADEC_CMD_INIT for BYPASS module(targetModule = %d)\n", __FUNCTION__, __LINE__, targetModule);
		return RET_OK;
	}
	...
	if(imcCmdParam == NULL)
	{
		AUD_KDRV_ERROR("%s.%d: kmalloc Err!!!\n", __FUNCTION__, __LINE__);
	...
	if (result != 0)
	{
		AUD_KDRV_ERROR("%s.%d: IMC_SendCommand(cmd=0x%08X, targetModule=%d) Error(result=%d)!!!\n", __FUNCTION__, __LINE__, cmd, targetModule, result);
		kfree(imcCmdParam);
	...
	AUD_KDRV_IPC_DEBUG("%s.%d: IMC_SendCommand(cmd=0x%08X, targetModule=%d) Error(result=%d)!!!\n", __FUNCTION__, __LINE__, cmd, targetModule, result);
	return RET_OK;
}
...
SINT32 AUDIO_IPC_DBG_Init(void)
{
	...
	_gIpcDbgList[IPC_DBG_DSP0].pWriteOffset		= (void *)(&g_pRealRegAud->aud_ipc_dbgd_wptr);
	_gIpcDbgList[IPC_DBG_DSP0].pReadOffset		= (void *)&g_pRealRegAud->aud_ipc_dbgd_rptr;
	...
	_gIpcDbgList[IPC_DBG_DSP1].pWriteOffset		= (void *)&g_pRealRegAud->aud_ipc_dbgp_wptr;
	_gIpcDbgList[IPC_DBG_DSP1].pReadOffset		= (void *)&g_pRealRegAud->aud_ipc_dbgp_rptr;
	...
	for (ipcDbgIndex = 0; ipcDbgIndex < IPC_DBG_LIST_MAX; ipcDbgIndex++)
	{
		...
		if (result != 0)
		{
			AUD_KDRV_ERROR("%s.%d: IPC_Init() Error - index=%u !!!\n", __FUNCTION__, __LINE__, ipcDbgIndex);
			goto IPC_INIT_ERROR;
		}
		// clean IPC read/write pointer
