	...
	ImcActionParameter* _imcActionParam)
{
	...
	NOTI_DBG("Receive Regist_Notification(%s) : eventID(%xd), actionID(%x) \n",
		IMC_PLATFORM_NAME, 
		_eventID, 
		_actionID);
	...
	ADEC_CORE_TYPE _core)
{
	...
	if ( gMutexInitialized == 1)
	{
		ADEC_MUTEX_DESTROY(&gImcMutex);
	}
	...
	unsigned int*		 _responseID)
{
	...
	ADEC_ASSERT(
		...
		return ADEC_ERR_RANGE);
	ADEC_ASSERT(
		targetCore < ADEC_CORE_MAX && myCore > ADEC_CORE_NOT_DEF,
		return ADEC_ERR_RANGE);
	...
	ImcActionParameter* _imcActionParam)
{
	...
	*_actionID = IMC_ACTION_ID(imc->imcActionIndex++, imc->core);
	...
	NOTI_DBG("Send Regist_Notification(%s) : eventID(%x), actionID(%x) \n",
		IMC_PLATFORM_NAME,  
		eventID, 
		*_actionID);
	...
	ImcEventParameter*	_imcEventParameter)
{
	...
	for(i = 0 ; i < eventInfo->actionCount ; i++)
	{
		...
		if(actionInfo->actionParam.actionType == IMC_ACTION_EXCUTE_CMD)
		{
			...
			IMC_SendCommand(_imc, &imcCommandParam, &responseID);
		}
	...
	void* _ipcReader)
{
	...
	while(1)
	{
		...
		else if (actionFlag & IMC_CMD_REGISTER_FLAG)
		{
			...
			NOTI_DBG("Receive Regist_Notification(%s) : eventID(%x), actionID(%x) \n", 
				IMC_PLATFORM_NAME,  
				eventID, 
				actionID);	
		}
