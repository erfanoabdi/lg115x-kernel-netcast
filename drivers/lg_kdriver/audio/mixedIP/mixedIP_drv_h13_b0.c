...
int H13B0_MIXED_IP_ADEC_SetClockSource( LX_AUD_SAMPLING_FREQ_T ui32SrcFreq, LX_AUD_CLOCK_SRC_T ui8AdecClockSrc, LX_AUD_MASTER_SPDIF_OUTPUT_T spdifOutput )
{
	...
	if(gAdecSrcFreq_H13B0 == ui32SrcFreq && gAdecClockSource_H13B0 == ui8AdecClockSrc && gAdecSpdifOutput_H13B0 == spdifOutput)
	{
	...
	#ifdef USE_DTO_AAD
	spdifDTORate = MIXED_IP_AUDIO_GetDTORate(ui8AdecClockSrc, LX_AUD_SAMPLING_FREQ_48_KHZ, AUD_DTO_AAD);
	(void)H13B0_MIXED_IP_ADEC_SetSPDIFSamplingFreq(LX_AUD_SAMPLING_FREQ_48_KHZ, spdifDTORate, AUD_DTO_AAD);
	#else
      ...
      ||(ui8AdecClockSrc == LX_AUD_CLOCK_ADC) ) 	//ADC to SPK, SCART AAD bypass
	{
		...
		if( ui8AdecClockSrc == LX_AUD_CLOCK_DTV )	//DTV to SPK(default)
		{
			...
			AUD_RdFL(aad_bypass);
			AUD_Wr01(aad_bypass, aad2scartbypassctrl, 0x0);
			AUD_WrFL(aad_bypass);
...
#ifdef USE_DTO_AAD
			CTOP_CTRL_H13B0_Wr01(ctr81, sel_aud_fs21clk_src, 0x0);	//SPDIF : AAD(0), A-DTO(2), D-DTO(4)
#else
			CTOP_CTRL_H13B0_Wr01(ctr81, sel_aud_fs21clk_src, 0x2);	//SPDIF : A-DTO(2), D-DTO(4)
#endif
   		...
   		else	//( ui8AdecClockSrc == LX_AUD_CLOCK_ADC ) //ADC to SPK, SCART AAD bypass
		{
			...
			AUD_RdFL(aad_bypass);
			AUD_Wr01(aad_bypass, aad2scartbypassctrl, 0x1);
			AUD_WrFL(aad_bypass);
...
int H13B0_MIXED_IP_ADEC_SetPcmClockRate ( LX_AUD_SAMPLING_FREQ_T ui32SrcFreq, UINT32 ui32PcmClock )
{
	...
	if(	gAdecSrcFreq_H13B0 == ui32SrcFreq && gAdecPcmClock_H13B0 == ui32PcmClock)
	{
...
int H13B0_MIXED_IP_ADEC_SetSPDIFSamplingFreq ( LX_AUD_SAMPLING_FREQ_T ui32SamplingFreq, LX_AUD_SPDIF_DTO_RATE_T stSpdifDtoRate, AUD_DTO_T dto )
{
	...
	//Check a previous status.
	if(	gAdecSpdifFreq_H13B0[dto] == ui32SamplingFreq && gAdecSpdifDtoRate_H13B0[dto] == stSpdifDtoRate)
	{
	...
	{
		ACE_REG_H13A0_Wr01(afe_aad_8, aad_dco_fcw_2, ((stSpdifDtoRate>>16) & 0x1f) );
		ACE_REG_H13A0_Wr01(afe_aad_9, aad_dco_fcw_1, ((stSpdifDtoRate>>8) & 0xff));
		ACE_REG_H13A0_Wr01(afe_aad_10, aad_dco_fcw_0, stSpdifDtoRate & 0xff);
	}
#endif
	else
	{
...
//	H13B0_ANALOG_WriteRegister(0x1D, 0x02, 0x40);
	...
	H13B0_ANALOG_WriteRegister(0x1D, 0x14, 0x05);	//DAC0 Mute on, SCART
