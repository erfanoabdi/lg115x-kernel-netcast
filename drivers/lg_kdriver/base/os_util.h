...
#include "l8/linux_platform.h"
#include "l8/linux_irqs.h"
 ...
 * receive event.
*/
#define OS_RecvEvent(pEvnt,ev,rEv,option,timeout)	OS_RecvEvent_Tag( pEvnt, ev, rEv, option, timeout, __FL__, __F__, __L__)
/**
 * @def OS_SendEvent
 * send event.
*/
...
*/
#define	OS_MsecSleep(msec)						OS_MsecSleep_Tag(msec, __FL__, __F__, __L__)
#define OS_NsecDelay(nsec)						OS_NsecDelay_Tag(nsec, __FL__, __F__, __L__)
#define OS_UsecDelay(usec)						OS_UsecDelay_Tag(usec, __FL__, __F__, __L__)
#define	OS_GetMsecTicks()						OS_GetMsecTicks_Tag()
#define	OS_GetUsecTicks()						OS_GetUsecTicks_Tag()
#define OS_GetCurrentTicks(pSec,pMsec,pUsec)	OS_GetCurrentTicks_Tag(pSec,pMsec,pUsec)

#define	OS_GetHighResTicks()					OS_GetHighResTicks_Tag()
#define OS_GetHighResTimeElapsed(oldTick)		OS_GetElapsedHighResTime_Tag(oldTick)
...
/**
 ...
 @param pTimer [IN] pointer to OS_TIMER_T
*/
#define OS_InitTimer(pTimer)										OS_InitTimer_Tag(pTimer, __FL__, __F__, __L__)

#define OS_StartTimer(pTimer,timer_callback,option,timeout,data)	OS_StartTimer_Tag(pTimer,timer_callback,option,timeout,data, __FL__, __F__, __L__)

#define OS_StopTimer(pTimer)										OS_StopTimer_Tag(pTimer, __FL__, __F__, __L__)
...
*/
#define OS_OpenCacheMap(pMap,phys_addr,length)	OS_OpenCacheMap_Tag(pMap,phys_addr,length)
#define OS_CloseCacheMap(pMap)					OS_CloseCacheMap_Tag(pMap)
#define OS_InvalCacheMap(pMap,off,length)		OS_InvalCacheMap_Tag(pMap,off,length)
#define OS_FlushCacheMap(pMap,off,length)		OS_FlushCacheMap_Tag(pMap,off,length)
#define OS_CleanCacheMap(pMap,off,length)		OS_CleanCacheMap_Tag(pMap,off,length)
 ...
 *	@see LX_CHIP_REV
 */
extern UINT32	lx_chip_rev		( void );
extern UINT32	lx_ace_rev		( void );

/** return platform information for customizing the kdriver
 *
 * @see LX_CHIP_PLATFORM_GP, LX_CHIP_PLATFORM_COSMO, LX_CHIP_PLATFROM_FPGA
 */
extern UINT32	lx_chip_plt		( void );

extern void		OS_CORE_Init			( void );
extern void		OS_CORE_InitDebug		( void );
extern void		OS_CORE_Cleanup 		( void );
...
----------------------------------------------------------------------------------------*/
extern	void	OS_FAILURE_Tag			( const char* szFile, const char* szFunc, const int nLine);

extern void		OS_InitMutex_Tag		( OS_SEM_T* pSem, UINT32 attr );
extern int		OS_LockMutex_Tag		( OS_SEM_T* pSem, UINT32 timeout,
											const char* szFile, const char* szFunc, const int nLine );
extern void		OS_UnlockMutex_Tag 		( OS_SEM_T* pSem, const char* szFile, const char* szFunc, const int nLine );

extern void		OS_InitEvent_Tag		( OS_EVENT_T* pEvnt );
extern int      OS_RecvEvent_Tag		( OS_EVENT_T* pEvnt, UINT32 ev, UINT32* rEv, UINT32 option, UINT32 timeout,
											const char* szFile, const char* szFunc, const int nLine );
extern void     OS_SendEvent_Tag		( OS_EVENT_T* pEvnt, UINT32 ev,
											const char* szFile, const char* szFunc, const int nLine );
extern void		OS_ClearEvent_Tag		( OS_EVENT_T* pEvnt,
											const char* szFile, const char* szFunc, const int nLine );

extern	UINT64	OS_GetMsecTicks_Tag 	( void );
extern	UINT64	OS_GetUsecTicks_Tag 	( void );
extern	void	OS_GetCurrentTicks_Tag 	( UINT32* pSec, UINT32* pMsec, UINT32* pUsec );

extern	UINT32	OS_GetHighResTicks_Tag	( void );
extern	UINT64	OS_GetElapsedHighResTime_Tag( UINT32 oldTick );

extern int		OS_MsecSleep_Tag 		( UINT32 msec,
											const char* szFile, const char* szFunc, const int nLine );
extern void		OS_NsecDelay_Tag	 	( UINT32 nsec,
											const char* szFile, const char* szFunc, const int nLine );
extern void		OS_UsecDelay_Tag 		( UINT32 usec,
											const char* szFile, const char* szFunc, const int nLine );

extern void		OS_InitTimer_Tag		( OS_TIMER_T* pTimer,
											const char* szFile, const char* szFunc, const int nLine  );
extern void		OS_StartTimer_Tag		( OS_TIMER_T* pTimer, OS_TIMER_CALLBACK_T timer_callback, UINT32 option, UINT32 timeout, UINT32 data,
											const char* szFile, const char* szFunc, const int nLine  );
extern void		OS_StopTimer_Tag		( OS_TIMER_T* pTimer,
											const char* szFile, const char* szFunc, const int nLine  );

extern int		OS_OpenFile_Tag			( OS_FILE_T* pFile, char* fname, int flags, mode_t mode,
											const char* szFile, const char* szFunc, const int nLine );
extern int		OS_SeekFile_Tag			( OS_FILE_T* pFile, off_t offset, int whence,
											const char* szFile, const char* szFunc, const int nLine );
extern int		OS_SizeOfFile_Tag		( OS_FILE_T* pFile,
											const char* szFile, const char* szFunc, const int nLine );
extern int		OS_WriteFile_Tag		( OS_FILE_T* pFile, char* buffer, int count,
											const char* szFile, const char* szFunc, const int nLine );
extern int		OS_ReadFile_Tag			( OS_FILE_T* pFile, char* buffer, int count,
											const char* szFile, const char* szFunc, const int nLine );
extern int		OS_CloseFile_Tag		( OS_FILE_T* pFile,
											const char* szFile, const char* szFunc, const int nLine );

extern void*	OS_KMalloc_Tag			( size_t size, const char* szFile, const char* szFunc, const int nLine );
extern void		OS_KFree_Tag			( void* ptr, const char* szFile, const char* szFunc, const int nLine );

extern int		OS_InitRegionEx_Tag		( OS_RGN_T* pRgn, OS_RGN_MGR_TYPE_T mgr_type, const void* phys_addr, const int size,  const int block_size, 
											const char* szFile, const char* szFunc, const int nLine );
extern int		OS_CleanupRegion_Tag	( OS_RGN_T* pRgn, const char* szFile, const char* szFunc, const int nLine );
extern void*	OS_MallocRegion_Tag		( OS_RGN_T* pRgn, size_t size, const char* szFile, const char* szFunc, const int nLine );
extern int		OS_FreeRegion_Tag		( OS_RGN_T* pRgn, void* ptr, const char* szFile, const char* szFunc, const int nLine );
extern int		OS_GetRegionInfo_Tag	( OS_RGN_T* pRgn, OS_RGN_INFO_T* pMemInfo, const char* szFile, const char* szFunc, const int nLine );

extern int		OS_OpenCacheMap_Tag		( OS_CACHE_MAP_T* pMap, void* phys_addr, UINT32 length );
extern int		OS_CloseCacheMap_Tag	( OS_CACHE_MAP_T* pMap );
extern int		OS_InvalCacheMap_Tag	( OS_CACHE_MAP_T* pMap, UINT32 off, UINT32 length );
extern int		OS_FlushCacheMap_Tag	( OS_CACHE_MAP_T* pMap, UINT32 off, UINT32 length );
extern int		OS_CleanCacheMap_Tag	( OS_CACHE_MAP_T* pMap, UINT32 off, UINT32 length );
