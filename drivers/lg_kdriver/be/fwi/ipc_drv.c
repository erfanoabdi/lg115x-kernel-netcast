...
int BE_IPC_Init(void)
{
	...
	do {
		...
		ret = IPC_HAL_Init(&addrIPCBuffToMCU, &addrIPCBuffFrMCU);
...
int BE_IPC_Free(void)
{
	...
	do {
		...
		ret = IPC_HAL_Free(&addrIPCBuffToMCU, &addrIPCBuffFrMCU);
		if (ret) break;
...
int FWI_CPU_WakeUpIPC(void)
{
	...
	{
		ret = INT_HAL_WakeUpReg(INT_CPU_MCU_IPC, TRUE);
	}
...
void FWI_CPU_ACK_FROM_MCU(unsigned long temp)
{
	...
	switch (ipcMsgFrMcu->u16Type) {
		case FWI_IPC_CMD_SYNC :
			PWM_HAL_SetPwmSync(ipcMsgFrMcu->u32Data);
			break;
