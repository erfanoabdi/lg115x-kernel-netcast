...
int BE_PWM_PE_Ioctl(unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		...
		case BE_IOR_PE_GET_STATUS: 
		{
			#ifndef USE_BE_IOCTL_MACROS
			ret = BE_PE_Get_Status(arg);
			#else
			//Added macro code
			#endif
		}
...
int BE_PWM_Set_Control(unsigned long arg)
{
	...
	do{
		...
		ret = PWM_HAL_SetPwmControl(&stParams);
		if(ret) BREAK_WRONG(ret);
	} while(0);
...
int BE_PWM_Set_AdaptFreqControl(unsigned long arg)
{
	...
	do{
		...
		ret = PWM_HAL_SetPwmAdaptFreqControl(&stParams);
		if(ret) BREAK_WRONG(ret);
	} while(0);
...
int BE_PWM_Set_Frequency(unsigned long arg)
{
	...
	do{
		...
		ret = PWM_HAL_SetPwmFrequency(&stParams);
		if(ret) BREAK_WRONG(ret);
	} while(0);
...
int BE_PWM_Set_DutyCycle(unsigned long arg)
{
	...
	do{
		...
		ret = PWM_HAL_SetPwmDutyCycle(&stParams);
		if(ret) BREAK_WRONG(ret);
	} while(0);
...
int BE_PWM_Get_ExternalPwm(unsigned long arg)
{
	...
	do{
		...
		ret = PWM_HAL_GetPwmInfoExternal(&stParams);
		if(ret) BREAK_WRONG(ret);
...
int BE_PE_Get_Status(unsigned long arg)
{
	int ret = RET_OK;
	BE_PE_STATUS_T stParams;

	do{
		ret = copy_from_user(&stParams, (void __user *)arg, sizeof(BE_PE_STATUS_T));
		if(ret) BREAK_WRONG(ret);
		ret = BE_KIPC_SetData(BE_PE_GET_STATUS, &stParams, sizeof(BE_PE_STATUS_T));
		if(ret) BREAK_WRONG(ret);
		ret = BE_KIPC_GetData(&stParams, sizeof(BE_PE_STATUS_T));
		if(ret) BREAK_WRONG(ret);
		ret = copy_to_user((void __user *)arg, &stParams, sizeof(BE_PE_STATUS_T));
		if(ret) BREAK_WRONG(ret);
	} while(0);
	return ret;
}
