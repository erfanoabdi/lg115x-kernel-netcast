...
SINT32 CIS_WriteCOR(UINT8 mId)
{
	...
	{
		L9_CI_REG_WRITE((CIS_infoConfig[mId].baseAddr) , CIS_infoCftableEntry[mId].cfgTableIdx & 0x3f);
		read_data = ( UINT16 )( * ( UINT16* )( L9_CI_CAM_BASE + (CIS_infoConfig[mId].baseAddr)) );//CI_REG_READ(CIS_infoConfig[mId].baseAddr);
	}
	else
	{
		L8_CI_REG_WRITE((CIS_infoConfig[mId].baseAddr) , CIS_infoCftableEntry[mId].cfgTableIdx & 0x3f);
		read_data = ( UINT16 )( * ( UINT16* )( L8_CI_CAM_BASE + (CIS_infoConfig[mId].baseAddr)) );//CI_REG_READ(CIS_infoConfig[mId].baseAddr);
	}
...
static int CIS_ReadTuple(UINT32 baseAddr, UINT8 *pBuf, UINT32 *pOffset, SINT16 nSleepTime)
{
	...
	*pData++  =  tupleValue;
	...
	if( tupleValue > (CIS_TPL_BUF_SZ-2) )
		return TUPLE_END; // error case
