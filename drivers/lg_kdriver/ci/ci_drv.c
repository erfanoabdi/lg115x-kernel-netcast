...
static int CI_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if ( gpstCIReg )
	{
		...
		gCIReg_QB.uniCntrlStat3.unCntrlStat3 = gpstCIReg->uniCntrlStat3.unCntrlStat3;

		gCIReg_QB.uniIntrCntrl.unIntrCntrl = gpstCIReg->uniIntrCntrl.unIntrCntrl;

		gCIReg_QB.uniIntrMask.unIntrMask = gpstCIReg->uniIntrMask.unIntrMask;

		gCIReg_QB.uniIntrFlag.unIntrFlag = gpstCIReg->uniIntrFlag.unIntrFlag;

		gCIReg_QB.uniPCCardCntrl2.unPCCardCntrl2 = gpstCIReg->uniPCCardCntrl2.unPCCardCntrl2;

		gCIReg_QB.uniPCCardCntrl3.unPCCardCntrl3 = gpstCIReg->uniPCCardCntrl3.unPCCardCntrl3;

#if 0 /* testing */
		CI_RegPrint();

		gCIReg_QB.uniCntrlStat3.unCntrlStat3 = 0xF1;

		gCIReg_QB.uniIntrCntrl.unIntrCntrl = 0xF1;

		gCIReg_QB.uniIntrMask.unIntrMask = 0xF1;

		gCIReg_QB.uniIntrFlag.unIntrFlag = 0xF1;

		gCIReg_QB.uniPCCardCntrl2.unPCCardCntrl2 = 0xF1;

		gCIReg_QB.uniPCCardCntrl3.unPCCardCntrl3 = 0xF1;
	...
	{
		CI_DTV_SOC_Message(CI_DBG_INFO,"[%s] suspend done - -\n", CI_MODULE);
	}
...
static int CI_resume(struct platform_device *pdev)
{
	...
	if ( gpstCIReg )
	{
		gpstCIReg->uniCntrlStat3.unCntrlStat3 = gCIReg_QB.uniCntrlStat3.unCntrlStat3;

		gpstCIReg->uniIntrCntrl.unIntrCntrl = gCIReg_QB.uniIntrCntrl.unIntrCntrl;

		gpstCIReg->uniIntrMask.unIntrMask = gCIReg_QB.uniIntrMask.unIntrMask;

		gpstCIReg->uniIntrFlag.unIntrFlag = gCIReg_QB.uniIntrFlag.unIntrFlag;

		gpstCIReg->uniPCCardCntrl2.unPCCardCntrl2 = gCIReg_QB.uniPCCardCntrl2.unPCCardCntrl2;

		gpstCIReg->uniPCCardCntrl3.unPCCardCntrl3 = gCIReg_QB.uniPCCardCntrl3.unPCCardCntrl3;
	...
	{
		CI_DTV_SOC_Message(CI_DBG_INFO,"[%s] resume done - -\n", CI_MODULE);
	}
...
int CI_Init(void)
{
	...
	if ( g_ci_debug_fd >= 0 )
	{
		...
		OS_DEBUG_EnableModuleByIndex ( g_ci_debug_fd, 2, DBG_COLOR_NONE );
	...
	{
		DBG_PRINT_ERROR("CI platform driver register failed\n");
	}
	else
	{
		if(platform_device_register(&ci_device))
		{
			...
			DBG_PRINT_ERROR("CI platform device register failed\n");
		}
		else
		{
	...
	if ( err < 0 )
	{
		DBG_PRINT_ERROR("can't register ci device\n" );
		return -EIO;
	}
	...
	for ( i=0; i<CI_MAX_DEVICE; i++ )
	{
		...
		if (err)
		{
			DBG_PRINT_ERROR("error (%d) while adding ci device (%d.%d)\n", err, MAJOR(dev), MINOR(dev) );
			return -EIO;
		}
	...
	CI_PRINT("ci device initialized\n");
	...
	switch(cmd)
	{
		...
		case CI_IOR_DETECT_CARD:
		{
			...
			if( ret == 0 )
			{
				CI_DTV_SOC_Message(CI_DBG_INFO,"[ CI Card %s <<S>> ]  \n", ((uwDetect==1)? ("INSERTED") : ("REMOVED")));
	 	        ...
	 	        {
					CI_DTV_SOC_Message(CI_DBG_INFO,"[ CI_Ioctl(CI_IOR_DETECT_CARD): copy_to_user <<F>> ]\n");
		...
		case CI_IOR_READ_DATA:
		{
			if( arg )
			{
				...
				{
					CI_DTV_SOC_Message(CI_DBG_INFO,"[ CI_Ioctl(CI_IOR_READ_DATA): copy_to_user <<F>> ]\n");
				...
				if( copy_to_user( ( void __user * )arg, &stReadData, sizeof( LX_CI_IOCTL_PARAM_T ) ) )
	 	        {
					CI_DTV_SOC_Message(CI_DBG_INFO, " copy_to_user <<F>>\n" );
		...
		case CI_IOR_NEGOTIATE_BUF_SIZE:
		{
			...
			if( ret == 0 )
			{
				...
				if( copy_to_user( ( void __user * )arg, &uwBufSize, sizeof( UINT32 ) ) )
	 	        {
					CI_DTV_SOC_Message(CI_DBG_INFO,"[ CI_Ioctl(CI_IOR_NEGOTIATE_BUF_SIZE): copy_to_user <<F>> ] \n");
			...
			{
				CI_DTV_SOC_Message(CI_DBG_INFO,"[ CI_Ioctl(CI_IOR_NEGOTIATE_BUF_SIZE): "
		...
		case CI_IOR_READ_DA_STATUS:
		{
			...
			if( ret == 0 )
			{
				...
				if( copy_to_user( ( void __user * )arg, &uwDataAvailable, sizeof( UINT32 ) ) )
	 	        {
					CI_DTV_SOC_Message(CI_DBG_INFO,"[ CI_Ioctl(CI_IOR_READ_DA_STATUS): copy_to_user <<F>> ]\n");
		...
		case CI_IOW_WRITE_DATA:
		{
			if( arg )
			{
				...
				if ( copy_from_user( &stReadData, (void __user *)arg, sizeof( LX_CI_IOCTL_PARAM_T ) ) )
				{
					CI_DTV_SOC_Message(CI_DBG_INFO,"[ CI_Ioctl(CI_IOW_WRITE_DATA): copy_from_user <<F>> ]\n");
		...
		case CI_IOR_READ_IIR_STATUS:
		{
			...
			if( ret == 0 )
			{
				...
				if ( copy_to_user( ( void __user * )arg, &uwIIRStatus, sizeof( UINT32 ) ) )
	 	        {
					CI_DTV_SOC_Message(CI_DBG_INFO,"[ CI_Ioctl(CI_IOR_READ_IIR_STATUS): copy_to_user <<F>> ]\n");
		...
		case CI_IOR_CHECK_CAPABILITY:
		{
			...
			if( ret == 0 )
			{
				if ( copy_to_user( ( void __user * )arg, &stData, sizeof( LX_CI_IOCTL_PARAM_CAMTYPE ) ) )
	 	        {
					CI_DTV_SOC_Message(CI_DBG_INFO,"[ CI_Ioctl(CI_IOR_CHECK_CAPABILITY): copy_to_user <<F>> ]\n");
			...
			{
				CI_DTV_SOC_Message(CI_DBG_INFO,"[ CI_Ioctl(CI_IOR_CHECK_CAPABILITY): Check CAM Type <<F>> ]\n");
			}
		}
		break;
		...
		case CI_IOR_GET_CIPLUS_VERSION:
		{
			...
			if( ret == 0 )
			{
				if ( copy_to_user( ( void __user * )arg, &stData, sizeof( LX_CI_IOCTL_PARAM_VERSION ) ) )
	 	        {
					CI_DTV_SOC_Message(CI_DBG_INFO,"[ CI_Ioctl(CI_IOR_GET_CIPLUS_VERSION): copy_to_user <<F>> ]\n");
			...
			{
				CI_DTV_SOC_Message(CI_DBG_INFO,"[ CI_Ioctl(CI_IOR_GET_CIPLUS_VERSION): Check CAM Support Version <<F>> ]\n");
			}
		}
		break;
		...
		case CI_IOR_GET_CIPLUS_OPROFILE:
		{
			...
			if( ret == 0 )
			{
				if ( copy_to_user( ( void __user * )arg, &stData, sizeof( LX_CI_IOCTL_PARAM_OPROFILE ) ) )
	 	        {
					CI_DTV_SOC_Message(CI_DBG_INFO,"[ CI_Ioctl(CI_IOR_GET_CIPLUS_OPROFILE): copy_to_user <<F>> ]\n");
			...
			{
				CI_DTV_SOC_Message(CI_DBG_INFO,"[ CI_Ioctl(CI_IOR_GET_CIPLUS_OPROFILE): Check CAM Operator Profile <<F>> ]\n");
			}
		}
		break;
		...
		case CI_IOW_CAM_SET_DELAY:
		{
			if( arg )
			{
				...
				{
					CI_DTV_SOC_Message(CI_DBG_INFO,"[ CI_Ioctl(CI_IOW_CAM_SET_DELAY): copy_from_user <<F>> ]\n");
		...
		case CI_IOW_WRITE_REGISTER:
		{
			if( arg )
			{
				...
				{
					CI_DTV_SOC_Message(CI_DBG_INFO,"[ CI_Ioctl(LX_CI_IOCTL_PARAM_REGISTER): copy_from_user <<F>> ]\n");
		...
		{
			printk("\n\n printk: Ci_drv.c / CI_IOW_SET_PCMCIA_SPEED 1 \n\n");
			if( arg )
			{
				...
				{
					CI_DTV_SOC_Message(CI_DBG_INFO,"[ CI_Ioctl(LX_CI_IOCTL_PARAM_SPEED): copy_from_user <<F>> ]\n");
		...
		case CI_IOW_ENABLE_LOG:
		{
			...
			copy_from_user( &stLogEnable, (void __user *)arg, sizeof( LX_CI_IOCTL_PARAM_LOG_ENABLE ) );
			CI_IO_EnableLog(stLogEnable.mask);
			
			CI_DTV_SOC_Message( CI_DBG_INFO, "CI_IOW_ENABLE_LOG ok\n");
			ret = CI_IO_EnableLog( stLogEnable.mask );
			if(LX_IS_ERR(ret))
			{
				CI_DEBUG_Print("CI_IO_EnableLog failed:[%d]", ret);
