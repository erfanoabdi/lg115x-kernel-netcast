 ...
 *	function prototypes
 *----------------------------------------------------------------------------------------*/
extern int SDEC_HAL_GSTC(UINT32 *pStcc_bs_32_1, UINT32 *pStcc_bs_0);
...
static ClockTime CLK_GetGstc ( void )
{
	...
	//CLK_PRINT("gstc_vdec = %u gstc = %llu\n", gstc_vdec, gstc);
...
int CLK_DD_GetGstc ( UINT32 arg )
{
	...
	{
		CLK_PRINT("ioctl: copy_from_user error !!! \n");
		ret = -EFAULT;
		goto exit;
	}
	...
	{
		CLK_PRINT("ioctl: copy_to_user error !!! \n");
		ret = -EFAULT;
		goto exit;
	}
...
static ClockTime CLK_DD_ConvertTime ( LX_CLK_OBJ_T *clock, ClockTime internal)
{
	...
	{
		CLK_PRINT("there is overwrapped clock!!! so calculate again!!\n");
	...
	CLK_PRINT("ret = %llu lexternal= %llu\n", ret, lexternal);
...
int	CLK_DD_RevertTime ( UINT32 arg )
{
	...
	CLK_PRINT("external = %llu, last_itime = %llu, cexternal = %llu\n", external, clock->last_itime, cexternal);
