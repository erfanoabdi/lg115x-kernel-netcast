...
static int CLK_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	CLK_PRINT("[%s] Memorizing reg content\n", CLK_MODULE);
...
static int CLK_resume(struct platform_device *pdev)
{
	...
	CLK_PRINT("[%s] Restoring reg content\n", CLK_MODULE);
...
int CLK_Init(void)
{
	...
	{
		CLK_PRINT("[%s] platform driver register failed\n",CLK_MODULE);
	...
	else
	{
		if(platform_device_register(&clk_device))
		{
			...
			CLK_PRINT("[%s] platform device register failed\n",CLK_MODULE);
		}
		else
		{
			CLK_PRINT("[%s] platform register done\n", CLK_MODULE);
		}
	...
	CLK_PRINT("clk device initialized\n");
		...
		case CLK_IOR_GET_TIMESTAMP:
			{
				...
				CLK_TRACE("CLK_DD_GetTimestamp ok\n");
			}
			break;
		...
		case CLK_IOR_GET_GSTC:
			{
				...
				CLK_TRACE("CLK_DD_RevertTime ok\n");
			}
			break;
