...
int DEMOD_MediaChange(DEMOD_CTX_T *pDemodInfo, LX_DEMOD_INIT_PARAM_T *pTranMedia)
{
	...
	if((pDemodInfo->operMode == LX_DEMOD_ANALOG) || (pDemodInfo->operMode == LX_DEMOD_UNKOWN_OPMODE))
	{
		...
		DEMOD_PRINT("[DEMOD] Success (%s:%d) Oper mode = %s, changed to power save mode !!!\n", __F__, __L__,"Analog or unkown");
		...
		case LX_DEMOD_DVBC :
			{
				...
				operMode = pDemodInfo->operMode;
				bandWd	 = LX_DEMOD_DVBT_NO_OPERATION; //pDemodInfo->setCfgParam.dvbc.channelBW;
				symbr	 = pDemodInfo->symbr;
		...
		case LX_DEMOD_DVBC :
			{
				...
				else
				{
					...
					if(RET_OK != DEMOD_DVBC_AutoSymbolRateDet(LX_DEMOD_SYMR_FIXED_MODE, pDemodInfo->setCfgParam.dvbc.symbolRate))
					{
						DEMOD_PRINT("[DEMOD] FAIL (%s:%d) DEMOD_DVBC_AutoSymbolRateDet() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
		...
		case LX_DEMOD_DVBT :
			{
				...
				if(LX_DEMOD_LOCKED == pDemodInfo->lockStatus)
				{
					...
					{
						pDemodInfo->getCfgParam.dvbt.bSpectrumInv	= pDemodInfo->getCfgParam.dvbt.bSpectrumInv;
					}
					...
					pDemodInfo->getCfgParam.dvbt.channelBW		= channelBW;
		...
		case LX_DEMOD_DVBC :
			{
				...
				if(LX_DEMOD_LOCKED == pDemodInfo->lockStatus)
				{
					...
					pDemodInfo->getCfgParam.dvbc.channelBW		= channelBW;
				}
		...
		case LX_DEMOD_QAM :
			{
				...
				if(LX_DEMOD_LOCKED == pDemodInfo->lockStatus)
				{
					...
					pDemodInfo->getCfgParam.qam.channelBW		= channelBW;
		...
		case LX_DEMOD_ISDBT :
			{
				...
				if(LX_DEMOD_LOCKED == pDemodInfo->lockStatus)
				{
					...
					pDemodInfo->getCfgParam.isdbt.channelBW		= channelBW;
		...
		case LX_DEMOD_NEWTUNE_REQUEST:
			{
				if((pDemodInfo->operMode != LX_DEMOD_ANALOG) && (pDemodInfo->operMode != LX_DEMOD_UNKOWN_OPMODE))
				{
					...
					notifyinglockTimeOut 					= LX_DEMOD_LOCK_DEALY_2000;
				...
				else
				{
					...
					DEMOD_PRINT("\nDEMOD_NEWTUNE : Analog tune From Channel Manager  !!\n");
				}
