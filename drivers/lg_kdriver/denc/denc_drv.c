...
int DENC_Init(void)
{
	...
	if (g_denc_debug_fd < 0) {
		DBG_PRINT_ERROR("can't open denc module\n" );
		return -EIO;
	}
	OS_DEBUG_EnableModule ( g_denc_debug_fd );

#if 0
	OS_DEBUG_EnableModuleByIndex ( g_denc_debug_fd, 0, DBG_COLOR_NONE );
	OS_DEBUG_EnableModuleByIndex ( g_denc_debug_fd, 1, DBG_COLOR_NONE );
	OS_DEBUG_EnableModuleByIndex ( g_denc_debug_fd, 2, DBG_COLOR_NONE );
	OS_DEBUG_EnableModuleByIndex ( g_denc_debug_fd, 3, DBG_COLOR_NONE );
#endif
	...
	{
		DENC_PRINT("[%s] platform driver register failed\n",DENC_MODULE);
	...
	else
	{
		if(platform_device_register(&denc_device))
		{
			...
			DENC_PRINT("[%s] platform device register failed\n",DENC_MODULE);
		}
		else
		{
			DENC_PRINT("[%s] platform register done\n", DENC_MODULE);
		}
	...
	if ( err < 0 )
	{
		DBG_PRINT_ERROR("can't register denc device\n" );
		return -EIO;
	}
	...
	for ( i=0; i<DENC_MAX_DEVICE; i++ )
	{
		...
		if (err)
		{
			DBG_PRINT_ERROR("error (%d) while adding denc device (%d.%d)\n", err, MAJOR(dev), MINOR(dev) );
			return -EIO;
		}
	...
	DENC_PRINT("denc device initialized\n");
