...
#include "../de/de_def.h"
#include "../de/l9/de_reg_def_l9.h"

#include "l9/de_atp_reg_l9.h"      	// 0x3c00
#include "../de/l9/de_cvd_reg_l9.h"       // 0x4100
#include "denc_kapi.h"
#include "l9/denc_reg_l9.h"
#include "../de/l9/ctop_ctrl_reg_l9.h"
...
extern volatile DE_CVD_REG_L9_T gDE_CVD_L9;
#endif
#ifdef INCLUDE_KDRV_SYS
#ifdef INCLUDE_KDRV_DE
extern volatile CTOP_CTRL_REG_L9_T gDE_CTOP_L9;
#endif
#endif
 ...
 *   Static Variables
 *---------------------------------------------------------------------------------------*/
static volatile DE_ATP_REG_L9_T gDE_ATP_L9;
...
#endif
#ifdef INCLUDE_KDRV_DE
		DE_CVD_L9_FLWr(cvbsafe_001, 0x01311000);  // VDAC On
#endif

		if ( lx_chip_rev() >= LX_CHIP_REV(L9,B0) ) {
#ifdef INCLUDE_KDRV_SYS
#ifdef INCLUDE_KDRV_DE
			CTOP_CTRL_L9B_RdFL(ctr27_extclk_swrst);
			CTOP_CTRL_L9B_Wr01(ctr27_extclk_swrst, reg_swrst_cve27, 0x0);
			CTOP_CTRL_L9B_Wr01(ctr27_extclk_swrst, reg_swrst_cve54, 0x0);
			CTOP_CTRL_L9B_WrFL(ctr27_extclk_swrst);
#endif
#endif
		}

		ret = DENC_REG_L9_NTSC_Init();
	} while (0);
...
int DENC_REG_L9_OnOff ( BOOLEAN *pOnOff )
{
	...
	do {
		DE_ATP_L9_FLCM(cve_config, enc_en, *pOnOff, 0x1);
...
int DENC_REG_L9_SetOutPutVideoStd(LX_DENC_VIDEO_SYSTEM_T *pstParams)
{
	...
	DE_ATP_L9_FLWf(cve_config, standard , *pstParams);
...
int DENC_REG_L9_TtxEnable(BOOLEAN *pstParams)
{
	...
	if ( lx_chip_rev() >= LX_CHIP_REV(L9,B0) ) {
		...
		DE_ATP_L9A_FLCM(denc_vbi_ctrl, ttx_en, *pstParams, 0x1);
	}
...
int DENC_REG_L9_WssEnable(BOOLEAN *pstParams)
{
	...
	DE_ATP_L9_FLCM(denc_vbi_ctrl, wss_en, *pstParams, 0x1);
    DE_ATP_L9_RdFL(vps_ctrl);
	if ( lx_chip_rev() >= LX_CHIP_REV(L9,B0) ) {
		DE_ATP_L9B_WfCM(vps_ctrl, wss_odd_en,   *pstParams, 0x1);
		DE_ATP_L9B_WfCM(vps_ctrl, wss_even_en,  *pstParams, 0x1);
	} else {
		DE_ATP_L9A_WfCM(vps_ctrl, wss_odd,     *pstParams, 0x1);
		DE_ATP_L9A_WfCM(vps_ctrl, wss_even,    *pstParams, 0x1);
	}
    DE_ATP_L9_WrFL(vps_ctrl);
...
int DENC_REG_L9_VpsEnable(BOOLEAN *pstParams)
{
    ...
    DE_ATP_L9_RdFL(vps_ctrl);
    DE_ATP_L9_WfCM(vps_ctrl, vbi_odd,    *pstParams, 0x1);
    DE_ATP_L9_WfCM(vps_ctrl, vbi_odd,    *pstParams, 0x1);
    DE_ATP_L9_Wr01(vps_ctrl, vbi_even,               0x0);
    DE_ATP_L9_Wr01(vps_ctrl, lpf_bypass,             0x0);
    DE_ATP_L9_WrFL(vps_ctrl);
...
int DENC_REG_L9_SetTtxData(LX_DENC_TTX_DATA_T *pstParams)
{
	...
	do {
		...
		DE_ATP_L9_FLWf(ttx_ver_ctrl1, ttx_line, pstParams->Line_Number );

		DE_ATP_L9_RdFL(ttx_load);
		DE_ATP_L9_Wr01(ttx_load, ttx_load, 1);
		for ( i=0; i<ttxSize;i++ ) {
			DE_ATP_L9_FLWr(ttx_data, pTtxData[i]);
			DE_ATP_L9_Wr01(ttx_load, ttx_write_addr, i);
			DE_ATP_L9_WrFL(ttx_load);
		}
		DE_ATP_L9_Wr01(ttx_load, ttx_load, 0);
		DE_ATP_L9_WrFL(ttx_load);
	} while (0);
...
int DENC_REG_L9_SetWssData(LX_DENC_WSS_DATA_T *pstParams)
{
	...
	DE_ATP_L9_FLCM(denc_vbi_ctrl, wss_en, pstParams->Enable, 0x1);
    DE_ATP_L9_RdFL(vps_ctrl);
	if ( lx_chip_rev() >= LX_CHIP_REV(L9,B0) ) {
		DE_ATP_L9B_WfCM(vps_ctrl, wss_odd_en,     pstParams->Enable, 0x1);
		DE_ATP_L9B_WfCM(vps_ctrl, wss_even_en,    pstParams->Enable, 0x1);
	} else {
		DE_ATP_L9A_WfCM(vps_ctrl, wss_odd,     pstParams->Enable, 0x1);
		DE_ATP_L9A_WfCM(vps_ctrl, wss_even,    pstParams->Enable, 0x1);
	}
    DE_ATP_L9_WrFL(vps_ctrl);
	DE_ATP_L9_FLWf(wss_data, wss_data, pstParams->WSS_DATA);
...
int DENC_REG_L9_SetVpsData(LX_DENC_VPS_DATA_T *pstParams)
{
    ...
    DE_ATP_L9_RdFL(vps_ctrl);
    DE_ATP_L9_WfCM(vps_ctrl, vps_enable, pstParams->Enable, 0x1);
    DE_ATP_L9_WfCM(vps_ctrl, vbi_odd,    pstParams->Enable, 0x1);
    DE_ATP_L9_Wr01(vps_ctrl, vbi_even,                      0x0);
    DE_ATP_L9_Wr01(vps_ctrl, lpf_bypass,                    0x0);
    DE_ATP_L9_Wr01(vps_ctrl, vps_wr_done,                   0x0);
    vpsData = 0;
    for (i=0;i<15;i++) {
        vpsData <<= 8;
        vpsData  |= pstParams->pVPS_DATA[i];
        if ( !(i%2) ) continue;
        DE_ATP_L9_Wr01(vps_ctrl, vps_data,    vpsData);
        DE_ATP_L9_WrFL(vps_ctrl);
        vpsData = 0;
    }
    DE_ATP_L9_Wr01(vps_ctrl, vps_wr_done, 0x1);
    DE_ATP_L9_Wr01(vps_ctrl, vps_data,    vpsData);
    DE_ATP_L9_WrFL(vps_ctrl);
...
int DENC_REG_L9_ColorBarEnable(BOOLEAN *pstParams)
{
	...
	DE_ATP_L9_FLCM(cve_config, color_bar_en, *pstParams, 0x1);
...
{
    DE_ATP_L9_FLWr(ttx_ver_ctrl0, 0x17061606);
    DE_ATP_L9_FLWr(ttx_ver_ctrl1, 0x0003c4c0);
    DE_ATP_L9_FLWr(wss_data,      0x6a5a9919);
    DE_ATP_L9_FLWr(cgms_data,     0x0B800000);
    DE_ATP_L9_FLWr(cve_config,    0x0B381b81);
    DE_ATP_L9_FLWr(phase_incr,    0x2a098acb);
    DE_ATP_L9_FLWr(denc_vbi_ctrl, 0x4ee00800);
    DE_ATP_L9_FLWr(cc_data,       0x80808080);

	DE_ATP_L9_FLWr(mv_n0_3,		  0x21171500);
	DE_ATP_L9_FLWr(mv_n4_9,		  0x1BB61B15);
	DE_ATP_L9_FLWr(mv_n10_11,	  0x07F80024);
	DE_ATP_L9_FLWr(mv_n12_14,	  0x0F0F0000);
	DE_ATP_L9_FLWr(mv_n15_20,	  0xA05C0060);

	DE_ATP_L9_FLWr(vps_ctrl, 0x123400a0);
	DE_ATP_L9_FLWr(vps_ctrl, 0x567800a1);
	DE_ATP_L9_FLWr(vps_ctrl, 0x9abc00a2);
	DE_ATP_L9_FLWr(vps_ctrl, 0xdef100a3);
	DE_ATP_L9_FLWr(vps_ctrl, 0x234500a4);
	DE_ATP_L9_FLWr(vps_ctrl, 0x678900a5);
	DE_ATP_L9_FLWr(vps_ctrl, 0xabcd01a6);
...
int DENC_REG_L9_VdacPowerControl(BOOLEAN *pstParams)
{
	...
	else
	{
		...
		//DE_CVD_L9_FLWr(cvbsafe_001, 0x00300000);  // VDAC On
		/*
		 VPORT_RdFL(cvbsafe_001);
