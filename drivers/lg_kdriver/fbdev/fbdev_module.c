...
----------------------------------------------------------------------------------------*/
extern void     FBDEV_L9_InitHWCtx  (FBDEV_HW_CTX_T*  hal); /* implemented at l8/gfx_draw_l9.c */
extern void     FBDEV_H13_InitHWCtx (FBDEV_HW_CTX_T*  hal); /* implemented at l8/gfx_draw_l9.c */
...
FBDEV_WIN_CTX_T* FBDEV_GetWinCtx (int layer)
{
	switch( layer )
	{
		...
		case LX_FBDEV_ID_OSD3:	return g_dm_ctx->winfo[layer]; 
...
{
	FBDEV_WIN_CTX_T* fb_win_ctx = FBDEV_GetWinCtx(layer);

	return (fb_win_ctx)? &fb_win_ctx->pixel : NULL;
}
...
void FBDEV_InitHWCtx ( void )
{
    ...
    {
       FBDEV_H13_InitHWCtx( &g_fbdev_hw_ctx );
    }
    else if( !LX_COMP_CHIP(lx_chip_rev(), LX_CHIP_L9 ) )
    {
        FBDEV_L9_InitHWCtx( &g_fbdev_hw_ctx );
    }
    else
    {
 ...
 *
 */
void FBDEV_InitCanvas(FBDEV_WIN_CTX_T *winfo)
{
	...
	hCanvas->bReverseScreenEnable= FALSE;
	hCanvas->bPixelOrderEnable	= FALSE;
	hCanvas->bColorKeyEnable	= FALSE;
	hCanvas->bGlobalAlphaEnable	= g_fbdev_cfg->fb[fb_dev_id].b_use_global_alpha;
	
	hCanvas->bits_per_pixel		= vinfo->bits_per_pixel;
//	hCanvas->eOSDPixFmt 		= FBDEV_GetOSDPxlFmt( hCanvas->bits_per_pixel );
//	hCanvas->eOSDPixDepth		= FBDEV_GetOSDPxlDepth ( hCanvas->bits_per_pixel );
 ...
 */
int FBDEV_CleanupLayer(FBDEV_WIN_CTX_T *winfo)
{
	if ( winfo )
	{
...
{
	FBDEV_CHECK_CODE( NULL == g_fbdev_hw_ctx.SetViewCtrl, return RET_ERROR, "not supported function\n"); 
...
{
	int	ret = RET_OK;
	if ( g_fbdev_hw_ctx.SetOutPathCtrl )
	{
		ret = g_fbdev_hw_ctx.SetOutPathCtrl( layer, ctrl );
	}
	return ret;
}
