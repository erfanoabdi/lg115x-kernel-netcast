...
#undef	USE_DST_CACHE
	...
	/* ioremap */
#ifdef USE_SRC_CACHE
//	src_vaddr = (UINT8*)ioremap_cached( (UINT32)src_setting.phys_addr, src_offset + src_setting.stride * sh );
	src_vaddr = (UINT8*)ioremap_cached( (UINT32)src_setting.phys_addr, src_setting.stride * src_setting.height );
#else
//	src_vaddr = (UINT8*)ioremap( (UINT32)src_setting.phys_addr, src_offset + src_setting.stride * sh );
	src_vaddr = (UINT8*)ioremap( (UINT32)src_setting.phys_addr, src_setting.stride * src_setting.height );
#endif
	GFX_CHECK_CODE( NULL == src_vaddr, goto func_exit, "ioremap failed\n");

#ifdef USE_DST_CACHE
//	dst_vaddr = (UINT8*)ioremap_cached( (UINT32)dst_setting.phys_addr, dst_offset + dst_setting.stride * sw );
	dst_vaddr = (UINT8*)ioremap_cached( (UINT32)dst_setting.phys_addr, dst_setting.stride * dst_setting.height );
#else
//	dst_vaddr = (UINT8*)ioremap( (UINT32)dst_setting.phys_addr, dst_offset + dst_setting.stride * sw );
	dst_vaddr = (UINT8*)ioremap( (UINT32)dst_setting.phys_addr, dst_setting.stride * dst_setting.height );
#endif
	GFX_CHECK_CODE( NULL == dst_vaddr, goto func_exit, "ioremap failed\n");

#ifdef USE_SRC_CACHE
	{
		enum dma_data_direction dir = DMA_FROM_DEVICE;
//		dmac_map_area( (void*)src_vaddr, src_setting.stride * sh, dir );
//		outer_inv_range( (UINT32)src_setting.phys_addr, (UINT32)src_setting.phys_addr + src_offset + src_setting.stride * sh -1 );
		dmac_map_area( (void*)src_vaddr, src_setting.stride * src_setting.height, dir );
		outer_inv_range( (UINT32)src_setting.phys_addr, (UINT32)src_setting.phys_addr + src_setting.stride * src_setting.height -1 );
	}
#endif

#define	SRC_XY_POS(x,y)		*(UINT32*)(src_vaddr + ((x)<<2) + (y)*src_setting.stride)
#define	DST_XY_POS(x,y)		*(UINT32*)(dst_vaddr + ((x)<<2) + (y)*dst_setting.stride)
...
func_exit:
	...
	if ( dst_vaddr )
	{
//		dmac_flush_range( dst_vaddr, (UINT32)dst_vaddr + dst_offset, dst_setting.stride * sw -1 );
		enum dma_data_direction dir = DMA_TO_DEVICE;
		dmac_map_area( (void*)dst_vaddr, dst_offset + dst_setting.stride * sw, dir );
		outer_clean_range( (UINT32)dst_setting.phys_addr, (UINT32)dst_setting.phys_addr + dst_offset + dst_setting.stride * sw - 1);
	}
#endif

	if( src_vaddr ) iounmap( src_vaddr );
	if( dst_vaddr ) iounmap( dst_vaddr );
}
