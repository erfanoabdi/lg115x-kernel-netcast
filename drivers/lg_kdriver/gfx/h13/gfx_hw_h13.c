...
#define GFX_WRITE_REG(reg,val)	\
    __raw_writel(val , (ULONG)(g_gfx_reg_h13) + (ULONG)OFFSET(GFX_H13_REG_T,reg))

#define GFX_READ_REG(reg)	\
    __raw_readl( (ULONG)(g_gfx_reg_h13) + (ULONG)OFFSET(GFX_H13_REG_T,reg))
...
static int	_g_gfx_h13_cmd_delay		= 0x0;
static int	_g_gfx_h13_cmd_delay_req	= 0x0;
module_param_named( gfx_h13_cmd_delay, _g_gfx_h13_cmd_delay_req, int, 0644 );
...
int GFX_H13_RunStartCommand(void)
{
	...
	if ( _g_gfx_h13_bw_val != _g_gfx_h13_bw_val_req	)
	{
		...
		printk("[gfx] bw_limit changed 0x%x -> 0x%x\n", _g_gfx_h13_bw_val, _g_gfx_h13_bw_val_req );
		_g_gfx_h13_bw_val = _g_gfx_h13_bw_val_req;
		OS_WrReg( 0xc000e4f8, _g_gfx_h13_bw_val );
	}

	/* write B/W limit if changed */
	if ( _g_gfx_h13_cmd_delay != _g_gfx_h13_cmd_delay_req	)
	{
		_g_gfx_h13_cmd_delay_req &= 0xffff;

		printk("[gfx] cmd_delay changed 0x%x -> 0x%x\n", _g_gfx_h13_cmd_delay, _g_gfx_h13_cmd_delay_req );
		_g_gfx_h13_cmd_delay = _g_gfx_h13_cmd_delay_req;

		GFX_RdFL( gfx_cmd_dly );
		GFX_Wr01( gfx_cmd_dly, gfx_cmd_dly, _g_gfx_h13_cmd_delay );
...
int GFX_H13_SwResetCommand(void)
{
	...
	GFX_TRACE_BEGIN();
//	GFX_WRITE_REG( scaler_frame_start, 0x1 );		/* reset scaler */
	GFX_WRITE_REG( gfx_reset, 0x1 );				/* reset GFX */
	...
	for ( i=0; i<100; i++ )
	{
		if( g_gfx_reg_h13->gfx_status2.mflust_st == 0x0 ) break;
	}
...
void GFX_H13_SetOperationStatus(GFX_CMD_OP_T *operate)
{
	 ...
	 */
#if 0
    GFX_REG_MEMCPY( &g_gfx_reg_h13->op_mode, &g_gfx_reg_h13_cache->op_mode, 3 /* reg count = 3 */ );
#else
...
void GFX_H13_SetScalerConfigure(GFX_SCALER_CONFIGURE_T* scaler)
{
	switch( scaler->cmd )
	{
		...
		case GFX_SCALER_CMD_START:
		{
			...
			GFX_Wr01( scaler_vh_order,		scaler_vh_order,		0x1 ); // FIXME	: H -> V
...
{
	GFX_TRACE_BEGIN();
	g_gfx_reg_h13->gfx_cmd_dly.gfx_cmd_dly = delay;
	GFX_TRACE_END();
 ...
 * @ingroup osd_ddi_func
*/
void GFX_H13_SetRegister(UINT32 addr, UINT32 val)
{
#if 1
	__raw_writel(val , (ULONG)(g_gfx_reg_h13) + addr);
#else
	/* do nothing */
#endif
}

UINT32 GFX_H13_GetRegister(UINT32 addr)
{
#if 1
    return __raw_readl( (ULONG)(g_gfx_reg_h13) + addr);
#else
	/* do nothing */ return 0;
#endif
}
