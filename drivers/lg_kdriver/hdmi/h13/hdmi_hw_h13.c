...
static HDMI_PORT_CONNECTION_TYPE_T _gHDMIConnectState = HDMI_PORT_NOT_CONNECTED;
static LX_HDMI_TIMING_INFO_T _gPrevTiming = {0,};
static UINT32 _gTimingReadCnt = 0;
...
static LX_HDMI_INFO_PACKET_T gPrevAVIPacket = {0, };
static LX_HDMI_INFO_PACKET_T gPrevVSIPacket = {0, };
static BOOLEAN _gAVIReadState 	  = FALSE;
static BOOLEAN _gVSIState 	  = FALSE;

//audio
static BOOLEAN _gAudioMuteState 	  = FALSE;
static UINT32  _gIntrAudioState 	  = HDMI_AUDIO_INIT_STATE;
static UINT32  _gHdmiPortStableCount  = 0;	//This count is used to detect if HDMI Switch port is changed.
static UINT32  _gHdmiFreqErrorCount   = 0;	//This count is used to detect if HDMI TMDS Frequency and Frequency is different.
static UINT32  _gHdmiAudioThreadCount = 0;	//_gHdmiAudioThreadCount is increased in _HDMI_H13_Periodic_Audio_Task function
static LX_HDMI_AUDIO_INFO_T _gHdmiAudioInfo = {LX_HDMI_AUDIO_DEFAULT, LX_HDMI_SAMPLING_FREQ_NONE};

//interrupt variables
static UINT32 _gIntr_vf_stable = HDMI_VIDEO_INIT_STATE;
static UINT32 _gIntr_avi = HDMI_AVI_INIT_STATE;
static UINT32 _gIntr_src_mute = HDMI_SOURCE_MUTE_CLEAR_STATE;
static UINT32 _gIntr_vsi = HDMI_VSI_INIT_STATE;

static UINT32 _gForce_thread_sleep  = 0;	//enable  = 2, disable  =0
static UINT32 _gHDMI_thread_running = 1;	//enable  = 1, disable  =0

//wait_queue_head_t	WaitQueue_HDMI;
DECLARE_WAIT_QUEUE_HEAD(WaitQueue_HDMI);
 ...
 *	access functions
 */
struct task_struct	*stHDMI_Thread;
 ...
 *				Local function
 *****************************************************************************/
static int _HDMI_H13_GetExtendTimingInfo(LX_HDMI_TIMING_INFO_T *info);
...
{
	gPHY_REG_CTRL_H13.shdw.addr = (UINT32 *)OS_KMalloc(sizeof(PHY_REG_CTRL_H13_T));

#ifndef	USE_QEMU_SYSTEM
	pstLinkReg =(HDMI_LINK_REG_H13_T *)ioremap(pHdmiRegCfg->link_reg_base_addr, pHdmiRegCfg->link_reg_size);
#else
	pstLinkReg =(HDMI_LINK_REG_H13_T *)ioremap(pHdmiRegCfg->link_qemu_base_addr, pHdmiRegCfg->link_reg_size);
#endif

	HDMI_PRINT("HDMI LINK REG CFG \n");
	HDMI_PRINT("LINK REG Base Address	0x%08x\n", pHdmiRegCfg->link_reg_base_addr);
	HDMI_PRINT("LINK REG Size 			0x%08x\n", pHdmiRegCfg->link_reg_size);
	HDMI_PRINT("LINK REG Qumu Address 	0x%08x\n", pHdmiRegCfg->link_qemu_base_addr);
	...
	stHDMI_Thread = kthread_create(_HDMI_H13_Thread, (void*)NULL, "hdmi_thread");
	if(stHDMI_Thread)
	{
...
int HDMI_H13_SetPort(UINT32 *port)
{
	if(*port)
	{
		_gForce_thread_sleep = 2;
		_gH13HDMIState = HDMI_STATE_DISABLE;
		HDMI_DEBUG("H13 sleeping HDMI Thread \n");
	}
	else
	{
		wake_up_interruptible(&WaitQueue_HDMI);
		_gH13HDMIState = HDMI_STATE_IDLE;
		_gForce_thread_sleep = 0;
		_gHDMI_thread_running = 1;
...
{
	ratio->eAspectRatio = (((gPrevAVIPacket.dataBytes[0] &0xff0000)>>16) &0x30)>>4;		// M1M0
...
int HDMI_H13_GetColorDomain(LX_HDMI_COLOR_DOMAIN_T *color)
{
	...
	if(_gIntr_vf_stable == HDMI_VIDEO_INIT_STATE || _gIntr_avi == HDMI_AVI_INIT_STATE)
	{
		return RET_OK;
	}

	if(_gIntr_avi > HDMI_AVI_INIT_STATE)
	{
		color->bHdmiMode =  gBootData->mode;

		if(color->bHdmiMode)
		{
			if(_gAVIReadState)
				temp = gPrevAVIPacket.dataBytes[0];
			else
			{
...
int HDMI_H13_GetTimingInfo(LX_HDMI_TIMING_INFO_T *info)
{
	...
	do{
		if(_gHDMI_thread_running == 1 && _gH13HDMIState < HDMI_STATE_READ)
			break;

		if(_gHDMI_thread_running == 1 && _gH13HDMIState == HDMI_STATE_STABLE)
			goto func_exit;
		...
		_HDMI_H13_GetExtendTimingInfo(&bufTiming);		// get extend information
		...
		if((_gH13HDMIState > HDMI_STATE_NO_SCDT && _gH13HDMIState <= HDMI_STATE_READ) ||_gHDMI_thread_running == 0)
		{
			if(_gIntr_vf_stable > HDMI_VIDEO_INIT_STATE)
			{
		...
		if( _gTimingReadCnt > 100) 	goto func_exit;

		if( memcmp(&_gPrevTiming, &bufTiming, sizeof(LX_HDMI_TIMING_INFO_T)) == 0 )
		{
			_gTimingReadCnt++;

			if(_gH13HDMIState == HDMI_STATE_READ && _gTimingReadCnt < 9)
				HDMI_VIDEO("[%d] %s : hActive[%d] vActive [%d] ReadCnt[%d] \n", __L__, __F__, bufTiming.hActive, bufTiming.vActive,_gTimingReadCnt);
		}
...
func_exit:
		...
		memcpy(&bufTiming, &_gPrevTiming, sizeof(LX_HDMI_TIMING_INFO_T));
	} while(0);
...
int HDMI_H13_GetAviPacket(LX_HDMI_INFO_PACKET_T *packet)
{
	...
	{
		gAVIReadCnt = 0;
		memset(&gPrevAVIPacket, 0 , sizeof(LX_HDMI_INFO_PACKET_T));
		//gPrevAVIPacket.InfoFrameType = LX_HDMI_INFO_AVI;
		return RET_OK;
	}

	if( _gHDMI_thread_running == 1 && _gIntr_avi == HDMI_AVI_STABLE_STATE)
		goto func_exit;

	if(_gIntr_avi == HDMI_AVI_CHANGE_STATE)
	{
		gAVIReadCnt = 0;
		spin_lock_irqsave(&_gIntrHdmiH13VideoLock, flags);
		_gIntr_avi = HDMI_AVI_INTERRUPT_STATE;
		_gAVIReadState = FALSE;
	...
	{
		spin_lock_irqsave(&_gIntrHdmiH13VideoLock, flags);
		_gIntr_avi = HDMI_AVI_STABLE_STATE;
		spin_unlock_irqrestore(&_gIntrHdmiH13VideoLock, flags);
	}
	...
	if( _gHDMI_thread_running == 1)
		packet->dataBytes[7] = gBootData->mode;		//HDMI Mode
	else
	{
		LINK_H13_RdFL(system_control_00);
		LINK_H13_Rd01(system_control_00, reg_hdmi_mode, data[7]);
		packet->dataBytes[7] = gBootData->mode = data[7];		//HDMI Mode
	}
	...
	if( memcmp(&gPrevAVIPacket, packet, sizeof(LX_HDMI_INFO_PACKET_T)) != 0 )
	{
		gAVIReadCnt = 0;
		HDMI_VIDEO("[%d] %s : changed AVI Packet / InfoFrameType gPre[%d] /packet[%d] \n", __L__, __F__, gPrevAVIPacket.InfoFrameType, packet->InfoFrameType);
		HDMI_VIDEO("AVI header	gPre[%d] /packet[%d] \n", gPrevAVIPacket.header, packet->header);
		HDMI_VIDEO("AVI data	gPre[0x%x][0x%x][0x%x][0x%x][0x%x][0x%x][0x%x][0x%x] \n", gPrevAVIPacket.dataBytes[0], gPrevAVIPacket.dataBytes[1], gPrevAVIPacket.dataBytes[2], \
			gPrevAVIPacket.dataBytes[3], gPrevAVIPacket.dataBytes[4], gPrevAVIPacket.dataBytes[5], gPrevAVIPacket.dataBytes[6], gPrevAVIPacket.dataBytes[7]);
		...
		memcpy(&gPrevAVIPacket, packet, sizeof(LX_HDMI_INFO_PACKET_T));
		return RET_OK;
	}
	else
	{
...
func_exit:
	memcpy(packet, &gPrevAVIPacket,  sizeof(LX_HDMI_INFO_PACKET_T));
	HDMI_PRINT("[%d] %s \n", __L__, __F__);
...
int HDMI_H13_GetVsiPacket(LX_HDMI_INFO_PACKET_T *packet)
{
	...
	//HDMI_DEBUG("%s, %d _gIntr_vf_stable= %d,_gIntr_vsi = %d, \n\n",__F__, __L__, _gIntr_vf_stable, _gIntr_vsi);

	if(_gIntr_vf_stable == HDMI_VIDEO_INIT_STATE ||_gIntr_vsi < HDMI_VSI_INTERRUPT_STATE)
	{
		gVSIReadCnt = 0;
		memset(&gPrevVSIPacket, 0 , sizeof(LX_HDMI_INFO_PACKET_T));
		gPrevVSIPacket.InfoFrameType = LX_HDMI_INFO_VSI;
		return RET_OK;
	}
	...
	if( _gHDMI_thread_running == 1 && _gIntr_vsi == HDMI_VSI_STABLE_STATE)
		goto func_exit;

	if(gVSIReadCnt > 200)
	{
		spin_lock_irqsave(&_gIntrHdmiH13VideoLock, flags);
		_gIntr_vsi = HDMI_VSI_STABLE_STATE;
		spin_unlock_irqrestore(&_gIntrHdmiH13VideoLock, flags);
	}
	...
	if( memcmp(&gPrevVSIPacket, packet, sizeof(LX_HDMI_INFO_PACKET_T)) != 0 )
	{
		gVSIReadCnt = 0;
		HDMI_VIDEO("[%d] %s : changed VSI Packet / InfoFrameType gPre[%d] /packet[%d] \n", __L__, __F__, gPrevVSIPacket.InfoFrameType, packet->InfoFrameType);
		HDMI_VIDEO("VSI header	gPre[%d] /packet[%d] \n", gPrevVSIPacket.header, packet->header);
		HDMI_VIDEO("VSI data	gPre[0x%x][0x%x][0x%x][0x%x][0x%x][0x%x][0x%x][0x%x] \n", gPrevVSIPacket.dataBytes[0], gPrevVSIPacket.dataBytes[1], gPrevVSIPacket.dataBytes[2], \
			gPrevVSIPacket.dataBytes[3], gPrevVSIPacket.dataBytes[4], gPrevVSIPacket.dataBytes[5], gPrevVSIPacket.dataBytes[6], gPrevVSIPacket.dataBytes[7]);
		HDMI_VIDEO("VSI data	packet[0x%x][0x%x][0x%x][0x%x][0x%x][0x%x][0x%x][0x%x] \n", 	packet->dataBytes[0], packet->dataBytes[1], packet->dataBytes[2], \
			packet->dataBytes[3], packet->dataBytes[4], packet->dataBytes[5], packet->dataBytes[6], packet->dataBytes[7]);

		memcpy(&gPrevVSIPacket, packet, sizeof(LX_HDMI_INFO_PACKET_T));
		return RET_OK;
	}
	else
	{
...
func_exit:
	memcpy(packet, &gPrevVSIPacket, sizeof(LX_HDMI_INFO_PACKET_T));
	HDMI_PRINT("[%d] %s \n", __L__, __F__);
...
int HDMI_H13_GetStatus(LX_HDMI_STATUS_T *status)
{
	...
	do {
		...
		status->eHotPlug = _gHDMIConnectState;
		...
		csc = (((gPrevAVIPacket.dataBytes[0] &0xff00)>>8) &0x60)>>5;
		if(_gPrevPixelEncoding == csc)		HDMI_DEBUG(" _gPrevPixelEncoding = %d , csc = %d \n", _gPrevPixelEncoding, csc);
		...
		HDMI_VIDEO("[HDMI] %d HDMI  State  = %d _gForce_thread_sleep= %d  _gHDMI_thread_running =%d \n",\
			__L__, _gH13HDMIState,_gForce_thread_sleep, _gHDMI_thread_running);
		HDMI_VIDEO("[HDMI]  AVI State[%d] VSI State[%d]  SRC Mute State[%d] \n",  _gIntr_avi, _gIntr_vsi, _gIntr_src_mute );
	} while (0);
...
int HDMI_H13_GetAudioInfo(LX_HDMI_AUDIO_INFO_T *pAudioInfo)
{
	...
	pAudioInfo->audioType	 = _gHdmiAudioInfo.audioType;
	pAudioInfo->samplingFreq = _gHdmiAudioInfo.samplingFreq;

	//Debug print
	if((_gHdmiAudioThreadCount % DEBUG_HDMI_AUDIO_MSG_PRINT_TIME_1S) == 0)
	{
		HDMI_AUDIO("HDMI_GetAudioInfo : type = %d, freq = %d, state = %d, mute = %d\n",	\
					pAudioInfo->audioType, pAudioInfo->samplingFreq, _gIntrAudioState, _gAudioMuteState);
	}
	else
	{
...
int HDMI_H13_SetMute(LX_HDMI_MUTE_CTRL_T *pMuteCtrl)
{
	...
	if(pMuteCtrl->eMode == LX_HDMI_AUDIO_MUTE || pMuteCtrl->eMode == LX_HDMI_AV_MUTE)
	{
		...
		if(_gAudioMuteState != pMuteCtrl->bAudioMute)
		{
			...
			//Update _gAudioMuteState
			_gAudioMuteState = pMuteCtrl->bAudioMute;
		...
		if(pMuteCtrl->bAudioMute == FALSE)
		{
			HDMI_DEBUG("[%d]SetMute : type = %d, freq = %d, mute = %d, PSC = %d\n", \
						__L__, _gHdmiAudioInfo.audioType, _gHdmiAudioInfo.samplingFreq, _gAudioMuteState, _gHdmiPortStableCount);
		}
	}
...
int HDMI_H13_GetAudioDebugInfo(LX_HDMI_DEBUG_AUDIO_INFO_T *pAudioDebugInfo)
{
	...
	HDMI_DEBUG("HDMI_H13_GetAudioDebugInfo : type = %d, freq = %d, state = %d, mute = %d, PSC = %d, video = %d\n", \
				_gHdmiAudioInfo.audioType, _gHdmiAudioInfo.samplingFreq, _gIntrAudioState, _gAudioMuteState, _gHdmiPortStableCount, _gH13HDMIState);
...
static int _HDMI_H13_GetExtendTimingInfo(LX_HDMI_TIMING_INFO_T *info)
{
	...
	do {
		...
		if(_gIntr_vsi < HDMI_VSI_INTERRUPT_STATE)
		{
			pTbl = &TBL_EXT_INFO[0];
	...
	if( ( _gHDMI_thread_running == 0 &&  _gTimingReadCnt < 23 && _gTimingReadCnt > 20) ||
		(_gH13HDMIState >= HDMI_STATE_READ && _gTimingReadCnt < 6 && _gTimingReadCnt > 3) )
	{
		//HDMI_VIDEO(" VIC  = %d [gpreAVI - %d] \n", videoIdCode, _gPrevAVIPacket.eAviPacket.VideoIdCode);
...
static int _HDMI_H13_SetInternalMute(LX_HDMI_MUTE_CTRL_T interMute)
{
	...
	{
		//Check a previous state
		if(_gAudioMuteState != interMute.bAudioMute)
		{
			...
			//Update _gAudioMuteState
			_gAudioMuteState = interMute.bAudioMute;
...
static int _HDMI_H13_GetPortConnection(void)
{
	...
	else
	{
		...
		{
			spin_lock_irqsave(&_gIntrHdmiH13VideoLock, flags);
			_gIntr_vf_stable = HDMI_VIDEO_INIT_STATE;
			_gIntr_avi = HDMI_AVI_INIT_STATE;
			_gIntr_vsi = HDMI_VSI_INIT_STATE;
			_gIntr_src_mute = HDMI_SOURCE_MUTE_CLEAR_STATE;
			_gTimingReadCnt = 0;
			_gAVIReadState 	  = FALSE;
			_gVSIState = FALSE;
			spin_unlock_irqrestore(&_gIntrHdmiH13VideoLock, flags);
...
static int _HDMI_H13_PhyRunReset(void)
{
	...
	//PHY_REG_H13_Wr01(eq_rs,eq_rs,0x5);		//2012-11-17 change 3 -> 5
...
*/static int _HDMI_H13_GetAudioInfo(void)
{
	...
	//Increase _gHdmiAudioThreadCount;
	_gHdmiAudioThreadCount++;
	...
	if( (intrAudioState == HDMI_AUDIO_STABLE_STATE && intrVideoState >= HDMI_STATE_READ)
	  ||(intrAudioState == HDMI_AUDIO_STABLE_STATE && _gHdmiPortStableCount > HDMI_AUDIO_PORT_STABLE_TIME_5S) )	//To protect a abnormal or transient video and audio info drop state.
#if 0
	  ||(_gTimingCnt >= HDMI_VIDEO_READ_STATE) )	//To protect a abnormal or transient video and audio info drop state.
#endif
	{
		//Check video active size from global variables.
#if 1
		if( (_gPrevTiming.vActive <  240) || (_gPrevTiming.hActive <  320) \
		  ||(_gPrevTiming.vActive > 2300) || (_gPrevTiming.hActive > 4096) \
		  ||(_gPrevTiming.vFreq > 850) )
#else
		if( ((_gPrevTiming.vActive <  240) || (_gPrevTiming.hActive <  320) \
		   ||(_gPrevTiming.vActive > 2300) || (_gPrevTiming.hActive > 4096) \
		   ||(_gPrevTiming.vFreq > 850))
...
#endif
		{
			HDMI_AUDIO("[%d]HDMI_GetAudioInfo : Video Active Size Error(v = %d, h = %d, vf = %d)!!!\n",	\
						__L__, _gPrevTiming.vActive, _gPrevTiming.hActive, _gPrevTiming.vFreq);
		...
		if((_gHdmiAudioThreadCount % HDMI_AUDIO_RECHECK_TIME_500MS) == 0)	// 0.5 seconds, Thread calls a every 20 ms.
		{
			//Check a current mute status for workaround
			if(_gAudioMuteState == TRUE)
			{
			...
			{
				HDMI_DEBUG("[%d]HDMI_GetAudioInfo : type = %d<-%d, tmds freq = %d<-%d, freq = %d, state = %d, mute = %d, PSC = %d\n", \
							__L__, audioType, _gHdmiAudioInfo.audioType, samplingFreqFromTmds, _gHdmiAudioInfo.samplingFreq,	\
							samplingFreq, _gIntrAudioState, _gAudioMuteState, _gHdmiPortStableCount);
		...
		//Increase _gHdmiPortStableCount(If HDMI port is changed, this count is cleared.)
		spin_lock_irqsave(&_gIntrHdmiH13AudioLock, flags);
		_gHdmiPortStableCount++;
		spin_unlock_irqrestore(&_gIntrHdmiH13AudioLock, flags);
		...
		if((_gHdmiAudioThreadCount % DEBUG_HDMI_AUDIO_MSG_PRINT_TIME_10S) == 0)
		{
			HDMI_AUDIO("[%d]HDMI_GetAudioInfo : type = %d, freq = %d, state = %d, PSC = %d. mute = %d\n",	\
						__L__, _gHdmiAudioInfo.audioType, _gHdmiAudioInfo.samplingFreq,	\
						_gIntrAudioState, _gHdmiPortStableCount, _gAudioMuteState);
		}
	...
	if(reg_hdmi_mode)
	{
		...
		if(audioType == LX_HDMI_AUDIO_PCM)
		{
			  ...
			  &&(samplingFreqFromTmds != LX_HDMI_SAMPLING_FREQ_44_1KHZ) )
			{
				...
				if(_gHdmiFreqErrorCount < HDMI_AUDIO_FREQ_ERROR_TIME_500MS)
				{
					//Increase _gHdmiFreqErrorCount.
					spin_lock_irqsave(&_gIntrHdmiH13AudioLock, flags);
					_gHdmiFreqErrorCount++;
					spin_unlock_irqrestore(&_gIntrHdmiH13AudioLock, flags);

					//Set a no audio mode
					audioType = LX_HDMI_AUDIO_NO_AUDIO;

					if((_gHdmiAudioThreadCount % DEBUG_HDMI_AUDIO_MSG_PRINT_TIME_100S) == 0)
					{
		...
		if((_gHdmiAudioThreadCount % DEBUG_HDMI_AUDIO_MSG_PRINT_TIME_100S) == 0)
		{
			HDMI_AUDIO("[%d]GetAudioInfo : type = %d, freq = %d, mute = %d, PSC = %d\n", \
						__L__, audioType, samplingFreq, _gAudioMuteState, _gHdmiPortStableCount);
		}
	}
	else
	{
	...
	if( (audioType != _gHdmiAudioInfo.audioType) || (samplingFreq != _gHdmiAudioInfo.samplingFreq) )
	{
		if( ((audioType == LX_HDMI_AUDIO_DVI) || (audioType == LX_HDMI_AUDIO_NO_AUDIO))	\
		  &&((_gHdmiAudioThreadCount % DEBUG_HDMI_AUDIO_MSG_PRINT_TIME_100S) == 0) )
		{
			//Set a debug print status
			bDebugEnabled = TRUE;
		}
	...
	if(bDebugEnabled == TRUE)
	{
		...
		HDMI_AUDIO("[%d]GetAudioInfo : type = %d(%d), freq = %d, mute = %d, PSC = %d\n", \
					__L__, audioType, reg_burst_pc_0 & 0x1F, samplingFreq, _gAudioMuteState, _gHdmiPortStableCount);
	}
...
func_exit:
	if(audioType == LX_HDMI_AUDIO_NO_AUDIO)
	{
		...
		_gHdmiAudioInfo.audioType	 = LX_HDMI_AUDIO_NO_AUDIO;
		_gHdmiAudioInfo.samplingFreq = LX_HDMI_SAMPLING_FREQ_NONE;

		//Clear _gHdmiPortStableCount
		_gHdmiPortStableCount = 0;
...
static int _HDMI_H13_GetAudioFreqFromTmdsClock(LX_HDMI_SAMPLING_FREQ_T *samplingFreqFromTmds)
{
	...
	{
		*samplingFreqFromTmds = LX_HDMI_SAMPLING_FREQ_48_KHZ;
		HDMI_PRINT("Get Fs from TDMS clock => LX_HDMI_SAMPLING_FREQ_NONE(48_KHZ)\n");
	}
...
static int _HDMI_H13_Thread(void *data)
{
	while(1)
	{
		...
		if(_gForce_thread_sleep > 1)
		{
			HDMI_PRINT("User force sleep HDMI Thread\n");
			_gForce_thread_sleep  = 1;
			_gHDMI_thread_running = 0;
			interruptible_sleep_on(&WaitQueue_HDMI);
...
static int _HDMI_H13_EnableInterrupt(void)
{
	...
	LINK_H13_Wr01(interrupt_enable_00, intr_burst_info_enable, 0x0);	///< 11 intr_burst_info_enable
...
irqreturn_t _HDMI_H13_IRQHandler(int irq, void *dev)
{
	...
	intra0 = LINK_H13_Rd(interrupt_00);		// reg_pkt_avi_hdr_0 (AVI Packet Version), reg_pkt_avi_hdr_1 (AVI Packet Length)

	LINK_H13_RdFL(interrupt_01);
	intra1 = LINK_H13_Rd(interrupt_01);		// reg_pkt_avi_hdr_0 (AVI Packet Version), reg_pkt_avi_hdr_1 (AVI Packet Length)
	...
	if((intra0 &0x100000)>>20)
	{
		...
		spin_lock_irqsave(&_gIntrHdmiH13VideoLock, flags);
		_gIntr_vf_stable = HDMI_VIDEO_INTERRUPT_STATE;
		spin_unlock_irqrestore(&_gIntrHdmiH13VideoLock, flags);
		...
		{
			HDMI_TASK("[%d] %s : -Changed -  _gIntr_vf_stable [STABLE_STATE] => [READ_STATE]     /   HDMI_STABLE[%d] / PSC[%d]\n", __L__, __F__, _gH13HDMIState, _gHdmiPortStableCount );
			spin_lock_irqsave(&_gIntrHdmiH13VideoLock, flags);
			_gIntr_vf_stable = HDMI_VIDEO_READ_STATE;
			...
			//Check a HDMI Port Stable Count
			if(_gHdmiPortStableCount > HDMI_AUDIO_PORT_STABLE_TIME_5S)
			{
				//spin lock for protection for audio
				spin_lock_irqsave(&_gIntrHdmiH13AudioLock, flags);
				_gIntrAudioState = HDMI_AUDIO_STABLE_STATE;
	...
	if((intra1 &0x20000)>>17) 	//	if(pstLinkReg->interrupt_01.intr_chg_avi_b12345)
	{
		...
		HDMI_TASK("[%d][HDMI_AVI] state [%d]  preCSS[%d] CSC = %d \n", __L__, _gH13HDMIState, _gPrevPixelEncoding, csc);

		if(_gH13HDMIState >= HDMI_STATE_READ)//(_gIntr_vf_stable == HDMI_VIDEO_STABLE_STATE) 			// for PS3 CSC
		{
			if(_gPrevPixelEncoding != csc)
			{
				//spin lock for protection
				spin_lock_irqsave(&_gIntrHdmiH13VideoLock, flags);
				_gIntr_avi = HDMI_AVI_CHANGE_STATE;
				_gTimingReadCnt = 0;
				_gAVIReadState = FALSE;
				spin_unlock_irqrestore(&_gIntrHdmiH13VideoLock, flags);

				HDMI_TASK("[%d] %s : -Changed  - _gIntr_avi [STABLE_STATE] => [CHANGE_STATE]     /   HDMI_STABLE[%d] \n", __L__, __F__,_gH13HDMIState );
	...
	if((intra1 &0x40)>>6) 	//	if(pstLinkReg->interrupt_01.intr_set_mute)
	{
		...
		//if(_gIntr_vf_stable > HDMI_VIDEO_INIT_STATE)
		{
			if(_gIntr_src_mute != HDMI_SOURCE_MUTE_STATE)
			{
				muteCtrl.eMode		= LX_HDMI_VIDEO_MUTE;
				muteCtrl.bVideoMute = TRUE;
				_HDMI_H13_SetInternalMute(muteCtrl);
				...
				spin_lock_irqsave(&_gIntrHdmiH13VideoLock, flags);
				_gIntr_src_mute = HDMI_SOURCE_MUTE_STATE;
				_gTimingReadCnt = 0;
				spin_unlock_irqrestore(&_gIntrHdmiH13VideoLock, flags);

				HDMI_DEBUG("[%d] intr_set_mute : HDMI_SOURCE MUTE Enable[%d] \n", __L__, _gIntr_src_mute );
			}
		}
	}
	...
	if (lx_chip_rev() >= LX_CHIP_REV(H13, B0))
	{
		if((intra1 &0x1000000)>>24) 	//	if(pstLinkReg->interrupt_01.intr_no_vsi)
		{
			...
			HDMI_TASK("[%d] %s : -Changed  - _gIntr_vsi [STABLE_STATE] => [NO_DATA_STAT]     /   HDMI_STABLE[%d] \n", __L__, __F__,_gH13HDMIState );
			HDMI_TASK("[%d] %s : -Changed  - _gIntr_vf_stable [STABLE_STATE] => [READ_STATE]     /   HDMI_STABLE[%d] \n", __L__, __F__,_gH13HDMIState );
			spin_lock_irqsave(&_gIntrHdmiH13VideoLock, flags);
			_gIntr_vsi = HDMI_VSI_NO_DATA_STATE;
			_gIntr_vf_stable = HDMI_VIDEO_READ_STATE;
			_gTimingReadCnt = 0;
			_gIntr_avi = HDMI_AVI_INTERRUPT_STATE;
			_gAVIReadState = FALSE;
	...
	if((intra1 &0x800000)>>23) 	//	if(pstLinkReg->interrupt_01.intr_chg_vsi_vformat)
	{
		...
		spin_lock_irqsave(&_gIntrHdmiH13VideoLock, flags);
		_gIntr_vsi = HDMI_VSI_INTERRUPT_STATE;
		_gVSIState = TRUE;
		spin_unlock_irqrestore(&_gIntrHdmiH13VideoLock, flags);
...
//audio related. => start
	...
	if(intra0 & HDMI_AUDIO_INTERRUPT_BIT_MASK)
	{
		//spin lock for protection for audio
		spin_lock_irqsave(&_gIntrHdmiH13AudioLock, flags);
		_gIntrAudioState = HDMI_AUDIO_INTERRUPT_STATE;
...
static void _HDMI_H13_Periodic_Task(void)
{
	...
	do {
		...
		if(_gHDMIConnectState == HDMI_PORT_NOT_CONNECTED)			// change check connection in the thread function by 20121206
		{
		...
		else
		{
			...
			if(_gIntr_vf_stable == HDMI_VIDEO_READ_STATE)
			{
				HDMI_TASK("[%d] %s : -Changed- HDMI_STATE_[%d]  => HDMI_STATE_READ && _gIntr_vf_stable[READ_STATE => INTERRUPT_STATE] \n", __L__, __F__, gPeriodicState);
		...
		switch(gPeriodicState)
		{
			...
			case HDMI_STATE_SCDT:		// if(HDMI_H13_SCDT_COUNT == 4)  5 call
				{
					if(gScdtCnt < HDMI_H13_SCDT_COUNT)	gScdtCnt++;
					else
					{
						gScdtCnt++;
						if(_gIntr_vf_stable == HDMI_VIDEO_INTERRUPT_STATE)
						{
							gPeriodicState = HDMI_STATE_CHECK_MODE;
						...
						if(gScdtCnt > 50)			// when no vf_stable Interrupt
						{
							...
							spin_lock_irqsave(&_gIntrHdmiH13VideoLock, flags);
							_gIntr_vf_stable = HDMI_VIDEO_INTERRUPT_STATE;
							if(_gIntr_avi == HDMI_AVI_INIT_STATE)		_gIntr_avi = HDMI_AVI_INTERRUPT_STATE;
							if(_gIntr_vsi == HDMI_VSI_INIT_STATE)		_gIntr_vsi = HDMI_VSI_INTERRUPT_STATE;
							spin_unlock_irqrestore(&_gIntrHdmiH13VideoLock, flags);
			...
			case HDMI_STATE_CHECK_AVI_NO:
				{
					...
					{
						gAviCnt ++;
						if(_gIntr_avi <= HDMI_AVI_INTERRUPT_STATE &&  gAviCnt < 50)		// No read AVI
						{
							LINK_H13_RdFL(packet_18);
					...
					if(gCheakCnt > HDMI_H13_CHEAK_COUNT)
					{
						gPeriodicState = HDMI_STATE_READ;
			...
			case HDMI_STATE_READ:
				{
					...
					else if(_gTimingReadCnt > HDMI_H13_READ_COUNT)
						gPeriodicState = HDMI_STATE_STABLE;

					HDMI_TASK("[%d] %s : HDMI State = [%d]  _gTimingReadCnt = %d \n", __L__, __F__, gPeriodicState, _gTimingReadCnt);
				}
				break;
			...
			case HDMI_STATE_STABLE:
				{
					spin_lock_irqsave(&_gIntrHdmiH13VideoLock, flags);
					_gIntr_vf_stable = HDMI_VIDEO_STABLE_STATE;
					spin_unlock_irqrestore(&_gIntrHdmiH13VideoLock, flags);
				}
				break;
