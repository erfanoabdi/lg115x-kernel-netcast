...
UINT8 *ui8HDMIPrintType[9]  = { "HDMI_PRINT", "HDMI_TRACE", "HDMI_DEBUG", "HDMI_ERROR",\
							   "AUDIO_ERROR", "AUDIO_DEBUG", "HDMI_VIDEO", "HDMI_TASK", "HDMI_INTR"};
...
int HDMI_Init(void)
{
	...
	OS_DEBUG_DisableModuleByIndex ( g_hdmi_debug_fd, 3);				    //HDMI_DEBUG
	OS_DEBUG_DisableModuleByIndex ( g_hdmi_debug_fd, 4);    				//HDMI_AUDIO
	OS_DEBUG_DisableModuleByIndex ( g_hdmi_debug_fd, 5);       				//HDMI_ATASK
	OS_DEBUG_DisableModuleByIndex ( g_hdmi_debug_fd, 6);    				//HDMI_VIDEO
	OS_DEBUG_DisableModuleByIndex ( g_hdmi_debug_fd, 7);					//HDMI_TASK
	OS_DEBUG_DisableModuleByIndex ( g_hdmi_debug_fd, 8);					//HDMI_INTR
...
HDMI_Open(struct inode *inode, struct file *filp)
{
	...
	HDMI_PRINT("hdmi device opened (%d:%d)\n", major, minor);
...
static int HDMI_Close(struct inode *inode, struct file *file)
{
    ...
    major = imajor(inode);
    minor = iminor(inode);
	HDMI_PRINT("hdmi device closed (%d:%d)\n", major, minor);
    return 0;
}
...
static long HDMI_Ioctl ( struct file *filp, unsigned int cmd, unsigned long arg )
{
	...
	switch(cmd)
	{
		...
		case HDMI_IOW_SET_UD_3D_MODE:
		{
			...
			HDMI_TRACE("[hdmi_drv]HDMI_IOW_SET_3D_N_UD\n");
		...
		case HDMI_IOR_GET_HDMI_AVI:
		{
			...
			ret = HDMI_GetAviPacket_L9(avi_packet);
			ret = copy_to_user((void *)arg, (void *)avi_packet, sizeof(LX_HDMI_AVI_PACKET_T));

			kfree(avi_packet);
			HDMI_TRACE("[hdmi_drv] HDMI_IOR_GET_HDMI_AVI IOCTL\n");
		}
		break;
		...
		case HDMI_IOR_GET_HDMI_VSI:
		{
			...
			HDMI_TRACE("[hdmi_drv]HDMI_IOR_GET_HDMI_VSI IOCTL\n");

			vsi_packet = (LX_HDMI_VSI_PACKET_T *)kmalloc(sizeof(LX_HDMI_VSI_PACKET_T) , GFP_KERNEL);

			ret = HDMI_GetVsiPacket_L9(vsi_packet);
		...
		case HDMI_IOR_GET_HDMI_SPD:
		{
			...
			ret = HDMI_GetSpdPacket_L9(spd_packet);
			ret = copy_to_user((void *)arg, (void *)spd_packet, sizeof(LX_HDMI_SPD_PACKET_T));

			kfree(spd_packet);
			HDMI_TRACE("[hdmi_drv]HDMI_IOR_GET_HDMI_SPD\n");
		}
		break;
 		...
 		case HDMI_IOW_DEBUG_PRINT_CTRL:
		{
			...
			{
				HDMI_ERROR("HDMI_PRINT_TYPE[%d] is enabled Color is [%d].\n", i_stDebugCtrl.printType, i_stDebugCtrl.printColor);
