 ...
 * 하나의 디렉토리로 나타나며, 그 디렉토리 안의 status 파일의 내용은 그 pool
 * 에서 할당된 메모리 상태를 나타낸다.
 *
 * /proc/hma 디렉토리 아래에 pool 이름으로 생기는 디렉토리에는 해당 pool에
 * 할당된 chunk에 따른 chunk 주소 이름의 파일이 생성되며, 해당 파일을
 * 읽음으로써 그 memory chunk 메모리 내용을 읽을 수 있다. 이 기능을 이용해 dd,
 * cat 프로그램을 이용해 해당 메모리 내용을 dump 할수 있다.
 *
 ...
 * hma_pool_free() 등의 함수로 memory pool을 관리하며, hma_alloc_user(),
...
static struct hma_chunk *search_best_match (struct hma_pool *pool, int size, int align)
{
		...
		int size, int align, const char *user)
{
	...
	best = search_best_match (pool, size, align);
	if (best == NULL)
	{
		error ("no best match\n");
		goto nomem;
		...
		int size, int align, const char *user)
{
	...
	if (poolname == NULL || *poolname == 0)
	{
		...
		while (pool->next != NULL)
		{
			...
			chunk = _hma_alloc (pool, size, align, user);
			if (chunk != NULL)
				goto allocated;
		}
	}
		...
		int size, int align)
{
	...
	return hma_alloc_user (poolname, size, align, comm);
}
 ...
 *
 * hma_alloc() 함수에 의해 할당받은 메모리를 반환한다. 반환 하고자 하는
 * 메모리의 memory pool name을 모르고 있다면 \a poolname 을 NULL로 할 수 있다.
 *
 * multithread에 안전하다.
 *
 * \param poolname	memory pool name
...
static char *pool_name;
static unsigned long pool_addr;
static unsigned int pool_size;

module_param (pool_name, charp, 0440);
module_param (pool_addr, ulong, 0440);
module_param (pool_size, uint,  0440);

int hma_init (void)
{
	hma_proc_entry = proc_mkdir ("hma", NULL);

	if (pool_name && pool_addr && pool_size)
	{
		debug ("register %s memory pool on 0x%08lx(0x%08x)\n",
				pool_name, pool_addr, pool_size);
		hma_pool_register (pool_name, pool_addr, pool_size);
	}
