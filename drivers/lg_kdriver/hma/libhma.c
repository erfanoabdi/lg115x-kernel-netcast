...
static inline int __libhma_cache_ctl(struct mem_desc *desc, size_t offset, size_t size, int ctl);
...
int libhma_memcpy_tocpu(void *vaddr_des, void *vaddr_src, void *paddr_src, unsigned int size)
{
    ...
    ret = __libhma_cache_ctl(&desc, 0, size, CACHE_INV);
...
/**
 * \ingroup libhma
 * \brief   HMA memory pool에서 paddr 물리주소를 가지는 mem_desc를 찾는다
 *          paddr : libhma_alloc_cached 으로 메모리를 할당 받았을 때 return 된 값. 해당 메모리의 키값으로 사용됨.
 *          현재는 일반 list에서 순차 검색하는 방법이다. 만약 mem_desc item의 수가 많아서
 *          검색 시간이 오래 걸리면 개선된 방법이 필요함.
 */
static struct mem_desc* libhma_search_mem(unsigned long paddr)
{
    struct mem_desc *desc;
 ...
 *          size :  컨트롤 하려는 크기
 *          ctl : cache invalidate(CACHE_INV), cache clean(CACHE_CLEAN)
 */
