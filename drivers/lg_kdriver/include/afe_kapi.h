...
Controls VDAC Mute using vdac swrst (not using VDAC PDB)
...
typedef struct
{
	...
	UINT16	hfcnt_value;		///< HF cnt_value;
	UINT16	lfcnt_value;		///< LF cnt_value;
	...
	UINT32	pattern_found;			/// < Test Pattern found???
...
typedef enum
{
	CVD_BYPASS_DAC,						///< Select DAC output (DTV out) for bypass
	CVD_BYPASS_CVBS_WITH_CLAMPING,		///< Select CVBS(selected by buf_ycm) for bypass control, and enable clamping to input cvbs.
	CVD_BYPASS_CVBS_WITH_CLAMPING_AV,	///< Select CVBS(selected by buf_ycm) for bypass control, and enable clamping to input cvbs & Enable CVBS Clamping Control(in av input mode)
	CVD_BYPASS_CVBS_WITHOUT_CLAMPING,	///< Select CVBS(selected by buf_ycm) for bypass control, and disable clamping to input cvbs.
