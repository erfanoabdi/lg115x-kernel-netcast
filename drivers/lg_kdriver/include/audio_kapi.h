...
typedef  enum
{
	LX_AUD_EVENT_MASK_NONE		= 0x00,		///< None
	LX_AUD_EVENT_DSP_RESET		= 0x01,		///< Event => DECx, ENCx, CAPx : DSP Reset Event, Notify to app. after DSP Reset(Request playing again)
	LX_AUD_EVENT_PRESENT_END	= 0x02,		///< Event => RENx : Render Present End Event, Notify to app. after Render is completed.
	LX_AUD_EVENT_UNDERFLOW		= 0x04,		///< Event => RENx : Underflow Event, Notify to app. after Render is in underflow.
	LX_AUD_EVENT_DEC_ERROR		= 0x08,		///< Event => DECx : Decoder Error Event, Notify to app. after Decoder is in error.
	LX_AUD_EVENT_DEC_AAC51_DUAL	= 0x10,		///< Event => DECx : Decoder AAC 5.1CH Event, Notify to app. when dual decoding is in error.(DSP Performance Issue)
	LX_AUD_EVENT_DEC_AAC_PCM	= 0x20,		///< Event => RENx : Render AAC SPDIF PCM Event, Notify to app. when decoder is not available for SPDIF ES output.(Not 48Khz.)
	LX_AUD_EVENT_DEC_AAC_CODEC	= 0x40,		///< Event => DECx : Decoder AAC/HEAAC Codec Event, Notify to app. when decoded TP stream is AAC codec detected..
	LX_AUD_EVENT_SB_CHANGED		= 0x80,		///< Event => RENx : Render Wireless Sound Bar Event, Notify to app. when wireless sound bar is enabled or not for changing Lip Sync Offset.
	LX_AUD_EVENT_MASK_ALL		= 0xFF,		///< Event => All event.
} LX_AUD_EVENT_MSG_T;
...
typedef enum {
	...
	LX_AUD_CODEC_AAC_BYPASS,		///< 16
#ifdef IC_CERTIFICATION
	LX_AUD_CODEC_DTS_ENC,			///< 17
#endif
...
typedef struct LX_AUD_EVENT_TYPE{
	LX_AUD_ALLOC_DEV_T		allocDev;	///< The allocated device.(Master device is not supported now.)
