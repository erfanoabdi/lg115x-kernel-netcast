...
/*----------------------------------------------------------------------------------------
	...
	module id (idx) is used for query something from the integrated data
----------------------------------------------------------------------------------------*/
typedef enum
{
	LX_DEV_AAD	= 0,/* AAD device		*/
	LX_DEV_ADEC,	/* ADEC device		*/
	LX_DEV_AFE,		/* AFE device		*/
	LX_DEV_CI,		/* CI device		*/
	LX_DEV_DE,		/* DE(Display Engine) device */
	LX_DEV_DEMOD,	/* DEMOD device		*/
	LX_DEV_DENC,	/* DENC device		*/
	LX_DEV_DIMM,	
	LX_DEV_FBDEV,	/* FBDEV device		*/
	LX_DEV_FMT3D,
	LX_DEV_FRC,		/* (internal) FRC device */
	LX_DEV_GFX,		/* 2D GFX device	*/
	LX_DEV_GPU,		/* 3D GPU device	*/
	LX_DEV_GPIO,	/* GPIO device		*/
	LX_DEV_HDMI,	/* HDMI device		*/
	LX_DEV_I2C,		/* I2C device		*/
	LX_DEV_PNG,		/* PNG device		*/
	LX_DEV_PE,		/* PE(Picture Engine) device */
	LX_DEV_PVR,		/* PVR device		*/
	LX_DEV_SDEC,	/* SDEC device		*/
	LX_DEV_SE,		/* SE device		*/
	LX_DEV_TTX,		/* TTX device		*/
	LX_DEV_VDEC,	/* VDEC device		*/
	LX_DEV_VENC,	/* VENC device		*/
	LX_DEV_SYS,		/* SYS device		*/
	LX_DEV_CLK,		/* CLK device		*/
	
	LX_DEV_MON,		/* monitor device	*/
}
LX_DEV_T;
...
#if 0
#ifdef	__KERNEL__
#define LX_MEM_ALLOC(size)				OS_Malloc( 1, size )
#define LX_MEM_FREE(ptr)				do { if((ptr)) { OS_Free(ptr); ptr=NULL; } } while(0)
#else
#define LX_MEM_ALLOC(size)				calloc( 1, size )
#define LX_MEM_FREE(ptr)				do { if((ptr)) { free(ptr); ptr=NULL; } } while(0)
#endif
#endif
