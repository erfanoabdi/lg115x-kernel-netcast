...
----------------------------------------------------------------------------------------*/
#define	LX_FBDEV_FRC_SCREEN_WIDTH	1920
#define	LX_FBDEV_FRC_SCREEN_HEIGHT	(1080+1)
...
#define FBDEV_IOW_FLIP_FB_SCREEN		_IOW( FBDEV_IOC_MAGIC, FBDEV_IOC_IDX(29), LX_FBDEV_FB_SCREEN_T )		
#define FBDEV_IOR_GET_FB_STATUS			_IOR( FBDEV_IOC_MAGIC, FBDEV_IOC_IDX(29), LX_FBDEV_FB_STATUS_T )
...
#define FBDEV_IOW_SET_OUT_PATH_CTRL		_IOW( FBDEV_IOC_MAGIC, FBDEV_IOC_IDX(44), LX_FBDEV_OUT_PATH_CTRL_T )

#define FBDEV_IOW_SET_FREEZE_CTRL		_IOW( FBDEV_IOC_MAGIC, FBDEV_IOC_IDX(45), LX_FBDEV_FREEZE_CTRL_T )

#define FBDEV_IOW_SET_BACK_IMAGE		_IOW( FBDEV_IOC_MAGIC, FBDEV_IOC_IDX(50), LX_FBDEV_BACK_IMAGE_T )
...
typedef enum
{
	LX_FBDEV_OUT_PATH_CTRL_BLEND		= 0,	///< OSD is blended width VIDEO ( it's normal and standard connection in almost devices )
	LX_FBDEV_OUT_PATH_CTRL_SEP			= 1,	///< OSD has its own path and not mixed with VIDEO ( it's standard connection in L9 )
}
LX_FBDEV_OUT_PATH_CTRL_T;

/** global output connnection
 *
 *	@note global conection controls each OSD output path
 */
typedef struct
{
...
#define	LX_FBDEV_FREEZE_CTRL_MASK_NONE				0x0000		/* screen update */
#define	LX_FBDEV_FREEZE_CTRL_MASK_PAN_DISPLAY		0x0001		/* screen update */
	UINT16				ctrl_mask;
}
...
typedef struct
{
	UINT8				b_enable;		/** BOOLEAN. enable/disable back image */	
	LX_MEM_BUF_T		img_phys_mem;	/** physical memory info. image shoule be 1920x1080x32bpp */
}
LX_FBDEV_BACK_IMAGE_T;
...
//	UINT8				pixel_format; 	/**< pixel format 	*/
