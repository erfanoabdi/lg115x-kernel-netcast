...
int Doumi_Init_I2C_Config(void)
{
	...
	_i2c_handle_hdmi[0] = I2C_DevOpenPriv(i2c_ch, 1); // 0:external, 1:internal
...
int Doumi_I2C_Read(UINT32 addr , UINT32 *value)
{
	...
	if(rc < 0)
	{
		printk("\n Internal i2c read failed.");
	}	

	*value = (UINT32)data;		

	//printk("\n idx:%d,slave:0x%2x,regL:0x%02x,regH:0x%02x\n",index,slave_addr,reg_addrL,reg_addrH);
...
int Doumi_I2C_Write(UINT32 addr , UINT32 value)
{
	...
	if(rc < 0)
	{
		printk("Internal i2c write failed");
	}
