...
static void PE_SHP_H13_SetSclFltRnwMode(UINT32 rnw_mode);
static UINT32 PE_SHP_H13_RdLumaSclFltCoeff(LX_PE_WIN_ID win_id, UINT32 config, UINT32 *data);
static UINT32 PE_SHP_H13_RdChromaSclFltCoeff(LX_PE_WIN_ID win_id, UINT32 config, UINT32 *data);
static UINT32 PE_SHP_H13_RdSubSclFltCoeff(UINT32 config, UINT32 *data);
 ...
 * @author
 */
void PE_SHP_HW_H13_SetDbgPrint(UINT32 on_off)
{
	_g_shp_hw_h13_trace = on_off? 1:0;
	return;
}
...
static PE_SHP_HW_H13B_VFC_FMT PE_SHP_HW_H13B_ConvDispInfoToVfcFmt(LX_PE_INF_DISPLAY_T *disp_inf)
{
	...
	switch(disp_inf->src_type)
	{
		case LX_PE_SRC_ATV:
			...
			else if(disp_inf->fmt_type==LX_PE_FMT_PAL)
			{
		...
		if(PE_KDRV_VER_H13)
		{
				...
				pInfo->c_fir[LX_PE_WIN_0]!=cur_param.c_fir[LX_PE_WIN_0])
			{
				...
				if(pInfo->h_y_tap[LX_PE_WIN_0]!=cur_param.h_y_tap[LX_PE_WIN_0] || \
					...
					pInfo->c_fir[LX_PE_WIN_0]!=cur_param.c_fir[LX_PE_WIN_0])
				{
					scaler_config	= (cur_param.h_y_tap[LX_PE_WIN_0]==PE_SHP_HW_H13_SCL_V12T)? 0x0:0x1;
					adap_filter_en	= (cur_param.h_y_tap[LX_PE_WIN_0]==PE_SHP_HW_H13_SCL_V12T)? 0x1:0x0;
				...
				pInfo->c_fir[LX_PE_WIN_1]!=cur_param.c_fir[LX_PE_WIN_1])
			{
				...
				if(pInfo->h_y_tap[LX_PE_WIN_1]!=cur_param.h_y_tap[LX_PE_WIN_1] || \
					...
					pInfo->c_fir[LX_PE_WIN_1]!=cur_param.c_fir[LX_PE_WIN_1])
				{
					adap_filter_en	= (cur_param.h_y_tap[LX_PE_WIN_1]==PE_SHP_HW_H13_SCL_V12T)? 0x1:0x0;
...
static int PE_SHP_HW_H13_GetScalerFilterValue(PE_TSK_H13_CFG_T *pCfg,PE_SHP_HW_H13_SETTINGS_T *pstParams)
{
	...
	do{
		...
		{
			pstParams->h_y_tap[LX_PE_WIN_0] = PE_SHP_HW_H13_SCL_V12T;
			pstParams->v_y_tap[LX_PE_WIN_0] = PE_SHP_HW_H13_SCL_V12T;
		}
		/* R : check y tap */
...
static void PE_SHP_HW_H13_CheckValidScalerFilterValue(PE_SHP_HW_H13_SETTINGS_T *pstParams)
{
	...
	do{
		...
		{
			/* L : h luma id check */
			PE_SHP_HW_H13_DBG_CHECK_CODE(count,(hy_tap0==PE_SHP_HW_H13_SCL_V12T&&\
				idx0->h_luma_id>=PE_SHP_SCL_HW_H13_Y_H_12T_COEF_NUM),\
			...
			PE_SHP_HW_H13_DBG_CHECK_CODE(count,(hy_tap0==PE_SHP_HW_H13_SCL_V06T_NORM&&\
				idx0->h_luma_id>=PE_SHP_SCL_HW_H13_Y_H_08T_COEF_NUM),\
				idx0->h_luma_id=PE_SHP_HW_H13_IDX_Y_DFLT,\
				"[%s,%d] hy_tap0(%d), h_luma_id(%d) is out of range.\n",\
				__F__,__L__,hy_tap0,idx0->h_luma_id);
			/* L : v luma id check */
			PE_SHP_HW_H13_DBG_CHECK_CODE(count,(vy_tap0==PE_SHP_HW_H13_SCL_V12T&&\
				idx0->v_luma_id>=PE_SHP_SCL_HW_H13_Y_V_12T_COEF_NUM),\
			...
			PE_SHP_HW_H13_DBG_CHECK_CODE(count,((vy_tap0==PE_SHP_HW_H13_SCL_V06T_NORM||\
				...
				idx0->v_luma_id>=PE_SHP_SCL_HW_H13_Y_V_06T_COEF_NUM),\
				idx0->v_luma_id=PE_SHP_HW_H13_IDX_Y_DFLT,\
				"[%s,%d] vy_tap0(%d), v_luma_id(%d) is out of range.\n",\
				__F__,__L__,vy_tap0,idx0->v_luma_id);
			/* L : h chroma id check */
			PE_SHP_HW_H13_DBG_CHECK_CODE(count,((hc_tap0==PE_SHP_HW_H13_SCL_V12T||\
				hc_tap0==PE_SHP_HW_H13_SCL_V06T_NORM)&&\
				idx0->h_chrm_id>=PE_SHP_SCL_HW_H13_C_08T_COEF_NUM),\
				idx0->h_chrm_id=PE_SHP_HW_H13_IDX_C_DFLT,\
				"[%s,%d] hc_tap0(%d), h_chrm_id(%d) is out of range.\n",\
				__F__,__L__,hc_tap0,idx0->h_chrm_id);
			/* L : v chroma id check */
			PE_SHP_HW_H13_DBG_CHECK_CODE(count,((vc_tap0==PE_SHP_HW_H13_SCL_V12T||\
				vc_tap0==PE_SHP_HW_H13_SCL_V06T_NORM||\
				vc_tap0==PE_SHP_HW_H13_SCL_V06T_HALF)&&\
				idx0->v_chrm_id>=PE_SHP_SCL_HW_H13_C_04T_COEF_NUM),\
				idx0->v_chrm_id=PE_SHP_HW_H13_IDX_C_DFLT,\
				"[%s,%d] vc_tap0(%d), v_chrm_id(%d) is out of range.\n",\
				__F__,__L__,vc_tap0,idx0->v_chrm_id);
			/* R : h luma id check */
			PE_SHP_HW_H13_DBG_CHECK_CODE(count,(hy_tap1==PE_SHP_HW_H13_SCL_V12T),\
			...
			PE_SHP_HW_H13_DBG_CHECK_CODE(count,(hy_tap1==PE_SHP_HW_H13_SCL_V06T_NORM&&\
				idx1->h_luma_id>=PE_SHP_SCL_HW_H13_Y_H_08T_COEF_NUM),\
				idx1->h_luma_id=PE_SHP_HW_H13_IDX_Y_DFLT,\
				"[%s,%d] hy_tap1(%d), h_luma_id(%d) is out of range.\n",\
				__F__,__L__,hy_tap1,idx1->h_luma_id);
			/* R : v luma id check */
			PE_SHP_HW_H13_DBG_CHECK_CODE(count,(vy_tap1==PE_SHP_HW_H13_SCL_V12T),\
			...
			PE_SHP_HW_H13_DBG_CHECK_CODE(count,((vy_tap1==PE_SHP_HW_H13_SCL_V06T_NORM||\
				...
				idx1->v_luma_id>=PE_SHP_SCL_HW_H13_Y_V_06T_COEF_NUM),\
				idx1->v_luma_id=PE_SHP_HW_H13_IDX_Y_DFLT,\
				"[%s,%d] vy_tap1(%d), v_luma_id(%d) is out of range.\n",\
				__F__,__L__,vy_tap1,idx1->v_luma_id);
			/* R : h chroma id check */
			PE_SHP_HW_H13_DBG_CHECK_CODE(count,(hc_tap1==PE_SHP_HW_H13_SCL_V12T),\
			...
			PE_SHP_HW_H13_DBG_CHECK_CODE(count,(hc_tap1==PE_SHP_HW_H13_SCL_V06T_NORM&&\
				idx1->h_chrm_id>=PE_SHP_SCL_HW_H13_C_08T_COEF_NUM),\
				idx1->h_chrm_id=PE_SHP_HW_H13_IDX_C_DFLT,\
				"[%s,%d] hc_tap0(%d), h_chrm_id(%d) is out of range.\n",\
				__F__,__L__,hc_tap1,idx1->h_chrm_id);
			/* R : v chroma id check */
			PE_SHP_HW_H13_DBG_CHECK_CODE(count,(vc_tap1==PE_SHP_HW_H13_SCL_V12T),\
			...
			PE_SHP_HW_H13_DBG_CHECK_CODE(count,((vc_tap1==PE_SHP_HW_H13_SCL_V06T_NORM||\
				...
				idx1->v_chrm_id>=PE_SHP_SCL_HW_H13_C_04T_COEF_NUM),\
...
static PE_SHP_HW_H13_FLAG_T PE_SHP_HW_H13_GetDoubleSetFlag(LX_PE_WIN_ID win_id,LX_PE_SHP_SCLFILTER_T pre_idx,PE_SHP_HW_H13_SETTINGS_T *cur_param)
{
	...
	else if((cur_param->h_y_tap[win_id]==PE_SHP_HW_H13_SCL_V12T) ||
	...
	else if((cur_param->h_c_tap[win_id]==PE_SHP_HW_H13_SCL_V12T) ||
	...
	else if((cur_param->v_y_tap[win_id]==PE_SHP_HW_H13_SCL_V12T) ||
	...
	else if((cur_param->v_c_tap[win_id]==PE_SHP_HW_H13_SCL_V12T) ||
...
static int PE_SHP_HW_H13_SetSclCoeff(LX_PE_WIN_ID win_id, PE_SHP_HW_H13_FLAG_T flag, PE_SHP_HW_H13_SETTINGS_T *pstParams)
{
	...
	if(flag.h_y)
	{
		wr_ctrl.tbl_size = (h_y_tap==PE_SHP_HW_H13_SCL_V12T)? \
			PE_SHP_SCL_HW_H13_Y_H_12T_NUM:PE_SHP_SCL_HW_H13_Y_H_08T_NUM;
		wr_ctrl.tap_type = (h_y_tap==PE_SHP_HW_H13_SCL_V12T)? \
			PE_SHP_HW_H13_SCL_WR_TAP_V12TM_Y_H_12T:PE_SHP_HW_H13_SCL_WR_TAP_V06TM_Y_H_08T;
		switch(h_y_tap)
		{
			case PE_SHP_HW_H13_SCL_V12T:
				PE_SHP_HW_H13_DBG_PRINT("set shp_scl_y_h_12tap_coeff_h13[%d]\n",h_y_idx);
				PE_SHP_HW_H13_WrSclCoeff(wr_ctrl,&(shp_scl_y_h_12tap_coeff_h13[h_y_idx][0]));
				break;
			case PE_SHP_HW_H13_SCL_V06T_DN75:
				PE_SHP_HW_H13_DBG_PRINT("set shp_scl_y_h_08tap_dn75_coeff_h13\n");
				PE_SHP_HW_H13_WrSclCoeff(wr_ctrl,&(shp_scl_y_h_08tap_dn75_coeff_h13[0]));
				break;
			case PE_SHP_HW_H13_SCL_V06T_DN50:
				PE_SHP_HW_H13_DBG_PRINT("set shp_scl_y_h_08tap_dn50_coeff_h13\n");
				PE_SHP_HW_H13_WrSclCoeff(wr_ctrl,&(shp_scl_y_h_08tap_dn50_coeff_h13[0]));
				break;
			case PE_SHP_HW_H13_SCL_V06T_DN25:
				PE_SHP_HW_H13_DBG_PRINT("set shp_scl_y_h_08tap_dn25_coeff_h13\n");
				PE_SHP_HW_H13_WrSclCoeff(wr_ctrl,&(shp_scl_y_h_08tap_dn25_coeff_h13[0]));
				break;
			case PE_SHP_HW_H13_SCL_V06T_NORM:
			case PE_SHP_HW_H13_SCL_V06T_HALF:
			default:
				PE_SHP_HW_H13_DBG_PRINT("set shp_scl_y_h_08tap_coeff_h13[%d]\n",h_y_idx);
				PE_SHP_HW_H13_WrSclCoeff(wr_ctrl,&(shp_scl_y_h_08tap_coeff_h13[h_y_idx][0]));
				break;
		}
	}
	if(flag.h_c)
	{
		wr_ctrl.tbl_size = PE_SHP_SCL_HW_H13_C_08T_NUM;
		wr_ctrl.tap_type = (h_c_tap==PE_SHP_HW_H13_SCL_V12T)? \
			PE_SHP_HW_H13_SCL_WR_TAP_V12TM_C_H_08T:PE_SHP_HW_H13_SCL_WR_TAP_V06TM_C_H_08T;
		switch(h_c_tap)
		{
			case PE_SHP_HW_H13_SCL_V12T:
				PE_SHP_HW_H13_DBG_PRINT("set shp_scl_c_08tap_coeff_h13[%d]\n",h_c_idx);
				PE_SHP_HW_H13_WrSclCoeff(wr_ctrl,&(shp_scl_c_08tap_coeff_h13[h_c_idx][0]));
				#ifndef PE_SHP_HW_H13_USE_V12TM_C08T_SEPARATE_HV
				flag.v_c = 0;	// avoid double setting
				#endif
				break;
			case PE_SHP_HW_H13_SCL_V06T_DN75:
				PE_SHP_HW_H13_DBG_PRINT("set shp_scl_c_08tap_dn75_coeff_h13\n");
				PE_SHP_HW_H13_WrSclCoeff(wr_ctrl,&(shp_scl_c_08tap_dn75_coeff_h13[0]));
				break;
			case PE_SHP_HW_H13_SCL_V06T_DN50:
				PE_SHP_HW_H13_DBG_PRINT("set shp_scl_c_08tap_dn50_coeff_h13\n");
				PE_SHP_HW_H13_WrSclCoeff(wr_ctrl,&(shp_scl_c_08tap_dn50_coeff_h13[0]));
				break;
			case PE_SHP_HW_H13_SCL_V06T_DN25:
				PE_SHP_HW_H13_DBG_PRINT("set shp_scl_c_08tap_dn25_coeff_h13\n");
				PE_SHP_HW_H13_WrSclCoeff(wr_ctrl,&(shp_scl_c_08tap_dn25_coeff_h13[0]));
				break;
			case PE_SHP_HW_H13_SCL_V06T_NORM:
			case PE_SHP_HW_H13_SCL_V06T_HALF:
			default:
	...
	if(flag.v_y)
	{
		wr_ctrl.tbl_size = (v_y_tap==PE_SHP_HW_H13_SCL_V12T)? \
			PE_SHP_SCL_HW_H13_Y_V_12T_NUM:PE_SHP_SCL_HW_H13_Y_V_06T_NUM;
		wr_ctrl.tap_type = (v_y_tap==PE_SHP_HW_H13_SCL_V12T)? \
			PE_SHP_HW_H13_SCL_WR_TAP_V12TM_Y_V_12T:PE_SHP_HW_H13_SCL_WR_TAP_V06TM_Y_V_06T;
		switch(v_y_tap)
		{
			case PE_SHP_HW_H13_SCL_V12T:
				PE_SHP_HW_H13_DBG_PRINT("set shp_scl_y_v_12tap_coeff_h13[%d]\n",v_y_idx);
				PE_SHP_HW_H13_WrSclCoeff(wr_ctrl,&(shp_scl_y_v_12tap_coeff_h13[v_y_idx][0]));
				break;
			case PE_SHP_HW_H13_SCL_V06T_HALF:
				PE_SHP_HW_H13_DBG_PRINT("set shp_scl_y_v_06tap_half_coeff_h13[%d]\n",v_y_idx);
				PE_SHP_HW_H13_WrSclCoeff(wr_ctrl,&(shp_scl_y_v_06tap_half_coeff_h13[v_y_idx][0]));
				break;
			case PE_SHP_HW_H13_SCL_V06T_DN75:
				PE_SHP_HW_H13_DBG_PRINT("set shp_scl_y_v_06tap_dn75_coeff_h13\n");
				PE_SHP_HW_H13_WrSclCoeff(wr_ctrl,&(shp_scl_y_v_06tap_dn75_coeff_h13[0]));
				break;
			case PE_SHP_HW_H13_SCL_V06T_DN50:
				PE_SHP_HW_H13_DBG_PRINT("set shp_scl_y_v_06tap_dn50_coeff_h13\n");
				PE_SHP_HW_H13_WrSclCoeff(wr_ctrl,&(shp_scl_y_v_06tap_dn50_coeff_h13[0]));
				break;
			case PE_SHP_HW_H13_SCL_V06T_DN25:
				PE_SHP_HW_H13_DBG_PRINT("set shp_scl_y_v_06tap_dn25_coeff_h13\n");
				PE_SHP_HW_H13_WrSclCoeff(wr_ctrl,&(shp_scl_y_v_06tap_dn25_coeff_h13[0]));
			break;
			case PE_SHP_HW_H13_SCL_V06T_NORM:
			default:
				PE_SHP_HW_H13_DBG_PRINT("set shp_scl_y_v_06tap_coeff_h13[%d]\n",v_y_idx);
				PE_SHP_HW_H13_WrSclCoeff(wr_ctrl,&(shp_scl_y_v_06tap_coeff_h13[v_y_idx][0]));
				break;
		}
	}
	if(flag.v_c)
	{
		wr_ctrl.tbl_size = (v_c_tap==PE_SHP_HW_H13_SCL_V12T)? \
			PE_SHP_SCL_HW_H13_C_08T_NUM:PE_SHP_SCL_HW_H13_C_04T_NUM;
		wr_ctrl.tap_type = (v_y_tap==PE_SHP_HW_H13_SCL_V12T)? \
		...
		switch(v_c_tap)
		{
			case PE_SHP_HW_H13_SCL_V12T:
				PE_SHP_HW_H13_DBG_PRINT("set shp_scl_c_08tap_coeff_h13[%d]\n",v_c_idx);
				PE_SHP_HW_H13_WrSclCoeff(wr_ctrl,&(shp_scl_c_08tap_coeff_h13[v_c_idx][0]));
				break;
			case PE_SHP_HW_H13_SCL_V06T_DN75:
				PE_SHP_HW_H13_DBG_PRINT("set shp_scl_c_04tap_dn75_coeff_h13\n");
				PE_SHP_HW_H13_WrSclCoeff(wr_ctrl,&(shp_scl_c_04tap_dn75_coeff_h13[0]));
				break;
			case PE_SHP_HW_H13_SCL_V06T_DN50:
				PE_SHP_HW_H13_DBG_PRINT("set shp_scl_c_04tap_dn50_coeff_h13\n");
				PE_SHP_HW_H13_WrSclCoeff(wr_ctrl,&(shp_scl_c_04tap_dn50_coeff_h13[0]));
				break;
			case PE_SHP_HW_H13_SCL_V06T_DN25:
				PE_SHP_HW_H13_DBG_PRINT("set shp_scl_c_04tap_dn25_coeff_h13\n");
				PE_SHP_HW_H13_WrSclCoeff(wr_ctrl,&(shp_scl_c_04tap_dn25_coeff_h13[0]));
				break;
			case PE_SHP_HW_H13_SCL_V06T_NORM:
			case PE_SHP_HW_H13_SCL_V06T_HALF:
			default:
				PE_SHP_HW_H13_DBG_PRINT("set shp_scl_c_04tap_coeff_h13[%d]\n",v_c_idx);
				PE_SHP_HW_H13_WrSclCoeff(wr_ctrl,&(shp_scl_c_04tap_coeff_h13[v_c_idx][0]));
				break;
		}
	}
	return ret;
 ...
 * @author
 */
int PE_SHP_H13_RdSclFltCoeff(LX_PE_ETC_DBG_T *pstParams)
{
	int ret = RET_OK;
	UINT32 ret_ofst,h_v_sel,ofst=0;
	UINT32 *pData=NULL;
	PE_SHP_HW_H13_SETTINGS_T *pInfo=&_g_pe_shp_hw_h13_info;
	...
	do{
		...
		PE_SHP_HW_H13_CHECK_CODE(pstParams->size!=1152,break,"[%s,%d] size mismatch.\n",__F__,__L__);
		pData = pstParams->data;
		PE_SHP_H13_SetSclFltRnwMode(0x1);	//read mode
		/**
		* - pData[0] : tap
		* [31:28]MSL h_y_tap, [27:24]MSL v_y_tap, [23:20]MSL h_c_tap, [19:16]MSL v_c_tap
		* [15:12]MSR h_y_tap, [11:08]MSR v_y_tap, [07:04]MSR h_c_tap, [03:00]MSR v_c_tap
		* - pData[1] : MSL idx
		* [31:24]MSL h_y idx, [23:16]MSL v_y idx, [15:08]MSL h_c idx, [07:00]MSL v_c idx
		* - pData[2] : MSR idx
		* [31:24]MSR h_y idx, [23:16]MSR v_y idx, [15:08]MSR h_c idx, [07:00]MSR v_c idx
		*/
		pData[ofst] = (pInfo->h_y_tap[LX_PE_WIN_0]<<28)+(pInfo->v_y_tap[LX_PE_WIN_0]<<24)+\
						(pInfo->h_c_tap[LX_PE_WIN_0]<<20)+(pInfo->v_c_tap[LX_PE_WIN_0]<<16)+\
						(pInfo->h_y_tap[LX_PE_WIN_1]<<12)+(pInfo->v_y_tap[LX_PE_WIN_1]<<8)+\
						(pInfo->h_c_tap[LX_PE_WIN_1]<<4)+(pInfo->v_c_tap[LX_PE_WIN_1]);
		ofst += 1;
		pData[ofst] = (pInfo->pre_scl_idx[LX_PE_WIN_0].h_luma_id<<24)+\
						(pInfo->pre_scl_idx[LX_PE_WIN_0].v_luma_id<<16)+\
						(pInfo->pre_scl_idx[LX_PE_WIN_0].h_chrm_id<<8)+\
						(pInfo->pre_scl_idx[LX_PE_WIN_0].v_chrm_id);
		ofst += 1;
		pData[ofst] = (pInfo->pre_scl_idx[LX_PE_WIN_1].h_luma_id<<24)+\
						(pInfo->pre_scl_idx[LX_PE_WIN_1].v_luma_id<<16)+\
						(pInfo->pre_scl_idx[LX_PE_WIN_1].h_chrm_id<<8)+\
						(pInfo->pre_scl_idx[LX_PE_WIN_1].v_chrm_id);
		ofst += 1;
		/** see pe_shp_scl_hw_h13.h
		* #define PE_SHP_SCL_HW_H13_Y_H_12T_NUM		192
		* #define PE_SHP_SCL_HW_H13_Y_V_12T_NUM		192
		* #define PE_SHP_SCL_HW_H13_Y_H_08T_NUM		112
		* #define PE_SHP_SCL_HW_H13_Y_V_06T_NUM		64
		* #define PE_SHP_SCL_HW_H13_C_08T_NUM		80
		* #define PE_SHP_SCL_HW_H13_C_04T_NUM		48
		*/
		/* MSC L : 1.12tap case : 192+192+80 = 464, 2.06tap case : 112+64+80+48 = 304 */
		if(h_y_tap0==PE_SHP_HW_H13_SCL_V12T)	//12tap mode
		{
			/** 0. read y,h coeff **/
			h_v_sel = 0x4;	//def flt h
			ret_ofst = PE_SHP_H13_RdLumaSclFltCoeff(LX_PE_WIN_0,h_v_sel,&(pData[ofst]));
			ofst += ret_ofst;//ret_ofst=112
			h_v_sel = 0x5;	//sub flt h
			ret_ofst = PE_SHP_H13_RdLumaSclFltCoeff(LX_PE_WIN_0,h_v_sel,&(pData[ofst]));
			ofst += ret_ofst;//ret_ofst=112
			/** 1. read y,v coeff **/
			h_v_sel = 0x2;	//def flt v
			ret_ofst = PE_SHP_H13_RdLumaSclFltCoeff(LX_PE_WIN_0,h_v_sel,&(pData[ofst]));
			ofst += ret_ofst;//ret_ofst=112
			h_v_sel = 0x3;	//sub flt v
			ret_ofst = PE_SHP_H13_RdLumaSclFltCoeff(LX_PE_WIN_0,h_v_sel,&(pData[ofst]));
			ofst += ret_ofst;//ret_ofst=112
			/** 2. read c,h coeff **/
			#ifdef PE_SHP_HW_H13_USE_V12TM_C08T_SEPARATE_HV
			h_v_sel = 0x4;	//def flt h
			#else
			h_v_sel = 0x0;	//def flt h&v
			#endif
			ret_ofst = PE_SHP_H13_RdChromaSclFltCoeff(LX_PE_WIN_0,h_v_sel,&(pData[ofst]));
			ofst += ret_ofst;//ret_ofst=80
			/** 3. read c,v coeff **/
			#ifdef PE_SHP_HW_H13_USE_V12TM_C08T_SEPARATE_HV
			h_v_sel = 0x2;	//def flt v
			#else
		...
		/** MSC R : 1.06tap case(only) : 112+64+80+48 = 304 */
		/** 0. read y,h coeff **/
		h_v_sel = 0x0;	//h flt
		ret_ofst = PE_SHP_H13_RdLumaSclFltCoeff(LX_PE_WIN_1,h_v_sel,&(pData[ofst]));
		ofst += ret_ofst;//ret_ofst=112
		/** 1. read y,v coeff **/
		h_v_sel = 0x1;	//v flt
		ret_ofst = PE_SHP_H13_RdLumaSclFltCoeff(LX_PE_WIN_1,h_v_sel,&(pData[ofst]));
		...
		/** 2. read c,h coeff **/
		h_v_sel = 0x0;	//h flt
		ret_ofst = PE_SHP_H13_RdChromaSclFltCoeff(LX_PE_WIN_1,h_v_sel,&(pData[ofst]));
		ofst += ret_ofst;//ret_ofst=80
		/** 3. read c,v coeff **/
		h_v_sel = 0x1;	//v flt
		ret_ofst = PE_SHP_H13_RdChromaSclFltCoeff(LX_PE_WIN_1,h_v_sel,&(pData[ofst]));
		...
		/***SSC read***/
		h_v_sel = 0x0;	//h flt
		ret_ofst = PE_SHP_H13_RdSubSclFltCoeff(h_v_sel, &(pData[ofst]));
		ofst += ret_ofst;//ret_ofst=80
		PE_SHP_H13_SetSclFltRnwMode(0x0);	//write mode back
	}while(0);
	return ret;
}
/**
 ...
 * @author
 */
int PE_SHP_HW_H13_RdDefault(LX_PE_ETC_DBG_T *pstParams)
{
	int ret = RET_OK;
	...
	do{
		CHECK_KNULL(pstParams);
		CHECK_KNULL(pstParams->data);
		PE_SHP_HW_H13_CHECK_CODE(pstParams->size!=300,break,"[%s,%d] size mismatch.\n",__F__,__L__);
		pData = pstParams->data;
		if(PE_KDRV_VER_H13BX)
		{
			ofst=0;
			if(pData[ofst]==0)	//shp l
			{
				...
				{
					table[i].addr = shp_l_pc_default_h13b0[i].addr;
				}
			}
			else if(pData[ofst]==1)	//shp r
			{
				...
				for(i=0;i<param_size;i++)
				{
					table[i].addr = shp_r_pc_default_h13b0[i].addr;
				}
			}
			else	//2,3 : vflt_c
			{
				...
				for(i=0;i<param_size;i++)
				{
					table[i].addr = vflt_c_l_pc_default_h13b0[i].addr;
					if(pData[ofst]==3)	table[i].addr += PE_PE0_REG_H13_LRDIFF;
				}
			}
			ret = PE_REG_GetTable(table,0,table_size);
			PE_SHP_HW_H13_CHECK_CODE(ret,break,"[%s,%d] PE_REG_SetTable() error.\n",__F__,__L__);
			ofst += 1;
	...
	do {
		...
		if(PE_KDRV_VER_H13BX)
		{
			...
			if(PE_CHECK_WIN0(win_id))
			{
				...
				PE_P1L_H13B0_QWr03(pe1_ti_ctrl_0, 	 coring_th,				pp->coring_th,\
												  	 y_gain,					pp->y_gain,\
												  	 c_gain,					pp->c_gain);
			}
