...
PE_D3D_THR_T     default_thr = {0, DEFAULT_D_TPD_THR, DEFAULT_D_PTN_THR, DEFAULT_D_MTN2_THR, DEFAULT_D_MTN3_THR, DEFAULT_D_FMT_TH1, DEFAULT_D_FMT_TH2, DEFAULT_D_FMT_TH3, DEFAULT_D_FMT_TH4,
...
PE_D3D_THR_T     default_thr_d = {0, DEFAULT_D_TPD_THR, DEFAULT_D_PTN_THR, DEFAULT_D_MTN2_THR, DEFAULT_D_MTN3_THR, DEFAULT_D_FMT_TH1, DEFAULT_D_FMT_TH2, DEFAULT_D_FMT_TH3, DEFAULT_D_FMT_TH4,
								DEFAULT_D_FMT_TH5, DEFAULT_D_FMT_TH6, DEFAULT_D_FMT_TH7, DEFAULT_D_FMT_TH8, DEFAULT_D_FMT_TH9, DEFAULT_D_FMT_TH10, DEFAULT_D_SKP_TH1, DEFAULT_D_SKP_TH2, DEFAULT_D_VLD_THR, DEFAULT_D_MBV_THR,
								DEFAULT_D_DFS2_THR, DEFAULT_D_DFS3_THR};
PE_D3D_THR_T     default_thr_a = {0, DEFAULT_A_TPD_THR, DEFAULT_A_PTN_THR, DEFAULT_A_MTN2_THR, DEFAULT_A_MTN3_THR, DEFAULT_A_FMT_TH1, DEFAULT_A_FMT_TH2, DEFAULT_A_FMT_TH3, DEFAULT_A_FMT_TH4,
...
----------------------------------------------------------------------------------------*/
static int PE_D3D_L9_InitHardware(void);
static int PE_D3D_L9_GetMax3(UINT32 *g_diff, UINT32 *b_diff, UINT32 *r_diff, UINT32 *diff_max);
static int PE_D3D_L9_GetMaxFromBuffer(UINT32 *pBuf, UINT32 count, UINT32 *pElect);
static int PE_D3D_L9_GetMin3(UINT32 value0, UINT32 value1, UINT32 value2);
static int PE_D3D_L9_GetDTWDistance(UINT32* s, UINT32* t, UINT32 N, UINT32 M, UINT32 wsize, UINT32 *diff);
#ifndef PE_D3D_STRICT_DECIDE
static int PE_D3D_L9_GetHistDistance(UINT32* hist_l, UINT32* hist_r, UINT32 N, UINT32* dist_l, UINT32* dist_r);
static int PE_D3D_L9_GetCount2Over3(UINT32 diff_g, UINT32 diff_b, UINT32 diff_r, UINT32 thr, UINT32* cnt);
#endif
static int PE_D3D_L9_GetDiff(UINT32 bin, UINT32 *rd_L, UINT32 *rd_R, UINT32 *diff);
static int PE_D3D_L9_EnableReadHIS(BOOLEAN flag);
static int PE_D3D_L9_RequestHIS(PE_D3D_WIN_ID_T win, PE_D3D_HISTOGRAM_TYPE_T histo_type);
static int PE_D3D_L9_ReleaseHIS(void);
static int PE_D3D_L9_GetHIS(PE_D3D_WIN_ID_T win, PE_D3D_HISTOGRAM_TYPE_T histo_type, UINT32 *pBuf, UINT32 size);
static int PE_D3D_L9_CollectBasicData(void);
static int PE_D3D_L9_GetLrcrHistogram(void);
static int PE_D3D_L9_GetDceHistogram(void);
static int PE_D3D_L9_GetDCEDelta(UINT32 cur_dce_diff, UINT32 *delta);
#ifdef PE_D3D_USE_LINE_DIFF_BY_SW
static int PE_D3D_L9_GetOVInfo(PE_D3D_CAP_SRC_T ov_src, PE_D3D_OV_INFO_T *ov_info);
static int PE_D3D_L9_GetStripFromFrame(PE_D3D_OV_INFO_T *ov_info, PE_D3D_CAP_INFO_T *capture_info, UINT32 h_line_num, UINT32 h_line_width, UINT32 v_line_num, UINT32 v_line_width);
static int PE_D3D_L9_MeasureLineDifference(PE_D3D_CAP_INFO_T *capture_info);
static int PE_D3D_L9_InitFrameMemory(UINT32 h_max_line, UINT32 v_max_line);
//static int PE_D3D_L9_UnInitFrameMemory(void);
static int PE_D3D_L9_AccumulateFormat(LX_PE_D3D_FORMAT *cur_fmt);
#endif
static int PE_D3D_L9_CheckStableFormat(BOOLEAN *pValid, LX_PE_D3D_FORMAT *cur_fmt);
#ifdef PE_D3D_USE_DISPARITY
static int PE_D3D_L9_CheckDepthInfo(BOOLEAN *pValid);
#endif
static int PE_D3D_L9_DumpHistogram(void);
static int PE_D3D_L9_DumpAllData(void);
static int PE_D3D_L9_DecideFormat(void);
static int PE_D3D_L9_DisplayResult(void);
static int PE_D3D_L9_DetectFormat(void);
static int PE_D3D_L9_SetThreshold(PE_D3D_THR_T *pstParams);
#ifdef PE_D3D_USE_NEW_ALGO
//static int PE_D3D_L9_DetectSourceChange(void);
#endif

#include "pe_d3d_alg_l9.c"

/*----------------------------------------------------------------------------------------
	Static Variables
----------------------------------------------------------------------------------------*/
static LX_PE_D3D_DEC _g_d3d_trace = LX_PE_D3D_DEC_OFF;	//default should be off.
static UINT32 _g_use_dtwd = 0x0;
...
#endif
	do {
		DE_VSYNC_Register(PE_D3D_L9_DetectFormat);

#ifdef PE_D3D_USE_LINE_DIFF_BY_SW
		ret = PE_D3D_L9_InitFrameMemory(/*h_max_line*/PE_D3D_MAX_LINE, /*v_max_line*/0);
		if (ret) break;
#endif
#ifdef PE_D3D_AUTO_START
		...
		ret = PE_D3D_L9_SetThreshold(&default_thr_d);
#endif

		ret = PE_D3D_L9_InitHardware();
		if (ret) break;
...
{
	int ret = RET_OK;
	LX_PE_SRC_TYPE cur_src_type;
	static LX_PE_SRC_TYPE pre_src_type=LX_PE_SRC_NUM;
	PE_INF_L9_SETTINGS_T inf_set;	
	do{
		CHECK_KNULL(pstParams);
		PE_CHECK_WINID(pstParams->win_id);
		PE_D3D_DBG_PRINT("[win id:%d]\n",pstParams->win_id);
		if(PE_CHECK_WIN0(pstParams->win_id))
		{
			ret = PE_INF_L9_GetCurInfSettings(&inf_set);
			PE_D3D_CHECK_CODE(ret,break,"[%s,%d] PE_INF_L9_GetCurInfSettings() error.\n",__F__,__L__);
			cur_src_type = inf_set.disp_info[LX_PE_WIN_0].src_type;
			if(pre_src_type!=cur_src_type)
			{
				switch(cur_src_type)
				{
					case LX_PE_SRC_ATV :
					...
					case LX_PE_SRC_COMP :
					case LX_PE_SRC_PC :
						memcpy(&default_thr, &default_thr_a, sizeof(PE_D3D_THR_T));
						break;
					case LX_PE_SRC_DTV :
					case LX_PE_SRC_HDMI :
					default :
						memcpy(&default_thr, &default_thr_d, sizeof(PE_D3D_THR_T));
						break;
				}
				ret = PE_D3D_L9_SetThreshold(&default_thr);
...
int PE_D3D_L9_DBG_SetThreshold(LX_PE_ETC_DBG_T *pstParams)
{
	...
	do {
		CHECK_KNULL(pstParams);
		PE_D3D_CHECK_CODE((sizeof(UINT32)*pstParams->size)!=sizeof(PE_D3D_THR_T),\
			break,"[%s,%d] size mismatch.\n",__F__,__L__);
		memcpy(&param,&pstParams->data[0],sizeof(PE_D3D_THR_T));
		ret = PE_D3D_L9_SetThreshold(&param);
		PE_D3D_CHECK_CODE(ret,break,"[%s,%d] PE_D3D_L9_SetThreshold() error.\n",__F__,__L__);
	} while(0);
	return ret;
}
...
{
	int ret = RET_OK;
	PE_D3D_THR_T *pThr = &d3d_info.thr;
	do{
		CHECK_KNULL(pstParams);
		PE_D3D_CHECK_CODE((sizeof(UINT32)*pstParams->size)!=sizeof(PE_D3D_THR_T),\
			break,"[%s,%d] size mismatch.\n",__F__,__L__);
		memcpy(&pstParams->data[0],pThr,sizeof(PE_D3D_THR_T));
	}while(0);
	return ret;
}
int PE_D3D_L9_SetThreshold(PE_D3D_THR_T *pstParams)
{
    int ret = RET_OK;
	PE_D3D_THR_T *pThr = &d3d_info.thr;
	...
	do {
		...
		if(pstParams->isDefault)
			memcpy(pThr, &default_thr, sizeof(PE_D3D_THR_T));
		else
			memcpy(pThr, pstParams, sizeof(PE_D3D_THR_T));
...
int PE_D3D_L9_SetD3D(LX_PE_D3D_SETTINGS_T *pstParams)
{
	...
	PE_D3D_CFG_T *pCfg = &d3d_info.cfg;
	PE_D3D_THR_T *pThr = &d3d_info.thr;
	PE_D3D_THR_T stParams;
	UINT32 i;

	do {
		CHECK_KNULL(pstParams);

		_g_d3d_trace = pstParams->enb;
		initial_fmt = pstParams->fmt;
		...
		if(pThr->tpd_thr == 0xFFFFFFFF)
		{
			memcpy(&stParams, &default_thr_d, sizeof(PE_D3D_THR_T));
			ret = PE_D3D_L9_SetThreshold(&stParams);
		}
	} while (0);
...
int PE_D3D_L9_GetD3DInfo(LX_PE_D3D_INFO_T *pstParams)
{
	...
	do{
		...
		{
			pstParams->d3d_inf = initial_fmt;
		}
		else if(_g_d3d_trace==LX_PE_D3D_DEC_FIX_2D)
		{
			pstParams->d3d_inf = LX_PE_D3D_FMT_2D_2D;
		}
	}while(0);
...
static int PE_D3D_L9_InitHardware(void)
{
	...
	do {
		...
		PE_DEB_L9B_RdFL(deb_intr_mask);
		PE_DEB_L9B_Wr01(deb_intr_mask, mask_disp_for_cpu, 0);  // unmasking interrupt from de
		PE_DEB_L9B_WrFL(deb_intr_mask);
	} while(0);
...
*/
	...
	for(i=1;i<=N;i++)
	{
		...
		for(j=JS;j<=JE;j++)
		{
			...
			#else
			DTW[j][i] = cost + PE_D3D_L9_GetMin3(DTW[j  ][i-1],	// Insertion
									DTW[j-1][i  ],	// Deletion
...
static int PE_D3D_L9_EnableReadHIS(BOOLEAN flag)
{
	...
	do {
		if(flag == FALSE)
		{
			...
			PE_P1L_L9B0_Wr01(pe1_l_lrcr_ctrl_00, hist_rd_en, 0);
			PE_P1L_L9B0_WrFL(pe1_l_lrcr_ctrl_00);
		}
		else
		{
			PE_P1L_L9B0_RdFL(pe1_l_lrcr_ctrl_00);
			PE_P1L_L9B0_Wr01(pe1_l_lrcr_ctrl_00, hist_rd_en, 1);
			PE_P1L_L9B0_WrFL(pe1_l_lrcr_ctrl_00);
		}
	} while(0);
		...
		switch ( histo_type )
		{
			case D3D_HIS_GRE :
				if( win == D3D_WIN_LFT || win == D3D_WIN_TOP ) {
					PE_P1L_L9B0_RdFL(pe1_l_g_hist_ia_ctrl);
					PE_P1L_L9B0_Wr01(pe1_l_g_hist_ia_ctrl, hif_hist_enable, 0);
					PE_P1L_L9B0_Wr01(pe1_l_g_hist_ia_ctrl, hif_hist_ai, 1);
					PE_P1L_L9B0_WrFL(pe1_l_g_hist_ia_ctrl);
				} else {
					PE_P1L_L9B0_RdFL(pe1_r_g_hist_ia_ctrl);
					PE_P1L_L9B0_Wr01(pe1_r_g_hist_ia_ctrl, hif_hist_enable, 0);
					PE_P1L_L9B0_Wr01(pe1_r_g_hist_ia_ctrl, hif_hist_ai, 1);
					PE_P1L_L9B0_WrFL(pe1_r_g_hist_ia_ctrl);
				}
				break;
			case D3D_HIS_BLU :
				if( win == D3D_WIN_LFT || win == D3D_WIN_TOP ) {
					PE_P1L_L9B0_RdFL(pe1_l_b_hist_ia_ctrl);
					PE_P1L_L9B0_Wr01(pe1_l_b_hist_ia_ctrl, hif_hist_enable, 0);
					PE_P1L_L9B0_Wr01(pe1_l_b_hist_ia_ctrl, hif_hist_ai, 1);
					PE_P1L_L9B0_WrFL(pe1_l_b_hist_ia_ctrl);
				} else {
					PE_P1L_L9B0_RdFL(pe1_r_b_hist_ia_ctrl);
					PE_P1L_L9B0_Wr01(pe1_r_b_hist_ia_ctrl, hif_hist_enable, 0);
					PE_P1L_L9B0_Wr01(pe1_r_b_hist_ia_ctrl, hif_hist_ai, 1);
					PE_P1L_L9B0_WrFL(pe1_r_b_hist_ia_ctrl);
				}
				break;
			case D3D_HIS_RED :
				if( win == D3D_WIN_LFT || win == D3D_WIN_TOP ) {
					PE_P1L_L9B0_RdFL(pe1_l_r_hist_ia_ctrl);
					PE_P1L_L9B0_Wr01(pe1_l_r_hist_ia_ctrl, hif_hist_enable, 0);
					PE_P1L_L9B0_Wr01(pe1_l_r_hist_ia_ctrl, hif_hist_ai, 1);
					PE_P1L_L9B0_WrFL(pe1_l_r_hist_ia_ctrl);
				} else {
					PE_P1L_L9B0_RdFL(pe1_r_r_hist_ia_ctrl);
					PE_P1L_L9B0_Wr01(pe1_r_r_hist_ia_ctrl, hif_hist_enable, 0);
					PE_P1L_L9B0_Wr01(pe1_r_r_hist_ia_ctrl, hif_hist_ai, 1);
					PE_P1L_L9B0_WrFL(pe1_r_r_hist_ia_ctrl);
				}
				break;
			default :
...
static int PE_D3D_L9_ReleaseHIS(void)
{
	...
	do {
		PE_P1L_L9B0_RdFL(pe1_r_r_hist_ia_ctrl);
		PE_P1L_L9B0_Wr01(pe1_r_r_hist_ia_ctrl, hist_rd_done, 1);
		PE_P1L_L9B0_WrFL(pe1_r_r_hist_ia_ctrl);
		PE_P1L_L9B0_Wr01(pe1_r_r_hist_ia_ctrl, hist_rd_done, 0);
		PE_P1L_L9B0_WrFL(pe1_r_r_hist_ia_ctrl);
		...
		PE_P1L_L9B0_RdFL(pe1_r_b_hist_ia_ctrl);
		PE_P1L_L9B0_RdFL(pe1_r_r_hist_ia_ctrl);
		PE_P1L_L9B0_Wr01(pe1_l_g_hist_ia_ctrl, hif_hist_enable, 1);
		PE_P1L_L9B0_Wr01(pe1_l_g_hist_ia_ctrl, hif_hist_ai, 0);
		PE_P1L_L9B0_Wr01(pe1_l_b_hist_ia_ctrl, hif_hist_enable, 1);
		PE_P1L_L9B0_Wr01(pe1_l_b_hist_ia_ctrl, hif_hist_ai, 0);
		PE_P1L_L9B0_Wr01(pe1_l_r_hist_ia_ctrl, hif_hist_enable, 1);
		PE_P1L_L9B0_Wr01(pe1_l_r_hist_ia_ctrl, hif_hist_ai, 0);
		PE_P1L_L9B0_Wr01(pe1_r_g_hist_ia_ctrl, hif_hist_enable, 1);
		PE_P1L_L9B0_Wr01(pe1_r_g_hist_ia_ctrl, hif_hist_ai, 0);
		PE_P1L_L9B0_Wr01(pe1_r_b_hist_ia_ctrl, hif_hist_enable, 1);
		PE_P1L_L9B0_Wr01(pe1_r_b_hist_ia_ctrl, hif_hist_ai, 0);
		PE_P1L_L9B0_Wr01(pe1_r_r_hist_ia_ctrl, hif_hist_enable, 1);
		PE_P1L_L9B0_Wr01(pe1_r_r_hist_ia_ctrl, hif_hist_ai, 0);
		PE_P1L_L9B0_WrFL(pe1_l_g_hist_ia_ctrl);
		PE_P1L_L9B0_WrFL(pe1_l_b_hist_ia_ctrl);
		PE_P1L_L9B0_WrFL(pe1_l_r_hist_ia_ctrl);
		PE_P1L_L9B0_WrFL(pe1_r_g_hist_ia_ctrl);
	...
	do {
		switch ( histo_type )
		{
			case D3D_HIS_GRE :
				if( win == D3D_WIN_LFT || win == D3D_WIN_TOP ) {
					for(i = 0; i < size; i++) {
						PE_P1L_L9B0_RdFL(pe1_l_g_hist_ia_data);
						pBuf[i] = PE_P1L_L9B0_Rd(pe1_l_g_hist_ia_data);
					}
					// host mode restore (read done)
					PE_P1L_L9B0_RdFL(pe1_l_g_hist_ia_ctrl);
					PE_P1L_L9B0_Wr01(pe1_l_g_hist_ia_ctrl, hif_hist_enable, 1);
					...
					for(i = 0; i < size; i++) {
						PE_P1L_L9B0_RdFL(pe1_r_g_hist_ia_data);
						pBuf[i] = PE_P1L_L9B0_Rd(pe1_r_g_hist_ia_data);
					}
					// host mode restore (read done)
					PE_P1L_L9B0_RdFL(pe1_r_g_hist_ia_ctrl);
					PE_P1L_L9B0_Wr01(pe1_r_g_hist_ia_ctrl, hif_hist_enable, 1);
			...
			case D3D_HIS_BLU :
				if( win == D3D_WIN_LFT || win == D3D_WIN_TOP ) {
					for(i = 0; i < size; i++) {
						PE_P1L_L9B0_RdFL(pe1_l_b_hist_ia_data);
						pBuf[i] = PE_P1L_L9B0_Rd(pe1_l_b_hist_ia_data);
					}
					PE_P1L_L9B0_RdFL(pe1_l_b_hist_ia_ctrl);
					PE_P1L_L9B0_Wr01(pe1_l_b_hist_ia_ctrl, hif_hist_enable, 1);
					PE_P1L_L9B0_Wr01(pe1_l_b_hist_ia_ctrl, hif_hist_ai, 0);
					...
					for(i = 0; i < size; i++) {
						PE_P1L_L9B0_RdFL(pe1_r_b_hist_ia_data);
						pBuf[i] = PE_P1L_L9B0_Rd(pe1_r_b_hist_ia_data);
					}
					PE_P1L_L9B0_RdFL(pe1_r_b_hist_ia_ctrl);
					PE_P1L_L9B0_Wr01(pe1_r_b_hist_ia_ctrl, hif_hist_enable, 1);
					PE_P1L_L9B0_Wr01(pe1_r_b_hist_ia_ctrl, hif_hist_ai, 0);
			...
			case D3D_HIS_RED :
				if( win == D3D_WIN_LFT || win == D3D_WIN_TOP ) {
					for(i = 0; i < size; i++) {
						PE_P1L_L9B0_RdFL(pe1_l_r_hist_ia_data);
						pBuf[i] = PE_P1L_L9B0_Rd(pe1_l_r_hist_ia_data);
					}
					PE_P1L_L9B0_RdFL(pe1_l_r_hist_ia_ctrl);
					PE_P1L_L9B0_Wr01(pe1_l_r_hist_ia_ctrl, hif_hist_enable, 1);
					PE_P1L_L9B0_Wr01(pe1_l_r_hist_ia_ctrl, hif_hist_ai, 0);
					...
					for(i = 0; i < size; i++) {
						PE_P1L_L9B0_RdFL(pe1_r_r_hist_ia_data);
						pBuf[i] = PE_P1L_L9B0_Rd(pe1_r_r_hist_ia_data);
					}
					PE_P1L_L9B0_RdFL(pe1_r_r_hist_ia_ctrl);
					PE_P1L_L9B0_Wr01(pe1_r_r_hist_ia_ctrl, hif_hist_enable, 1);
					PE_P1L_L9B0_Wr01(pe1_r_r_hist_ia_ctrl, hif_hist_ai, 0);
...
static int PE_D3D_L9_CollectBasicData(void)
{
	...
	do {
		...
		ret = PE_D3D_L9_GetMaxFromBuffer(motion_l, 4, &pData->motion_l);
		...
		ret = PE_D3D_L9_GetMaxFromBuffer(motion_r, 4, &pData->motion_r);
...
static int PE_D3D_L9_GetLrcrHistogram(void)
{
	...
	UINT32 use_dtwd = _g_use_dtwd;

	do {
		if(prev_outtype != pCfg->out_type)
		{
			ret = PE_D3D_L9_ReleaseHIS();
			if (ret) break;
#ifdef PE_D3D_USE_NEW_ALGO
...
#endif
			ret = PE_WIN_HW_L9_SetLrcrWin(LX_PE_OUT_2D, pStm->win_pos);
			if (ret) break;
			ret = PE_WIN_HW_L9_SetLrcrInOutSel();
			if (ret) break;
		}
		...
		switch (pStm->cur_stm)
		{
			case D3D_STM_HIS_RDY__LR :
				...
				PE_WIN_HW_L9_SetLrcrWin(LX_PE_OUT_SS, pStm->win_pos);
				PE_WIN_HW_L9_SetLrcrInOutSel();
			...
			case D3D_STM_HIS_SET__LR :
				ret = PE_D3D_L9_EnableReadHIS(TRUE);
			...
			case D3D_STM_HIS_GET_LRG :
				ret = PE_D3D_L9_RequestHIS(D3D_WIN_LFT, D3D_HIS_GRE);
				if (ret) break;
				ret = PE_D3D_L9_GetHIS(D3D_WIN_LFT, D3D_HIS_GRE, pData->hist_l_g, pCfg->rb_bin_d3d);
				if (ret) break;
				ret = PE_D3D_L9_RequestHIS(D3D_WIN_RGT, D3D_HIS_GRE);
				if (ret) break;
				ret = PE_D3D_L9_GetHIS(D3D_WIN_RGT, D3D_HIS_GRE, pData->hist_r_g, pCfg->rb_bin_d3d);
				if (ret) break;
				// cal G diff
				if(use_dtwd == 0)
					PE_D3D_L9_GetDiff(pCfg->rb_bin_d3d, pData->hist_l_g, pData->hist_r_g, &pData->g_diff);
				else
					PE_D3D_L9_GetDTWDistance(pData->hist_l_g, pData->hist_r_g, pCfg->rb_bin_d3d, pCfg->rb_bin_d3d, pCfg->rb_bin_d3d, &pData->g_diff);
			#ifndef PE_D3D_STRICT_DECIDE
				ret = PE_D3D_L9_GetHistDistance(pData->hist_l_g, pData->hist_r_g, pCfg->rb_bin_d3d, &pData->dist_l_g, &pData->dist_r_g);
			#endif
				pStm->nxt_stm = D3D_STM_HIS_GET_LRB;
				break;
			case D3D_STM_HIS_GET_LRB :
				ret = PE_D3D_L9_RequestHIS(D3D_WIN_LFT, D3D_HIS_BLU);
				if (ret) break;
				ret = PE_D3D_L9_GetHIS(D3D_WIN_LFT, D3D_HIS_BLU, pData->hist_l_b, pCfg->rb_bin_d3d);
				if (ret) break;
				ret = PE_D3D_L9_RequestHIS(D3D_WIN_RGT, D3D_HIS_BLU);
				if (ret) break;
				ret = PE_D3D_L9_GetHIS(D3D_WIN_RGT, D3D_HIS_BLU, pData->hist_r_b, pCfg->rb_bin_d3d);
				if (ret) break;
				// cal B diff
				if(use_dtwd == 0)
					PE_D3D_L9_GetDiff(pCfg->rb_bin_d3d, pData->hist_l_b, pData->hist_r_b, &pData->b_diff);
				else
					PE_D3D_L9_GetDTWDistance(pData->hist_l_b, pData->hist_r_b, pCfg->rb_bin_d3d, pCfg->rb_bin_d3d, pCfg->rb_bin_d3d, &pData->b_diff);
			#ifndef PE_D3D_STRICT_DECIDE
				ret = PE_D3D_L9_GetHistDistance(pData->hist_l_b, pData->hist_r_b, pCfg->rb_bin_d3d, &pData->dist_l_b, &pData->dist_r_b);
			#endif
				pStm->nxt_stm = D3D_STM_HIS_GET_LRR;
				break;
			case D3D_STM_HIS_GET_LRR :
				ret = PE_D3D_L9_RequestHIS(D3D_WIN_LFT, D3D_HIS_RED);
				if (ret) break;
				ret = PE_D3D_L9_GetHIS(D3D_WIN_LFT, D3D_HIS_RED, pData->hist_l_r, pCfg->rb_bin_d3d);
				if (ret) break;
				ret = PE_D3D_L9_RequestHIS(D3D_WIN_RGT, D3D_HIS_RED);
				if (ret) break;
				ret = PE_D3D_L9_GetHIS(D3D_WIN_RGT, D3D_HIS_RED, pData->hist_r_r, pCfg->rb_bin_d3d);
				if (ret) break;
				ret = PE_D3D_L9_ReleaseHIS();
				if (ret) break;
				ret = PE_D3D_L9_EnableReadHIS(FALSE);
				// cal R diff
				if(use_dtwd == 0)
					PE_D3D_L9_GetDiff(pCfg->rb_bin_d3d, pData->hist_l_r, pData->hist_r_r, &pData->r_diff);
				else
					PE_D3D_L9_GetDTWDistance(pData->hist_l_r, pData->hist_r_r, pCfg->rb_bin_d3d, pCfg->rb_bin_d3d, pCfg->rb_bin_d3d, &pData->r_diff);
			#ifndef PE_D3D_STRICT_DECIDE
				ret = PE_D3D_L9_GetHistDistance(pData->hist_l_r, pData->hist_r_r, pCfg->rb_bin_d3d, &pData->dist_l_r, &pData->dist_r_r);
			#endif
				// cal LR diff with g/b/r
				PE_D3D_L9_GetMax3(&pData->g_diff, &pData->b_diff, &pData->r_diff, &pData->lr_diff);
				pStm->nxt_stm = D3D_STM_HIS_TMP;
			case D3D_STM_HIS_TMP :
				pStm->nxt_stm = D3D_STM_HIS_RDY__TB;
				break;
			case D3D_STM_HIS_RDY__TB :
				PE_WIN_HW_L9_SetLrcrWin(LX_PE_OUT_TB, pStm->win_pos);
				PE_WIN_HW_L9_SetLrcrInOutSel();
			...
			case D3D_STM_HIS_GET_TBG :
				ret = PE_D3D_L9_RequestHIS(D3D_WIN_TOP, D3D_HIS_GRE);
				if (ret) break;
				ret = PE_D3D_L9_GetHIS(D3D_WIN_TOP, D3D_HIS_GRE, pData->hist_t_g, pCfg->rb_bin_d3d);
				if (ret) break;
				ret = PE_D3D_L9_RequestHIS(D3D_WIN_BOT, D3D_HIS_GRE);
				if (ret) break;
				ret = PE_D3D_L9_GetHIS(D3D_WIN_BOT, D3D_HIS_GRE, pData->hist_b_g, pCfg->rb_bin_d3d);
				if (ret) break;
				// cal G diff
				if(use_dtwd == 0)
					PE_D3D_L9_GetDiff(pCfg->rb_bin_d3d, pData->hist_t_g, pData->hist_b_g, &pData->g_diff);
				else
					PE_D3D_L9_GetDTWDistance(pData->hist_t_g, pData->hist_b_g, pCfg->rb_bin_d3d, pCfg->rb_bin_d3d, pCfg->rb_bin_d3d, &pData->g_diff);
			#ifndef PE_D3D_STRICT_DECIDE
				ret = PE_D3D_L9_GetHistDistance(pData->hist_t_g, pData->hist_b_g, pCfg->rb_bin_d3d, &pData->dist_t_g, &pData->dist_b_g);
			#endif
				pStm->nxt_stm = D3D_STM_HIS_GET_TBB;
				break;
			case D3D_STM_HIS_GET_TBB :
				ret = PE_D3D_L9_RequestHIS(D3D_WIN_TOP, D3D_HIS_BLU);
				if (ret) break;
				ret = PE_D3D_L9_GetHIS(D3D_WIN_TOP, D3D_HIS_BLU, pData->hist_t_b, pCfg->rb_bin_d3d);
				if (ret) break;
				ret = PE_D3D_L9_RequestHIS(D3D_WIN_BOT, D3D_HIS_BLU);
				if (ret) break;
				ret = PE_D3D_L9_GetHIS(D3D_WIN_BOT, D3D_HIS_BLU, pData->hist_b_b, pCfg->rb_bin_d3d);
				if (ret) break;
				// cal B diff
				if(use_dtwd == 0)
					PE_D3D_L9_GetDiff(pCfg->rb_bin_d3d, pData->hist_t_b, pData->hist_b_b, &pData->b_diff);
				else
					PE_D3D_L9_GetDTWDistance(pData->hist_t_b, pData->hist_b_b, pCfg->rb_bin_d3d, pCfg->rb_bin_d3d, pCfg->rb_bin_d3d, &pData->b_diff);
			#ifndef PE_D3D_STRICT_DECIDE
				ret = PE_D3D_L9_GetHistDistance(pData->hist_t_b, pData->hist_b_b, pCfg->rb_bin_d3d, &pData->dist_t_b, &pData->dist_b_b);
			#endif
				pStm->nxt_stm = D3D_STM_HIS_GET_TBR;
				break;
			case D3D_STM_HIS_GET_TBR :
				ret = PE_D3D_L9_RequestHIS(D3D_WIN_TOP, D3D_HIS_RED);
				if (ret) break;
				ret = PE_D3D_L9_GetHIS(D3D_WIN_TOP, D3D_HIS_RED, pData->hist_t_r, pCfg->rb_bin_d3d);
				if (ret) break;
				ret = PE_D3D_L9_RequestHIS(D3D_WIN_BOT, D3D_HIS_RED);
				if (ret) break;
				ret = PE_D3D_L9_GetHIS(D3D_WIN_BOT, D3D_HIS_RED, pData->hist_b_r, pCfg->rb_bin_d3d);
				if (ret) break;
				ret = PE_D3D_L9_ReleaseHIS();
				if (ret) break;
				ret = PE_D3D_L9_EnableReadHIS(FALSE);
				// cal R diff
				if(use_dtwd == 0)
					PE_D3D_L9_GetDiff(pCfg->rb_bin_d3d, pData->hist_t_r, pData->hist_b_r, &pData->r_diff);
				else
					PE_D3D_L9_GetDTWDistance(pData->hist_t_r, pData->hist_b_r, pCfg->rb_bin_d3d, pCfg->rb_bin_d3d, pCfg->rb_bin_d3d, &pData->r_diff);
			#ifndef PE_D3D_STRICT_DECIDE
				ret = PE_D3D_L9_GetHistDistance(pData->hist_t_r, pData->hist_b_r, pCfg->rb_bin_d3d, &pData->dist_t_r, &pData->dist_b_r);
			#endif
				// cal TB diff with g/b/r
				PE_D3D_L9_GetMax3(&pData->g_diff, &pData->b_diff, &pData->r_diff, &pData->tb_diff);
...
static void PE_D3D_L9_GetAcGain3dInfo(void)
{
	PE_D3D_CFG_T *pCfg = &d3d_info.cfg;
	UINT32 type_chg=0;
	static LX_PE_D3D_FORMAT cur_fmt = LX_PE_D3D_FMT_MAX;
	...
	do{
		...
		if((pCfg->out_type==LX_PE_OUT_TB||pCfg->out_type==LX_PE_OUT_SS||\
			pCfg->out_type==LX_PE_OUT_FS||pCfg->out_type==LX_PE_OUT_DUAL_TB|| \
			...
			(pCfg->in_type==LX_PE_3D_IN_TB||pCfg->in_type==LX_PE_3D_IN_SS))
		{
			if(type_chg && cur_fmt<LX_PE_D3D_FMT_MAX)
			{
				...
				{
					pCfg->d3d_chk=2;	//not ss
				}
				else if(pCfg->in_type==LX_PE_3D_IN_TB && cur_fmt!=LX_PE_D3D_FMT_3D_TB)
				{
					pCfg->d3d_chk=1;	//not tb
				}
				else
				{
					pCfg->d3d_chk=0;	//normal
				}
				/*printk("[%s,%d][chg:%d]o:%d,i:%d, cur_fmt:%d, -> d3d_chk:%d\n\n",__F__,__L__,\
					type_chg,pCfg->out_type,pCfg->in_type,cur_fmt,pCfg->d3d_chk);*/
			}
...
static int PE_D3D_L9_GetDceHistogram(void)
{
	int ret = RET_OK;
	PE_D3D_CFG_T *pCfg = &d3d_info.cfg;
	PE_DCM_HW_L9_HISTO_T histo_info_0;
	PE_DCM_HW_L9_HISTO_T histo_info_1;
	static LX_PE_OUT_TYPE prev_outtype = LX_PE_OUT_NUM;
	...
	do {
		if(prev_outtype != pCfg->out_type)
		{
			PE_WIN_HW_L9_SetD3dDceHistoWin();
		}
		prev_outtype = pCfg->out_type;

		histo_info_0.bin_num = 32;
		histo_info_1.bin_num = 32;
		PE_DCM_HW_L9_GetDceAllHistogram(&histo_info_0,&histo_info_1,&pCfg->validL, &pCfg->validR);
	...
	#ifndef PE_D3D_STRICT_DECIDE
		ret = PE_D3D_L9_GetHistDistance(pData->dce_l, pData->dce_r, pCfg->rb_bin_d3d, &pData->dist_l, &pData->dist_r);
	#endif
		pData->dce_diff = 0;
...
static int PE_D3D_L9_GetOVInfo(PE_D3D_CAP_SRC_T ov_src, PE_D3D_OV_INFO_T *ov_info)
{
	...
	do {
		switch (ov_src)
		{
			...
			{
				PE_DEC_L9B_RdFL(dec_intr_mask);
				PE_DEC_L9B_Rd01(dec_intr_mask, dec_intr_mask, intr_mask);
				ov_info->src_valid = ((intr_mask & (1<<23)) == 0)?TRUE:FALSE;

				PE_MIB_L9_RdFL(mifb_saddr_rd_ext0);
				PE_MIB_L9_Rd01(mifb_saddr_rd_ext0, req_rd_saddr_8_ovm_y_msb4b, ov_info->y_msb);
				PE_MIB_L9_Rd01(mifb_saddr_rd_ext0, req_rd_saddr_8_ovm_c_msb4b, ov_info->c_msb);

				PE_MIB_L9_RdFL(mifb_saddr_rd_ovm_y);
				PE_MIB_L9_Rd01(mifb_saddr_rd_ovm_y, req_rd_saddr_8_ovm_y, ov_info->y_base);
				PE_MIB_L9_RdFL(mifb_saddr_rd_ovm_c);
				PE_MIB_L9_Rd01(mifb_saddr_rd_ovm_c, req_rd_saddr_8_ovm_c, ov_info->c_base);

				PE_MIB_L9_RdFL(mifb_rd_set_10b);
				PE_MIB_L9_Rd01(mifb_rd_set_10b, set_10b_req_rd_ovm_y, ov_info->bpp);

				#if 0
				PE_MIB_L9_RdFL(mifb_rd_cmd_stride0);
				PE_MIB_L9_Rd01(mifb_rd_cmd_stride0, cmd_rd_stride_v_8_ovm_y, ov_info->stride);
				#else
				ov_info->stride = 2048;
				#endif

				PE_OVL_L9_RdFL(ovl_ov_msize);
				PE_OVL_L9_Rd01(ovl_ov_msize, m_hsize, ov_info->h);
				PE_OVL_L9_Rd01(ovl_ov_msize, m_vsize, ov_info->v);

				PE_MSL_L9_RdFL(msc_c_format_l);
				PE_MSL_L9_Rd01(msc_c_format_l, out_c_format, ov_info->sub_sample);
			}
			break;
			case D3D_CAP_SRC_S_CVR :
			{
				...
				PE_MIF_L9_RdFL(miff_saddr_rd_ext0);
				PE_MIF_L9_Rd01(miff_saddr_rd_ext0, req_rd_saddr_cve_y_msb4b, ov_info->y_msb);
				PE_MIF_L9_Rd01(miff_saddr_rd_ext0, req_rd_saddr_cve_c_msb4b, ov_info->c_msb);

				PE_MIF_L9_RdFL(miff_saddr_rd_cve_yc);
				PE_MIF_L9_Rd01(miff_saddr_rd_cve_yc, req_rd_saddr_cve_y, ov_info->y_base);
				PE_MIF_L9_Rd01(miff_saddr_rd_cve_yc, req_rd_saddr_cve_c, ov_info->c_base);
				#if 0
				PE_MIF_L9_RdFL(miff_rd_cmd_stride0);
				PE_MIF_L9_Rd01(miff_rd_cmd_stride0, cmd_rd_stride_v_cve_y, stride);
				#endif

				PE_SSC_L9_RdFL(ssc_win_out_size);
				PE_SSC_L9_Rd01(ssc_win_out_size, out_win_h_size, ov_info->h);
				PE_SSC_L9_Rd01(ssc_win_out_size, out_win_v_size, ov_info->v);

				PE_SSC_L9_RdFL(ssc_ctrl);
				PE_SSC_L9_Rd01(ssc_ctrl, dvr_write_mode, stride);
				if(stride)
					ov_info->stride = ((ov_info->h + 15) / 16) * 16;
				else
					ov_info->stride = 1024;

				PE_SSC_L9_RdFL(ssc_c_format);
				PE_SSC_L9_Rd01(ssc_c_format, out_c_format, ov_info->sub_sample);
			}
			break;
...
static int PE_D3D_L9_GetStripFromFrame(PE_D3D_OV_INFO_T *ov_info, PE_D3D_CAP_INFO_T *capture_info, UINT32 h_line_num, UINT32 h_line_width, UINT32 v_line_num, UINT32 v_line_width)
{
	...
	do {
		...
		if((ov_info->y_base << 14) > 0x10000000)
		{
			if(pCfg->cap_enb >= LX_PE_D3D_CAP_FMT) PE_PRINT("y_base  0x%08x\n", (ov_info->y_base << 14));
			ret = RET_ERROR;
			break;
		}
...
static int PE_D3D_L9_CheckTime(char *fn_str, PE_D3D_GET_TIME_T location, PE_D3D_TIME_T pos)
{
	int ret = RET_OK;
...
static int PE_D3D_CaptureFrame(void)
{
	...
	do {
		...
		PE_D3D_L9_CheckTime("Capture line", D3D_GET_TIME_CAP, D3D_GET_START);

		ret = PE_D3D_L9_GetOVInfo(D3D_CAP_SRC_S_CVR, &ov_info);
		if (ret) break;
		ret = PE_D3D_L9_GetStripFromFrame(&ov_info, pCapture, pCfg->cap_h_line_num, pCfg->cap_h_line_width, pCfg->cap_v_line_num, pCfg->cap_v_line_width);
		if (ret) break;

		PE_D3D_L9_CheckTime("Capture line", D3D_GET_TIME_CAP, D3D_GET_END);

		PE_D3D_L9_CheckTime("Measure difference", D3D_GET_TIME_MAT, D3D_GET_START);

		ret = PE_D3D_L9_MeasureLineDifference(pCapture);
		if (ret) break;

		PE_D3D_L9_CheckTime("Measure difference", D3D_GET_TIME_MAT, D3D_GET_END);
	} while(0);
...
static int PE_D3D_L9_AccumulateFormat(LX_PE_D3D_FORMAT *cur_fmt)
{
	...
	do {
		...
		else
		{
			...
			PE_D3D_L9_GetMax3(&ratio_2d, &ratio_ss, &ratio_tb, &max);
			if(max < 50)
			{
...
static int PE_D3D_L9_CheckStableFormat(BOOLEAN *pValid, LX_PE_D3D_FORMAT *cur_fmt)
{
	...
	do {
		...
		for(i = 0;i < vld_thr- 1;i++)
		{
			...
			{
				if((i<(D3D_VOTE_STACK_SIZE_ON_FORMAT_CHANGE-1))&&(temp < LX_PE_D3D_FMT_MAX)) vote_nominee[temp] += 1;
			}
			else
			{
				if(temp < LX_PE_D3D_FMT_MAX) vote_nominee[temp] += 1;
			}
		}
...
static int PE_D3D_L9_DumpAllData(void)
{
	...
	do {
		...
		PE_D3D_L9_GetMax3(&pData->cb, &pData->cc, &pData->ll, &max_pd);
...
//				printk(" **********(PE_WIN_LRTB_HAF1) tmp_thr_const0 = %d, tmp_thr_const1 = %d, tmp_thr_const2 = %d, tmp_thr_const3 = %d*********\n",  tmp_thr_const0, tmp_thr_const1, tmp_thr_const2, tmp_thr_const3);
		...
		#ifndef PE_D3D_STRICT_DECIDE
			ret = PE_D3D_L9_GetCount2Over3(DIFF(pData->dist_l_g,pData->dist_r_g), DIFF(pData->dist_l_b,pData->dist_r_b), DIFF(pData->dist_l_r,pData->dist_r_r), thr_const2, &lr_over);
			ret = PE_D3D_L9_GetCount2Over3(DIFF(pData->dist_t_g,pData->dist_b_g), DIFF(pData->dist_t_b,pData->dist_b_b), DIFF(pData->dist_t_r,pData->dist_b_r), thr_const2, &tb_over);
...
#ifdef PE_D3D_USE_NEW_ALGO 	// Added by won.hur 2011.12.19
			    ...
			    pCfg->out_type == LX_PE_OUT_DUAL_FULL)
		{
			...
			{
				ret = PE_D3D_L9_GetDCEDelta(pData->dce_diff, &delta_avg);
...
//			}
		...
		// voting
		PE_D3D_L9_CheckStableFormat(&valid, &pCfg->tmp_fmt);
#ifdef PE_D3D_USE_LINE_DIFF_BY_SW
		// accumulating
		PE_D3D_L9_AccumulateFormat(&pCfg->tmp_fmt);
#endif

#ifdef PE_D3D_USE_DISPARITY
		PE_D3D_L9_CheckDepthInfo(&depth_valid);
		if(depth_valid)
		{
		...
		if(1)
		{
			...
			{
				//PE_D3D_L9_DumpHistogram();
				PE_D3D_L9_DumpAllData();
				count = 0;
			}
			if(count==100)PE_D3D_L9_DumpHistogram();
		}
	} while(0);
...
static int PE_D3D_L9_DetectFormat(void)
{
	...
	PE_WIN_HW_L9_SETTINGS_T win_set;
	PE_INF_L9_SETTINGS_T inf_set;
	PE_D3D_CFG_T *pCfg = &d3d_info.cfg;
	memset0(setting);
	do {
		ret = PE_WIN_HW_L9_GetCurWinSettings(&win_set);
		if (ret) break;	
		ret = PE_INF_L9_GetCurInfSettings(&inf_set);
		if (ret) break;	
		pCfg->out_type = inf_set.disp_info[LX_PE_WIN_0].out_type;
		pCfg->in_type = inf_set.disp_info[LX_PE_WIN_0].in_type;
		pCfg->tp_mode = inf_set.disp_info[LX_PE_WIN_0].mode.is_tp? 1:0;
		pCfg->act_x0 = win_set.win_inf[LX_PE_WIN_0].act0_x0;
		pCfg->act_y0 = win_set.win_inf[LX_PE_WIN_0].act0_y0;
		pCfg->act_x1 = win_set.win_inf[LX_PE_WIN_0].act0_x1;
		pCfg->act_y1 = win_set.win_inf[LX_PE_WIN_0].act0_y1;
		pCfg->out_win_x = pCfg->act_x1 - pCfg->act_x0;
		pCfg->out_win_y = pCfg->act_y1 - pCfg->act_y0;
		pCfg->tb_ratio = win_set.d3d_info.tb_ratio;
		pCfg->ss_ratio = win_set.d3d_info.ss_ratio;
		pCfg->mute_en = inf_set.ctrl_info[LX_PE_WIN_0].mute_on;
#ifdef PE_D3D_USE_NEW_ALGO
		//gInputSourceChangedFlag = PE_D3D_L9_DetectSourceChange();
#endif
		ret = PE_D3D_L9_CollectBasicData();
		if (ret) break;
		ret = PE_D3D_L9_GetLrcrHistogram();
		if (ret) break;
		ret = PE_D3D_L9_GetDceHistogram();
		if (ret) break;
		if(!pCfg->enable) break;
		ret = PE_D3D_L9_DecideFormat();
		//if (ret) break;
		ret = PE_D3D_L9_DisplayResult();
		if (ret) break;
	} while(0);
