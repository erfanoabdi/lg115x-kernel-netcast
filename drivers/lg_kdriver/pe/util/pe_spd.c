...
/*----------------------------------------------------------------------------------------
	Macro Definitions
----------------------------------------------------------------------------------------*/
#define PE_SPD_ERROR	printk

#ifdef PE_USE_DBG_PRINT
#define PE_SPD_DBG_PRINT(fmt,args...)	printk("[%s,%d] "fmt,__F__,__L__, ##args)
#else
#define PE_SPD_DBG_PRINT(fmt,args...)
#endif

#define PE_SPD_CHECK_CODE(_checker,_action,fmt,args...)	{if(_checker){PE_SPD_ERROR(fmt,##args);_action;}}
...
PE_SUSPEND_STORE_T pe_suspend_store[] = {
	...
	{FALSE	,PE_SUSPEND_NRD_DNR	,sizeof(LX_PE_NRD_DNR0_CMN_T)		,PE_ITEM_PKTFUNC(LX_PE_NRD_DNR0_CMN_T), PE_NRD_SetFunction ,NULL},		
	{FALSE	,PE_SUSPEND_NRD_TNR ,sizeof(LX_PE_NRD_TNR0_CMN_T)		,PE_ITEM_PKTFUNC(LX_PE_NRD_TNR0_CMN_T), PE_NRD_SetFunction ,NULL},		
	...
	{FALSE	,PE_SUSPEND_SHP_RES ,sizeof(LX_PE_SHP_RE0_CMN_T) ,PE_ITEM_PKTFUNC(LX_PE_SHP_RE0_CMN_T), PE_SHP_SetFunction ,NULL},		
	{FALSE	,PE_SUSPEND_SHP_EDGE ,sizeof(LX_PE_SHP_EE0_T) ,PE_ITEM_PKTFUNC(LX_PE_SHP_EE0_T), PE_SHP_SetFunction ,NULL},		
...
int PE_SPD_CheckInitResume(unsigned int *data, unsigned int datasize)
{
	...
	do{
		...
		PE_SPD_DBG_PRINT("check_resume:%d\n",check_resume);
	}while(0);
