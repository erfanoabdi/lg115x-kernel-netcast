...
//UINT32 gui32DvrCommonBufSize	= 0x50000;
//UINT32 DVR_BUFFER_BASE		  = 0x9A90000;
...
void DVR_DefaultMemoryMap ( LX_PVR_CH_T	ch )
{
	...
	{
		astDvrMemMap[ch].stDvrDnBuff.ui32BufferBase = gMemCfgDvr[0][LX_PVR_MEM_DN].base;
		astDvrMemMap[ch].stDvrDnBuff.ui32BufferEnd  = gMemCfgDvr[0][LX_PVR_MEM_DN].base;
		astDvrMemMap[ch].stDvrDnBuff.ui32ReadPtr    = gMemCfgDvr[0][LX_PVR_MEM_DN].base;
		astDvrMemMap[ch].stDvrDnBuff.ui32WritePtr   = gMemCfgDvr[0][LX_PVR_MEM_DN].base;

		astDvrMemMap[ch].stDvrUpBuff.ui32BufferBase = gMemCfgDvr[0][LX_PVR_MEM_UP].base;
		astDvrMemMap[ch].stDvrUpBuff.ui32BufferEnd  = gMemCfgDvr[0][LX_PVR_MEM_UP].base;
		astDvrMemMap[ch].stDvrUpBuff.ui32ReadPtr    = gMemCfgDvr[0][LX_PVR_MEM_UP].base;
		astDvrMemMap[ch].stDvrUpBuff.ui32WritePtr   = gMemCfgDvr[0][LX_PVR_MEM_UP].base;
	}

		/* UP Buffer base */
		DVR_UP_SetBufBoundReg( 
		ch,
		astDvrMemMap[ch].stDvrUpBuff.ui32BufferBase,
		astDvrMemMap[ch].stDvrUpBuff.ui32BufferEnd );
		
		/* DN Buffer base */
		DVR_DN_SetBufBoundReg(
		ch,
...
#if 0
void DVR_SetBufferConfig ( UINT32 ui32DvrBufferBase, UINT32  ui32CommonDvrBufSize )
{
	int mIdx;
	//DVR_BUFFER_BASE		  = ui32DvrBufferBase;
	//gui32DvrCommonBufSize = ui32CommonDvrBufSize;

	for (mIdx = 0; mIdx < NELEMENTS(gMemCfgDvr); mIdx++)
	{
		if (mIdx > 0)	gMemCfgDvr[mIdx].base = gMemCfgDvr[mIdx-1].base + gMemCfgDvr[mIdx-1].size;
		else			gMemCfgDvr[mIdx].base = ui32DvrBufferBase;
		gMemCfgDvr[mIdx].size = ui32CommonDvrBufSize;
	}
	
	//Initialize the memory map after assigning the new values
	DVR_DefaultMemoryMap ( );
}
#endif
/** @} */
