...
UINT32 ui32DvrDnMinBufLimit = 1; // DDI 384 x 1 buffer.
UINT32 ui32DvrDnMinPktCount = 0x800;
...
UINT32	PVR_DD_Init ( LX_PVR_CH_T	 ch )
{
	 ...
	 * PIE Mode:2 -> Interrupt when index table full or packet limit
	 */
...
UINT32	PVR_DD_StartDownload ( LX_PVR_CH_T	ch)
{
	...
	if ( astDvrState[ch].eDnState != LX_PVR_DN_STATE_IDLE )
	{
		...
		PVR_PRINT ( "DVR_DN> One recording already active in [%d] channel, State[%d]\n",
			ch,
		...
		/*return -1;*/
		PVR_PRINT ( "DVR_DN> Restarting download[%d]channel !!!!\n",
			ch );
	}
...
UINT32	PVR_DD_StopDownload ( LX_PVR_CH_T	ch)
{
	...
	{
		/* One recording already going on */
		PVR_PRINT ( "DVR_DN> Recording not active in [%d]channel, State[%d]\n",
			ch,
			(UINT32)astDvrState[ch].eDnState );
		//rtrn = -1;
		/* return -1; */
...
UINT32	PVR_DD_SetPiebuffer ( LX_PVR_CH_T	ch, UINT32 uiBufAddr, UINT32 uiBufSize)
{
	...
	{
		PVR_PRINT("[ERROR]PIE Buffer Setting! ch[%d] Prev[0x%x]Size[%d] Now[0x%x]Size[%d]\n", ch,
...
UINT32	PVR_DD_StartUpload ( LX_PVR_CH_T	ch, UINT8 ui8PacketLen)
{
	...
	{
		/* One recording already going on */
		PVR_PRINT ( "DVR_UP> One playback already active in [%d]channel, State[%d]\n",
			ch,
			(UINT32)astDvrState[ch].eUpState );
		/* return -1; */
		/* Previous upload seem to be active, so close it and restart again */
		PVR_PRINT ( "DVR_UP> Restarting playback[%d]channel !!!!\n",
			ch );
	...
	PVR_PRINT ("> Start Upload [%c] setting...\n", ch ? 'B' : 'A' );
...
UINT32	PVR_DD_StopUpload ( LX_PVR_CH_T	ch)
{
	...
	{
		/* One recording already going on */
		PVR_PRINT ( "DVR_UP> Playback not active in [%d]channel, State[%d]\n",
			ch,
			(UINT32)astDvrState[ch].eUpState );
		return -1;
	}
...
UINT32	PVR_DD_ResumeUpload ( LX_PVR_CH_T ch )
{
	...
	{
		/* One recording already going on */
		PVR_PRINT ( "DVR_UP> Playback not in Pause state-[%d]channel, State[%d]\n",
			ch,
			(UINT32)astDvrState[ch].eUpState );
		return -1;
	}
...
UINT32 DVR_SetDownloadPID(LX_PVR_CH_T ch, UINT32 ui32Pid, LX_PVR_PIE_TYPE_T ePieType)
{
	...
	//The interrupt level configuration is used for PIE buffer number calculation
	//DVR_DN_ConfigureIntrLevel(ch, DVR_DN_MAX_BUF_CNT, DVR_DN_MAX_PKT_CNT);

	// Set the index INT level to 384 x 1 download buffer.
	DVR_DN_ConfigureIntrLevel(ch, DVR_DN_MIN_BUF_CNT, DVR_DN_MIN_PKT_CNT);
	...
	switch ( ePieType )
	{
		...
		case LX_PVR_PIE_TYPE_OTHERS:
			...
			PVR_PRINT ( "PIE for H.264 not implemented yet !!!!!\n\n\n" );
...
UINT32	PVR_DD_MemoryInit ( LX_PVR_GPB_INFO_T	*stpLXPvrGPBInfo)
{
	...
	{
		stpLXPvrGPBInfo->uiDnBase = gMemCfgDvr[0][LX_PVR_MEM_DN].base;
		stpLXPvrGPBInfo->uiDnSize = gMemCfgDvr[0][LX_PVR_MEM_DN].size;

		stpLXPvrGPBInfo->uiUpBase = gMemCfgDvr[0][LX_PVR_MEM_UP].base;
		stpLXPvrGPBInfo->uiUpSize = gMemCfgDvr[0][LX_PVR_MEM_UP].size;

		stpLXPvrGPBInfo->uiUpBufBase = gMemCfgDvr[0][LX_PVR_MEM_UPBUF].base;
		stpLXPvrGPBInfo->uiUpBufSize = gMemCfgDvr[0][LX_PVR_MEM_UPBUF].size;

		stpLXPvrGPBInfo->uiPieBase = gMemCfgDvr[0][LX_PVR_MEM_PIEBUF].base;
		stpLXPvrGPBInfo->uiPieSize = gMemCfgDvr[0][LX_PVR_MEM_PIEBUF].size;
	}
...
UINT32 PVR_DD_GetTPHeader(UINT8 *pSingleTP, PVR_TP_HEADER_INFO_T *pTPHeader)
{
	...
	PVR_PRINT("Sync. Byte [0x%x]\n", pTPHeader->sync_byte);
	PVR_PRINT("Transport error [%1u]  Payload unit start [%1u] Priority [%1u]\n",
			pTPHeader->transport_error, pTPHeader->payload_unit_start, pTPHeader->transport_priority);
	PVR_PRINT("PID [0x%x]  Scrambling [%2u]  Adaptation [%2u]  CC [%u]\n",
	...
	#ifdef DEBUG_H264_HEADER_PASER
	PVR_PRINT("Start address [%p]  Current address [%p] Adaptation field length [%u]\n",
			pStart, pSingleTP, adaptationSize);
	PVR_PRINT("Payload offset is [%u]\n", offset);
	#endif
...
UINT32 PVR_DD_ReadNBits(PVR_BIT_STREAM_T *pBitStreamData, UINT32 nBits)
{
	...
	if (pBitStreamData != NULL && pBitStreamData->pData != NULL)
	{
		...
		{
			PVR_PRINT("Can't read more than 32 bits!!\n");
			return result;
		}
...
UINT32 PVR_DD_ParseVUI(PVR_BIT_STREAM_T *pBitStream, PVR_VUI_T *pVUI)
{
	...
	if (pBitStream != NULL && pBitStream->pData != NULL && pVUI != NULL)
	{
		...
		bitSize += pBitStream->bitSize;
		PVR_PRINT("^R^pVUI->tipFlag [ %u ] bitSize [%d]\n", pVUI->tipFlag, bitSize);
		...
		bitSize += pBitStream->bitSize;
		PVR_PRINT("^R^pVUI->nhppFlag [ %u ] bitSize [%d]\n", pVUI->nhppFlag, bitSize);
		...
		bitSize += pBitStream->bitSize;
		PVR_PRINT("^R^pVUI->vhppFlag [ %u ] bitSize [%d]\n", pVUI->vhppFlag, bitSize);
		...
		bitSize += pBitStream->bitSize;
		PVR_PRINT("^R^pVUI->pspFlag [ %u ] bitSize [%d]\n", pVUI->pspFlag, bitSize);

		pVUI->brFlag = PVR_DD_ReadNBits(pBitStream, 1);
		bitSize += pBitStream->bitSize;
		PVR_PRINT("^R^pVUI->brFlag [ %u ] bitSize [%d]\n", pVUI->brFlag, bitSize);
...
void PVR_DD_DumpHRD(PVR_HRD_T *pHRD)
{
	if (0)
	{
		PVR_PRINT("cpb_cnt_minus1 [ %u ]\n",pHRD->ccm1);			/* cpb_cnt_minus1 */
		PVR_PRINT("bit_rate_scale [ %u ]\n",pHRD->brs);		/* bit_rate_scale */
		PVR_PRINT("cpb_size_scale [ %u ]\n",pHRD->css);		/* cpb_size_scale */
		PVR_PRINT("initial_cpb_removal_delay_length_minus1 [ %u ]\n",pHRD->icrdlm1);	/* initial_cpb_removal_delay_length_minus1 */
		PVR_PRINT("cpb_removal_delay_length_minus1 [ %u ]\n",pHRD->crdlm1);		/* cpb_removal_delay_length_minus1 */
		PVR_PRINT("dpb_output_delay_length_minus1 [ %u ]\n",pHRD->dodlm1);		/* dpb_output_delay_length_minus1 */
		PVR_PRINT("time_offset_length [ %u ]\n",pHRD->tol);		/* time_offset_length */
	}
}
...
void PVR_DD_DumpSPS(PVR_SPS_T *pSPS)
{
	...
	if (0)
	{
		PVR_PRINT("^R^profileIdc [ %u ]\n", pSPS->profileIdc);
		PVR_PRINT("constraint_set0_flag [ %u ]\n", pSPS->cs0flag);
		PVR_PRINT("constraint_set1_flag [ %u ]\n", pSPS->cs1flag);
		PVR_PRINT("constraint_set2_flag [ %u ]\n", pSPS->cs2flag);
		PVR_PRINT("constraint_set3_flag [ %u ]\n", pSPS->cs3flag);
		PVR_PRINT("constraint_set4_flag [ %u ]\n", pSPS->cs4flag);

		PVR_PRINT("levelIdc [ %u ]\n", pSPS->levelIdc);
		PVR_PRINT("seq_parameter_set_id [ %u ]\n", pSPS->spsID);
		PVR_PRINT("chroma_format_idc [ %u ]\n", pSPS->cfIDC);
		PVR_PRINT("separate_colour_plane_flag [ %u ]\n", pSPS->scpFlag);
		PVR_PRINT("bit_depth_luma_minus8 [ %u ]\n", pSPS->bdlm8);
		PVR_PRINT("bit_depth_chroma_minus8 [ %u ]\n", pSPS->bdcm8);

		PVR_PRINT("qpprime_y_zero_transform_bypass_flag [ %u ]\n", pSPS->qyztbFlag);
		PVR_PRINT("seq_scaling_matrix_present_flag [ %u ]\n", pSPS->ssmpFlag);

		for (loopVar = 0; loopVar < ((pSPS->cfIDC != 3) ? 8 : 12); loopVar++)
		{
			PVR_PRINT("seq_scaling_list_present_flag%d [ %u ]\n", loopVar,  pSPS->sslpFlag[loopVar]);
		}
		PVR_PRINT("log2_max_frame_num_minus4 [ %u ]\n", pSPS->lmfnm4);
		PVR_PRINT("pic_order_cnt_type [ %u ]\n", pSPS->poct);
		PVR_PRINT("log2_max_pic_order_cnt_lsb_minus4 [ %u ]\n", pSPS->lmpoclm4);
		PVR_PRINT("delta_pic_order_always_zero_flag [ %u ]\n", pSPS->dpoazFlag);
		PVR_PRINT("offset_for_non_ref_pic [ %u ]\n", pSPS->ofnrp);
		PVR_PRINT("offset_for_top_to_bottom_field [ %u ]\n", pSPS->ofttbf);
		PVR_PRINT("num_ref_frames_in_pic_order_cnt_cycle [ %u ]\n", pSPS->nrfipocc);

		PVR_PRINT("max_num_ref_frames [ %u ]\n", pSPS->mnrf);
		PVR_PRINT("gaps_in_frame_num_value_allowed_flag [ %u ]\n", pSPS->gifnvaFlag);
		PVR_PRINT("pic_width_in_mbs_minus1 [ %u ]\n", pSPS->pwimm1);
		PVR_PRINT("pic_height_in_map_units_minus1 [ %u ]\n", pSPS->phimum1);
		PVR_PRINT("frame_mbs_only_flag [ %u ]\n", pSPS->fmoFlag);
		PVR_PRINT("mb_adaptive_frame_field_flag [ %u ]\n", pSPS->maffFlag);
		PVR_PRINT("direct_8x8_inference_flag [ %u ]\n", pSPS->d8iFlag);
		PVR_PRINT("frame_cropping_flag [ %u ]\n", pSPS->fcFlag);

		PVR_PRINT("frame_crop_left_offset [ %u ]\n", pSPS->fclo);
		PVR_PRINT("frame_crop_right_offset [ %u ]\n", pSPS->fcro);
		PVR_PRINT("frame_crop_top_offset [ %u ]\n", pSPS->fcto);
		PVR_PRINT("frame_crop_bottom_offset [ %u ]\n", pSPS->fcbo);

		PVR_PRINT("vui_parameters_present_flag [ %u ]\n", pSPS->vuippFlag);
...
UINT32 PVR_DD_AdjustH264StreamFrameRate(UINT16 *pFrameRateIndex)
{
	...
	*pFrameRateIndex = newFrameIndex;
	...
	PVR_PRINT("[%s] bFoundSeqSPS = %d: vuippFlag = %d, tipFlag = %d, gSPS.vuip.ts = %d, gSPS.vuip.nuit=%d, oldFrameIndex = %d, newFrameIndex = %d\n",
...
UINT32 PVR_DD_UpdateH264StreamInfo(BOOLEAN bFoundSPS, UINT8 *tpstream, UINT32 packetOffset)
{
	...
	//PVR_PRINT("Update H264 Strem Info. : Packet Offset [%d], SPS [%s]]\n", packetOffset, bFoundSPS ? "TRUE" : "FALSE");
	...
	{
		PVR_PRINT("Warning : Paket alignment is broken!\n");
		return PVR_FAILURE;
	}
	...
	else
	{
		...
		PVR_PRINT("^Y^tsDiff %d gRefTimeStamp %d gPictureNum %d\n", tsDiff, gRefTimeStamp, gPictureNum);
		...
		if (tsDiff >= PVR_TIME_STAMP_CLOCK)
		{
			...
			PVR_PRINT("^G^Cal FR RATE = %d (NR Pic [%d] Prev TS [%d] Cur TS [%d] TS Diff [%d])\n", frRate,	gPictureNum, gRefTimeStamp, curTS, tsDiff);
			gPictureNum = 0;
			gRefTimeStamp = 0;
