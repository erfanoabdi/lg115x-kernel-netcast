...
UINT32 DVR_RegInit(void)
{
	...
	{
		gpPvrReg = (DVR_REG_T *)ioremap(L9_DVR_REG_BASE, 0x500);
	}
	else
	{
		gpPvrReg = (DVR_REG_T *)(L8_DVR_REG_BASE);
	}
...
UINT32 DVR_UP_EnableReg(LX_PVR_CH_T ch ,BOOLEAN bEnable, UINT8 ui8PacketLen)
{
    ...
    switch (ch) {
        case LX_PVR_CH_A :
			...
			//One time TSC Config command for Timestamped stream
			if (ui8PacketLen == 192)
			{
