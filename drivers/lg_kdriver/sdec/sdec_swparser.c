...
void SDEC_SWP_Init(S_SDEC_PARAM_T *stpSdecParam)
{
	...
	if(stSWTSParser_Thread)
	{
		...
		SDEC_DEBUG_Print("Run SDEC SWTS Parser Thread PID:[%d]\n", stSWTSParser_Thread->pid);
	}
	else
	{
		SDEC_DEBUG_Print("SDEC SWTS Parser Thread Already Created\n");
	}
 ...
 * @see
 */
void SDEC_SWP_ComponentInit(void)
{
	...
	UINT32 ui32Arg)
{
	...
	stpLXSdecSecfltEnableSWParser =(LX_SDEC_SECFLT_ENABLE_SWPARSER_T *)ui32Arg;
	...
	if(bEnable)
	{
		SDEC_DTV_SOC_Message( SDEC_SWPARSER, "Send START to SDEC SWTS Parser Thread\n");
		OS_SendEvent( &stpSdecParam->stSdecSWPEvent, SWP_EVENT_START );
	}
	else
	{
		SDEC_DTV_SOC_Message( SDEC_SWPARSER, "Send STOP to SDEC SWTS Parser Thread\n");
...
BOOLEAN SDEC_SWP_Section_Proc(S_SDEC_PARAM_T *stpSdecParam, UINT8 ui8Ch, UINT8 ui8SecFltIdx)
{
	...
	if (check_one_bit(stpSdecParam->stSecMap[ui8Ch][ui8SecFltIdx].mode, 1))
	{
		...
		/* 2012. 11. 17 PVR_RELATED To Modify DN Channel B to A, PVR Use DN Channel B , SDT will use Ch A */
		PVR_DD_StopDownload(PVR_CH_FOR_SDT);
		SDEC_HAL_CDIC2PIDFSetPidfData(ui8PidFltIdx, 0x1FFF);
		SDEC_HAL_CDIC2PIDFEnablePidFilter(ui8PidFltIdx, SDEC_HAL_DISABLE);	/* seperate enable as L9 correction */
	}
	...
	while(!kthread_should_stop())
	{
		...
		if ( ret == RET_OK )
		{
			...
			else
			{
				...
				{
					SDEC_DTV_SOC_Message( SDEC_SWPARSER, "SDEC Parser Thread Received Start \n");
					...
					DVR_DN_GetBufBoundReg(PVR_CH_FOR_SDT, &ui32PhyDNBufStart, &ui32PhyDNBufEnd);
		...
		uiSize = 0;
		if( RET_OK != PVR_DD_GetDownloadWriteAddr(PVR_CH_FOR_SDT, &ui32PhyDNWritePtr))
		{
...
NEXT_TS:
		...
		if( RET_OK != PVR_DD_SetDownloadReadAddr(PVR_CH_FOR_SDT, (UINT32)SWP_DN_VIRT_TO_PHYS(pui8DNBuffer)))
		{
			SDEC_DEBUG_Print("KADP_PVR_DnGetWriteAddr Error");
			continue;
		}
