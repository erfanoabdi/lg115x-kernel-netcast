...
int SYS_Init(void)
{
...
#if 0
	OS_DEBUG_EnableModuleByIndex ( g_sys_debug_fd, 0, DBG_COLOR_NONE );
	OS_DEBUG_EnableModuleByIndex ( g_sys_debug_fd, 1, DBG_COLOR_NONE );
	OS_DEBUG_EnableModuleByIndex ( g_sys_debug_fd, 2, DBG_COLOR_RED );
	OS_DEBUG_EnableModuleByIndex ( g_sys_debug_fd, 3, DBG_COLOR_NONE );
#else
	OS_DEBUG_EnableModuleByIndex ( g_sys_debug_fd, 1, DBG_COLOR_NONE );
	OS_DEBUG_EnableModuleByIndex ( g_sys_debug_fd, 2, DBG_COLOR_RED );
#endif
	...
	if(!LX_COMP_CHIP(lx_chip_rev(), LX_CHIP_L9))	/* L9 */
	{
		BW_LimitCtrl_Init();
	}
...
//	SYS_TRACE("cmd = %08X (cmd_idx=%d)\n", cmd, _IOC_NR(cmd) );
	...
	switch(cmd)
	{
		...
		case SYS_IOW_WRITE_MEM_ARRAY:
		{
			...
			if(copy_from_user((void __user *)mem.data, (void*)data, mem.size) != 0)
			{
				OS_Free(data);
				return -EFAULT;
			}
