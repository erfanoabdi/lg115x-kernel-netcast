...
// setter
static int _H264SetBitrate( H1ENCODE_API_T *pstEncoder, u32 bitrate );
...
static int _H264Open( H1ENCODE_API_T *pstEncoder )
{
	...
	//int interlacedFrame = 0;
	int fieldOrder = 1;	// TOP-Field First
...
#if 0
	if ( rcCfg.bitPerSecond > 35000000 )
	{
			rcCfg.qpMin = 25;
		}
		else
	...
	// 25% lower target bitrate
	rcCfg.bitPerSecond = rcCfg.bitPerSecond - (rcCfg.bitPerSecond>>2);
	if ( rcCfg.bitPerSecond < H1ENCODE_BITRATE_MIN )
	{
		rcCfg.bitPerSecond = H1ENCODE_BITRATE_MIN;
	}
#else
	if ( rcCfg.bitPerSecond > 0 )
	{
		//rcCfg.hrdCpbSize = rcCfg.bitPerSecond / 2;
		if ( pstEncoder->frameRateNum > 0 && pstEncoder->frameRateDenom > 0 )
		{
			rcCfg.hrdCpbSize = rcCfg.bitPerSecond / ( pstEncoder->frameRateNum / pstEncoder->frameRateDenom ) * 5;
		}
		else
		{
			rcCfg.hrdCpbSize = rcCfg.bitPerSecond / 4;
		}
	}
	...
	codingCfg.fieldOrder = fieldOrder;
...
static int _H264Encode( H1ENCODE_API_T *pstEncoder, H1ENCODE_ENC_PARAMS_T *pstEncParams )
{
	...
	if ( pstEncoder == NULL )
	{
		VENC_ERROR("%s: pH264 is NULL.\n", __FUNCTION__);
		return RET_ERROR;
	}

	// 5. Encode the video frame
