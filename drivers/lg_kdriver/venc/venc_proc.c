...
static OS_PROC_DESC_TABLE_T	_g_venc_device_proc_table[] =
{
	...
	{ "printctrl",	PROC_ID_PRINT_CTRL, OS_PROC_FLAG_READ | OS_PROC_FLAG_WRITE },
...
static int	_VENC_ReadProcFunction(	UINT32 procId, char* buffer )
{
	...
	switch( procId )
	{
		...
		case PROC_ID_PRINT_CTRL:
		{
			int len = 0;
			len += sprintf( buffer + len, "VENC PRINT CTRL status\n");
			len += sprintf( buffer + len, "[%d]VENC NORMAL: %s\n", VENC_MSG_NORMAL, GET_VENC_MSG_INDEX( VENC_MSG_NORMAL ) ? "On":"Off" );
			len += sprintf( buffer + len, "[%d]VENC ERROR : %s\n", VENC_MSG_ERROR , GET_VENC_MSG_INDEX( VENC_MSG_ERROR ) ? "On":"Off" );
			len += sprintf( buffer + len, "[%d]VENC DEBUG : %s\n", VENC_MSG_DEBUG , GET_VENC_MSG_INDEX( VENC_MSG_DEBUG ) ? "On":"Off" );
			len += sprintf( buffer + len, "[%d]VENC TRACE : %s\n", VENC_MSG_TRACE , GET_VENC_MSG_INDEX( VENC_MSG_TRACE ) ? "On":"Off" );
			len += sprintf( buffer + len, "[%d]VENC TIMER : %s\n", VENC_MSG_TIMER , GET_VENC_MSG_INDEX( VENC_MSG_TIMER ) ? "On":"Off" );
			len += sprintf( buffer + len, "[%d]VENC INTER : %s\n", VENC_MSG_INTER , GET_VENC_MSG_INDEX( VENC_MSG_INTER ) ? "On":"Off" );
			len += sprintf( buffer + len, "[%d]VENC INFO  : %s\n", VENC_MSG_INFO  , GET_VENC_MSG_INDEX( VENC_MSG_INFO ) ? "On":"Off" );
			len += sprintf( buffer + len, "[%d]VENC WARN  : %s\n", VENC_MSG_WARN  , GET_VENC_MSG_INDEX( VENC_MSG_WARN ) ? "On":"Off" );
			len += sprintf( buffer + len, "\n");

			len += sprintf( buffer + len, "Usage: echo index on_off > /proc/lg/venc/printctrl\n");
			len += sprintf( buffer + len, "       index : 0 ~ %d\n", VENC_MSG_INDEX_COUNT);
			len += sprintf( buffer + len, "       on_off: 0(off), 1(on)\n");
			ret = len;
		}
		break;
...
static int _VENC_WriteProcFunction( UINT32 procId, char* command )
{
	...
	switch( procId )
	{
		...
		case PROC_ID_PRINT_CTRL:
		{
			...
			sscanf( command, " %d %d", &index, &on_off );

			if ( index >= 0 && index < VENC_MSG_INDEX_COUNT )
			{
				if ( on_off ) 
				{
					SET_VENC_MSG_INDEX( index );
				}
				else
				{
					UNSET_VENC_MSG_INDEX( index );
				}
			}
		}
		break;
