...
#if 1
	/* Start up out control thread bound to the CPU 0 not
	 * to disturb the V-sync interrupt bound to the CPU 1
	 */
	th = kthread_create(usb_stor_control_thread, us, "usb-storage:0");
	if (IS_ERR(th)) {
...
#ifdef CONFIG_USER_INITCALL_USB
user_initcall_grp("USB",usb_stor_init);
#else
